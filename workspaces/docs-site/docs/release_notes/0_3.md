...
- **Language/Compiler**: RFC 084 begins callable preset support with RHS partial declarations such as `pub get = partial route(method="GET")`, same-type method partials, local partial expressions, projected callable signatures where presets display as ordinary defaults, wrapper lowering for top-level function and constructor presets, public checked-API exports for projected partial signatures, and LSP default-argument signature help improvements (#453, RFC 084).
