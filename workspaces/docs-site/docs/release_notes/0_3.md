...
The first user-facing work in `0.3` now spans both language surface and tooling contracts. In particular, Incan adds RFC 049's `if let` / `while let` control flow together with a tighter formatter contract and a documented code-style guide, so both authored source and formatter output are becoming more explicit and predictable across CLI, editor, and library entry points.
...
There are no required source migrations for the current `0.3` work.
