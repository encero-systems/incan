...
See also: [`std.regex`](../language/reference/stdlib/regex.md), [Strings and bytes](../language/reference/strings.md), [Callable objects](../language/reference/stdlib_traits/callable.md), [RFC 059](../RFCs/closed/implemented/059_std_regex.md).
...
- **Language/Interop**: Trait-provided associated Rust methods now carry the selected trait method's inspected parameter shape into lowering, so calls such as `FileDescriptorSet.decode(encoded.as_slice())` preserve by-value generic slice arguments instead of falling back to a method-name borrow heuristic (#612).
...
- **Compiler/Rust interop**: Metadata-backed external Rust calls now preserve inspected generic by-value parameters, so prost-style inherent and trait-provided `decode<T: Buf>(buf: T)` calls pass owned cursors or borrowed slices directly instead of generating an invalid shared borrow (#609, #612).
