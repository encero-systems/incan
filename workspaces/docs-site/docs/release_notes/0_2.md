...
- **Compiler/Codegen**: helper calls inside repeated loops no longer lower shared `list[str]` arguments through consuming `into_iter()` conversions, so patterns like `out.append(match_index(xs, y))` preserve the outer list across iterations. The same ownership pass also tightened `list[str]` literal materialization, string-key dict lookups, and dict comprehensions that reuse non-`Copy` keys in their value expressions (#383).
- **Parser/Compiler**: `for` loops now accept tuple-unpack binding patterns such as `for idx, name in enumerate(xs):`, matching the documented syntax and lowering them to Rust tuple patterns instead of rejecting the comma before `in` (#389).
...
- **Compiler**: `len(...)` now lowers to a parse-safe `usize -> i64` expression in generated Rust comparisons, so expressions like `len(xs) < 2` no longer fail emission with a Rust syntax error (#380).
- **Compiler/Codegen**: helper calls inside repeated loops no longer lower shared `list[str]` arguments through consuming `into_iter()` conversions, so patterns like `out.append(match_index(xs, y))` preserve the outer list across iterations instead of failing with Rust move errors (#383).
