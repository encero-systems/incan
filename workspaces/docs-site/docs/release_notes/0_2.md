...
- **Web**: Web-free compiler — all web-specific knowledge (route registration, extractor types, response builders)
  lives in `std.web` stdlib modules and the `incan_web_macros` proc-macro crate. The compiler uses generic decorator
  passthrough (`@rust.extern` + `rust.module()`) and derive passthrough (`@derive(X)` from `rust.module()` traits)
  with no domain-specific web logic in the compiler core ([RFC 023]).
