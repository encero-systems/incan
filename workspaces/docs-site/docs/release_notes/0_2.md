...
- **Compiler / Stdlib**: `std.traits.*` now has a curated stdlib reference entry, and `std.traits.{ops,error,indexing,callable,prelude}` now has compiled-source coverage proving they already compile directly from `.incn` source without `rust.module()` / `@rust.extern` helper shims. `std.traits.convert` remains the explicit blocked closeout item because `from` / `try_from` still collide with the hard `from` keyword in declaration position; follow-up is tracked under RFC 043. (#306)
