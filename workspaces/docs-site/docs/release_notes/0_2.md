...
Use `static` when a module needs one live storage cell that persists across calls, for example counters, registries,
caches, or shared mutable collections.
...
This release also includes `pub static`, so library modules can export live shared state through library manifests and
`pub::` imports rather than only exporting frozen compile-time constants.
...
- **Compiler**: New `static` keyword for module-owned runtime state, plus `pub static` exports through library
  manifests and `pub::` imports ([RFC 052], #242). See [Module Static Storage](#module-static-storage).
...
- **Compiler**: Imported Rust free functions that require borrowed Rust-backed values now preserve borrow semantics
  through lowering, so generated Rust call sites emit `&value` / `&mut value` instead of incorrectly passing owned
  values (#257).
