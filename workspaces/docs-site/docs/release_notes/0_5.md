...
- **Compiler / stdlib**: Toolchain archives carry one relocatable checked stdlib seed containing the `.incnlib`, generated Rust crate, and resolved dependency closure. Installed compilers reuse that seed while consumer build and test projects link it instead of copying the migrated stdlib source closure. Cargo build intermediates and historical local identities are not shipped (#544).
