...
- **Compiler / stdlib**: Built-in `std.*` modules are compiled once into a local checked library artifact. Consumer build and test projects now link that artifact instead of copying the migrated stdlib source closure into every generated project (#544).
