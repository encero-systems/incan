...
- **Lockfiles**: Canonical dependency fingerprints now use checked SDK provider identities instead of physical cache paths, and rooted-workspace lock generation snapshots artifacts after preparation, so clean and relocated provider stores converge on the same lock (#921).
