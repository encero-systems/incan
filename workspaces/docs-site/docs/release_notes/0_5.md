...
- **Stdlib/tooling**: `std.registry` adds typed declaration catalogues, structural `@derive(Descriptor)` snapshots, non-wrapping `@describe` decorators, compilation-unit/package entries, deterministic checked inspection, package-artifact metadata, LSP membership navigation, and codegraph facade projections. The generated feature inventory now reads this checked source-authored catalogue rather than comment metadata (RFC 113, #575).
