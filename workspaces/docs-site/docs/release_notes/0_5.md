...
- **Compiler**: Published library Rust ABI metadata now comes from the complete effective dependency graph instead of the current rust-inspect prewarm cache, keeping cold and warm artifacts identical even when earlier commands inspected only part of the public surface (#922).
