...
- **Stdlib**: `std.environ` adds read-only current-process environment access through required, optional, defaulted string reads and trait-based `TryFrom[str]` typed reads. This is an initial RFC 089 slice; primitive/defaulted typed overloads, byte access, and environment mutation remain future #557 work (#557).
