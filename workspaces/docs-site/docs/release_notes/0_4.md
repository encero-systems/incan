...
- **Install and first run**: A release manifest, checksum-verified toolchain installer, Homebrew formula generation, npm and pip thin installers, and a zero-clone starter project make Incan usable without cloning the compiler repository first (#428, #553, #551).
...
- **Homebrew, npm, and pip adapters**: Homebrew formula generation, `@incan/toolchain` npm packaging, and `incan` pip/pipx packaging are thin adapters over the same toolchain manifest and archive contract. They are install channels, not separate compiler builds (#428).
