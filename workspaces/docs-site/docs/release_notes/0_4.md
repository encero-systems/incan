...
<section class="inc-release-summary" aria-label="Release 0.4 summary">
  <div><span>Theme</span><strong>Tryable from install</strong><p>A verified toolchain, zero-clone starter, and explicit first-run path.</p></div>
  <div><span>Highlights</span><strong>Inspect and automate</strong><p>Stable diagnostics, build reports, Rust inspection, and codegraph JSONL.</p></div>
  <div><span>Migration</span><strong>Low source impact</strong><p>Most 0.3 programs continue to compile; tooling gains canonical commands.</p></div>
  <div><span>Limits</span><strong>Focused platform support</strong><p>No native Windows or Linux arm64 installer in the initial release.</p></div>
  <div><span>Start</span><strong><a href="../../tooling/tutorials/getting_started/">Getting started →</a></strong><p>Install, run, test, and build a small project.</p></div>
</section>
...
This is primarily a tooling migration, not a language rewrite. Adopt the canonical commands below when scripts or integrations need stable structured output.
...
- **Generated feature inventory stays current**: 0.4 feature entries are present in the generated [feature inventory](../language/reference/feature_inventory.md), including install, starter, diagnostics, build reports/Rust inspection, build/test preheat observability, and codegraph inspection (#223, #554).
...
- [#223](https://github.com/encero-systems/incan/issues/223): 0.4 umbrella for the tooling, inspection, installer, starter, and first-contact release.
- [#554](https://github.com/encero-systems/incan/issues/554): release direction notes and explicit exclusions.
- [#592](https://github.com/encero-systems/incan/issues/592): RFC inspectability prompts for metadata, commands, diagnostics, provenance, generated output, and reports.
...
- [#589](https://github.com/encero-systems/incan/issues/589): stable machine-readable diagnostics through `incan check`.
- [#590](https://github.com/encero-systems/incan/issues/590): diagnostic explanation catalog through `incan explain`.
...
- [#666](https://github.com/encero-systems/incan/issues/666): RFC 102 semantic inspection umbrella.
- [#778](https://github.com/encero-systems/incan/issues/778): RFC 106 first slice for compiler-backed codegraph JSONL export.
- [#770](https://github.com/encero-systems/incan/issues/770): compiler-proven codegraph reference and call targets.

### Performance and observability

- [#707](https://github.com/encero-systems/incan/issues/707): visible prewarm/progress behavior for long-running build and test phases.
- [#723](https://github.com/encero-systems/incan/issues/723): test runner progress/performance observability.
- [#697](https://github.com/encero-systems/incan/issues/697): generated-library dependency preheat alignment when timing evidence justifies it.
- [#769](https://github.com/encero-systems/incan/issues/769): rust-inspect cold prewarm timing, cache reuse, and cache split explanation.

### Boundary parity and symbol identity

- [#760](https://github.com/encero-systems/incan/issues/760): compact boundary parity fixture suite.
- [#699](https://github.com/encero-systems/incan/issues/699): unified symbol identity across local/import/reexport/package/test/vocab/generated-Rust boundaries.
- [#753](https://github.com/encero-systems/incan/issues/753): partial presets constructing const metadata values.
- [#806](https://github.com/encero-systems/incan/issues/806): Rust interop receiver-side turbofish placement for generic associated constructors.

### Compiler correctness and interop hardening

- [#801](https://github.com/encero-systems/incan/issues/801): direct Rust `Result` interop calls keep usable return typing for source-level `?` across `incan check` and `incan build`.
- [#802](https://github.com/encero-systems/incan/issues/802): contextual `f32` float literals emit inferable Rust literals instead of hard-coded `f64` literals.
