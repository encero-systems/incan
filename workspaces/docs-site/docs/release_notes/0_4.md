...
- **Build reports and generated Rust inspection**: `incan build --report json` emits a versioned machine-readable build report for executable and library builds, and `incan inspect rust` provides an intentional way to inspect current generated Rust files without treating that output as a stable Rust ABI contract.
