...
- **Semantic inspection**: a compiler-backed codegraph export for files, modules, declarations, imports, exports, body-level reference and call syntax, diagnostics, spans, and degraded-state facts.
...
- **Build reports and generated Rust inspection**: `incan build --report json` emits a versioned machine-readable build report for executable and library builds, `incan inspect rust` provides an intentional way to inspect current generated Rust files without treating that output as a stable Rust ABI contract, and generated public Rust items preserve checked source docstrings as Rust doc comments when available.
- **Codegraph inspection**: `incan inspect codegraph` exports deterministic JSONL records for source files, modules, top-level declarations, imports, public exports, body-level reference and call syntax, containment, spans, provenance, and diagnostics. This is the concrete 0.4 codegraph slice for tooling and agent context, distinct from runtime `std.graph` and intentionally smaller than a resolved reference/call graph.
