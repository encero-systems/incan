...
- **Preheat observability**: `incan test -v` reports generated-harness planning, preparation-cache hits or misses, preheat timing, Cargo target directories, and Cargo execution timing, while ordinary console output remains focused on collected tests and visible long-running preheat work.
- **Generated-library preheat alignment**: `incan build --lib` preheats generated-library dependencies from the generated library Cargo project into the same release-profile Cargo target directory as the real generated library build, so unchanged manifest, lock, source, profile, and target inputs can reuse warmed Cargo artifacts instead of compiling the same dependency graph in a separate preheat domain.
- **Stable diagnostics**: `incan check` is now the canonical type-check command, `--format json` emits versioned diagnostics with stable codes and source spans, and `incan explain` provides catalog-backed help for those codes. The legacy `--check` spelling remains compatible.
- **Build reports and generated Rust inspection**: `incan build --report json` emits a versioned machine-readable build report for executable and library builds, and `incan inspect rust` provides an intentional way to inspect current generated Rust files without treating that output as a stable Rust ABI contract. Generated public Rust items now preserve checked source docstrings as Rust doc comments where those docstrings are available.
- **Codegraph inspection**: `incan inspect codegraph` exports deterministic JSONL records for source files, modules, top-level declarations, imports, public exports, containment, spans, provenance, and diagnostics. This is the concrete 0.4 codegraph slice for tooling and agent context, distinct from runtime `std.graph` and intentionally smaller than a full reference/call graph.
- **SDK installer and starter path**: 0.4 now has a manifest-driven SDK installer contract, tag-time SDK archive workflow, checksum-verified local installer script, and a zero-clone `incan new hello --yes` starter flow that runs, tests, and release-builds without cloning the compiler repository.
- **Release integration gate**: `make release-0-4-surface-gate` checks that release notes, roadmap scope, generated feature inventory, CLI/reference docs, installer/starter docs, and representative regression tests still describe the same staged 0.4 surface.
