...
The example shows the semantic grouping; the authored TOML grammar must be fixed before implementation and documented in the manifest reference. Every field is plan-visible before anything runs. `cfg` and `out` are data: the plan shows normalized flags and owner-relative digests of the committed files or trees. `native` and `tool` are publisher-side bake work with complete declared input and output closures. Their products enter the same admitted selected-unit and store path as other generated and native inputs; a consumer receives the finished archive and generated inputs and never executes either. A tool is a host-domain unit whose executable identity, inputs, outputs, selected target association, and receipt are part of the consuming closure.
...
2. A selected crate provider may read the crate package's `Cargo.toml` as constrained provider metadata after the typed `crate` dependency and source have already been selected. It may use only the metadata required to understand that dependency package's Rust units, dependency requirements, feature declarations, target conditions, inert build-script presence, and native-link facts. Reading metadata never authorizes build-script execution.
...
2. Each compile unit is keyed by the compiler/toolchain identity, normalized command inputs, source digests, dependency artifact identities, selected features, cfg facts, build host, target, profile, generated inputs, native-link facts, tool products, and the receipts that admitted those facts.
...
- A bounded compatibility matrix will leave Cargo compatibility as the explicit route for packages whose generated, native, or tool requirements cannot be expressed by the supported manifest contract.
...
- **Rust operational core/API** — must materialize and execute already selected compiler, native, tool, artifact, receipt, store, lease, and recovery operations without Cargo in native mode; it must not recover build-script execution from source inventory.
...
- **Plan:** before execution, Oven shows every Rust compile unit, host macro artifact, native and tool stage, inert build-script inventory warning, selected target/carrier/profile, exact input and output identities, and expected `*.loaf` asset/receipt. Proc-macro policy is shown separately from native/tool execution facts.
- **Artifacts and receipts:** each `*.loaf` asset exposes its facet, graph digest, payload digest, target, carrier, profile, generated/native/tool provenance, applicable proc-macro receipts, and final bake receipt.
...
- **Not implicit:** no Cargo project discovery, Cargo lock reuse, build-script execution, proc-macro execution, native/tool execution, linker search path, registry registration, or crates.io publication occurs merely because a file or dependency exists.
...
- [ ] Execute declared native and tool units only during publisher baking; refuse missing tools and undeclared outputs.
