...
The standard library is reached the same way, under one reserved spelling that mirrors the Incan one. Where Incan writes `from std.collections import OrdinalMap`, a Rust-authored unit in an Oven-managed project writes:

```rust
use incan::std::collections::OrdinalMap;

fn index_by_id(rows: Vec<(String, u64)>) -> OrdinalMap<String, u64> {
    let mut index = OrdinalMap::new();
...
`incan::std::<namespace>` is the caller surface of the standard-library namespace `std.<namespace>`, complete for every representable `pub` export whether or not this project calls it, baked once per toolchain release rather than derived from usage (see "Public export profiles"). The toolchain reserves the crate name `incan` for that umbrella; which standard-library component Loaf carries a given namespace is a fact of the standard-library ring, not part of the spelling a Rust caller writes. The plain Rust runtime facets the standard library links (`incan_std_core` and its siblings) remain implementation and are not the caller surface.
...
The standard library is the one exception to usage-derivation. Its caller facet is eager and complete: every representable `pub` export of every `std.<namespace>`, materialized once per toolchain release into the store under the reserved `incan::std::<namespace>` spelling, whatever any project happens to call. The reason usage-derivation is right for a user library — representability work and generated surface growing with exports nobody calls, on every bake — does not apply to a surface the toolchain ships, versions with the compiler, bakes once, and that a Rust author reaches for precisely to browse what is available.
...
The two questions have different answerers. The compiled union — "which `pub` exports does any Rust-authored unit in this project reference" — is answered per Rust-authored unit by reading that unit's own source for the paths it names under a caller namespace (`<library>::caller::incan::…` and `incan::std::…`); a Rust unit can only reach a caller-projected item by spelling such a path, so the scan is syntactic and complete. It is gated by the same source-digest staleness check that governs Oven build-unit reuse (RFC 119): it runs only for units whose source, or transitive dependency closure, changed since the last successful bake receipt, never as a full-project re-scan on every bake. Under RFC 124 that check is the unit identity: a caller artifact is a compiled unit whose identity includes the callee library unit it was built against, so a stale pairing is a different identity, not a warning. Usage detection that re-scans unchanged units, or that depends on an editor session being open, is not an acceptable implementation of this section. Discovery — "which `pub` exports are representable", the full eligible surface a developer can browse before writing a reference — is answered by the session-owned, incrementally-maintained semantic-facts service RFC 106 defines for LSP reference queries, the same machinery that already answers "find references" at interactive latency; the bake does not depend on it.
...
- **Eagerly build a caller wrapper for every `pub` export regardless of Rust-side usage** — Rejected for user libraries as distinct from the chosen design. It would flatten every public Incan symbol into the same Rust-hosted contract whether or not anything calls it from Rust, growing the checked/versioned caller surface with dead weight and doing needless representability-checking work on every bake. The standard library is the deliberate exception (see "Public export profiles" and Design decisions): baked once per toolchain release, its complete surface costs no per-bake work and is exactly what a Rust author browses.
...
- **The standard library's caller surface is `incan::std::<namespace>`, mirroring the Incan spelling, under a crate name the toolchain reserves:** `incan::std::collections::OrdinalMap` is the caller projection of `std.collections.OrdinalMap`. The toolchain reserves the crate name `incan` for one umbrella unit the standard-library ring bakes, whose `std::<namespace>` modules re-export the caller module of whichever component Loaf carries that namespace; the component boundary (the `sdk-components.toml` catalog) is an implementation fact of the ring and never part of the spelling. A user library keeps the general rule, `<library>::caller::incan::…`; the umbrella is for the standard library alone, so no user Loaf may take the name `incan`. The plain Rust runtime facets (`incan_std_core` and its siblings) stay implementation crates a Rust author does not import.
- **The standard library's caller facet is eager and complete, the one exception to usage-derivation:** every representable `pub` export of every `std.<namespace>` is projected, once per toolchain release, into the store — not the union of what a project's Rust units call. Usage-derivation exists to keep a user library's checked surface from growing with dead weight on every bake; a surface the toolchain ships, versions with the compiler, and bakes once carries no per-bake cost and is reached precisely to be browsed. The eager surface is still checked by the same representability rule, so an export the rule rejects is absent from `incan::std` rather than projected badly, and the identity record of the umbrella unit is the compiler release that baked it.
- **Usage detection for the compiled facet is a per-unit path scan keyed by RFC 124 identity; RFC 106's service owns discovery, not the bake:** a Rust-authored unit reaches a caller-projected item only by spelling a caller-namespace path, so "which exports does this unit reference" is read from the unit's own source and re-read only when that unit's identity changes. That meets the staleness bar "Public export profiles" sets without an editor session or a semantic-facts service in the bake's critical path. The service RFC 106 defines answers the other question — the full representable surface a developer can browse before writing a reference — for LSP completion and `incan inspect caller`; an implementation of this RFC may land the bake-side derivation before that service exists.
