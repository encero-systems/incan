...
`incan.pub` is the registry for Oven-built projects. Its unit of publication is the Loaf, whatever facets the Loaf carries, so an Incan library, a Rust-only crate that adopted `loaf.toml`, and a mixed project publish and resolve the same way. The registry stores two tiers of immutable, content-addressed artifacts: **source Loaves**, which are the authoritative publication signed by their publisher, and **baked assets**, which are the target-bound `*.loaf` outputs Oven already seals, each carrying its receipt and an attestation that binds it to a source digest and a toolchain. A consumer whose plan matches an available asset downloads it instead of compiling; when no asset matches, the consumer bakes from source. Discovery goes through a static sparse index of scoped names; integrity through per-artifact checksums; trust through keyless signing and trusted publishing from the first release; state changes through signed registry events. crates.io remains a consumption-only source reached through the same client, cache, and lockfile. This RFC supersedes RFC 034, which was written before Loaves existed.
...
Rust itself has asked for this since 2015: the Cargo issue for pre-built dependencies is still open, the Cargo team's 2026 project goal is a cross-workspace build cache that may later be pre-populated from a remote cache, and the one time a major crate shipped a precompiled artifact without a source of record, a verifiable link to it, or a way to decline it, the ecosystem rejected it within a month and its maintainer asked for a first-class mechanism instead. The two-tier design here is that mechanism.
...
- Define integrity, signing, trusted publishing, and consumer-side verification as normative behavior from the first release.
...
The reservation of unscoped names for the toolchain is `incan.pub` policy. A registered compatible registry sets its own naming policy, so a private registry may serve unscoped names; a client must never treat a name as the same package across two registries. Package names on `incan.pub` and crate names on crates.io are distinct namespaces. A `loaf.toml` dependency selects the registry through its dependency key (`loaf` or `crate`) as RFC 117 defines; the registry must not attempt to disambiguate bare names across sources.
...
Index lines must stay small: no descriptions, readmes, or asset tables. The per-version asset manifest at the recorded path must list every attested asset with its archive digest, its discovery facts, the RFC 124 unit identity and payload digest of every unit it carries, its builder kind, and its attestation reference, so that admission is decided from the manifest before any asset is downloaded. Both files must be served as static, cacheable content, and the registry should serve them from a CDN.
...
Two signatures therefore appear on registry state, and they answer different questions. A publisher-originated change (a publish, a yank, an ownership or trusted-publisher change) must be authorized by an identity the scope permits, and that authorization must be recorded in the event; the registry must then sign the event into the log with a delegated key. The first signature says who asked; the second says the registry accepted it and when. Publisher signatures on source Loaves are unchanged by this section.

### Encrypted artifacts

A registry may serve artifacts encrypted for its authorized clients, for example a private registry protecting its assets against disclosure of the storage behind it. When it does, unit identities, payload digests, receipts, and attestations remain over the plaintext, so that verification and reuse are identical to the unencrypted case, and the asset manifest must additionally carry the digest of the ciphertext as the transport digest. Clients must verify the transport digest before decryption and the payload digest after it. Key distribution is outside this RFC.

### Registry events

Every state change other than a first publish is a signed event: `asset`, `yank`, `unyank`, `ownership`, `trusted-publisher`, `advisory`, `supersede`, and the key-management events `delegate`, `rotate`, and `revoke`. Events must be signed by an identity authorized for the scope at the time of the event, must be append-only, and must be publicly readable. The index, the asset manifests, and the web site must be derivable from the event log plus the immutable artifacts alone. This is the package-level substrate the RFC 079 artifact graph projects from.
...
`crate` dependencies resolve against the crates.io sparse index. `incan.pub` is the leading protocol: Oven's own registry client is designed for it, and crates.io is the secondary source it also reads. Acquisition of crates.io source archives may, in the 0.6 line, use the Cargo binary the Incan toolchain already provisions as a download tool and nothing more: Cargo fetches bytes into a location Oven controls, and Oven performs verification against the sparse index checksum, storage, lock recording, and everything after. Cargo must never resolve, plan, or build on this path. A later release may fetch crates.io archives directly through the same client that serves `incan.pub`, since the sparse protocol is the simpler of the two; that change alters no contract in this RFC. Oven must parse a crate's manifest only as provider metadata under the constraints RFC 119 places on it. The registry must not mirror crates.io source. The registry may publish registry-built assets for crates.io crates at selected closures. Such an asset does not descend from a publisher-signed source Loaf and must not be presented as if it did. Instead it descends from a signed **external-source record** that binds the upstream registry identity, package name, version, and the crates.io checksum of the archive the bakery consumed, together with the registry's builder attestation. External-source records and their assets must be indexed separately from scoped packages at `external/<upstream-registry-id>/<name>`, keyed by upstream package name and version so that a client which has already resolved a `crate` dependency against crates.io can look up, by that identity, whether registry-built assets exist, and the consumer's selected source kind in `loaf.toml` remains authoritative: a `crate` dependency is always resolved against crates.io for its source of truth, and an `incan.pub` asset for it is only ever an admitted shortcut for units whose identities match.
...
`loaf.toml` already distinguishes `loaf` and `crate` dependency keys and lets a project name a registered source. This RFC adds no manifest syntax. The `requires` index field maps to the requirement ranges RFC 117 lets a project declare. Registry trust policy, credentials, and registered sources stay in Oven-controlled user or organization configuration, never in `loaf.toml`.
...
- **`@scope/name` spelling.** Rejected in favor of `scope/name`. The `@` character is the conventional version separator in tool-run syntax (`tool@1.2`), and the slash form matches container and source-forge conventions.
...
- How are scopes claimed and verified: free claim on first publish, linked to a source-forge organization, or both with different display?
