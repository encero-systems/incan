...
the generated project depend primarily on `incan_stdlib` (with features), while user `rust::...` imports continue to be governed by `incan.toml` and lock policy.
...
types. Type definitions (`model`, `enum`, `class` in the example below) are compiled normally from their Incan source; only the items whose *bodies* are Rust-provided carry the marker.
