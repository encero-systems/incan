...
- **SDK provider:** `stdlib-core` must publish `std.registry` and the source-owned capability catalog so minimal, default, and full SDK profiles share the same checked registry contract.
