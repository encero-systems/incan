...
- A `format` entry must satisfy the same invariants and one more: it may change how the program is written and nothing about its structure. Because it has no level, it is applied to every file, so the bar is higher than `fix`'s, not lower. Where the admissibility of a normalization depends on the text it meets, the entry states the condition and the formatter proves it per construct, leaving the construct as written where the proof fails; `fmt_nested_fstring` under "Formatter entries" is the worked example, and its proof is a lex-and-compare of the two spellings. A normalization the formatter cannot prove on the spot is not a `format` entry.
...
2. **Select and apply.** From the original findings, those whose entry has fix mode `fix` and whose effective level at the location is not `allow` are selected, and their rewrites are applied to the tree. A finding at `allow` is neither reported nor fixed; a finding at `warn`, `deny`, or `forbid` is fixed. An entry with fix mode `suggest` or `none` is reported as under `incan architect` and never applied.
...
5. **Report.** The report is RFC 105's finding envelope and nothing beside it: no separate array of applied fixes. It holds every original finding whose fix was applied, marked `applied: true` and at its original span, and every finding of the final evaluation, marked `applied: false` and at its final span. Record identity in the final set is the pair (rule, final span), so a rule that still fires at the same construct after its fix was attempted is one record of the final evaluation, not two; a `suggest` or `none` finding appears once, in the final set, because nothing was applied to it.
...
| `format_in_format_args` | | `fmt_nested_fstring`: an f-string placeholder that is itself an f-string is spliced into the outer string. The lexer scans a placeholder by brace depth, so an f-string inside a placeholder is a form the language accepts, and the splice is admissible only where re-lexing the spliced text yields exactly the same sequence of literal and placeholder parts as the two-level form: each placeholder's expression text and format spec unchanged, literal braces `{{` and `}}` preserved, and escapes decoding to the same text. The formatter proves it by lexing both spellings and comparing the part sequences, and leaves the construct as written otherwise (syntax; format) | |
...
| `to_string_in_format_args` | | `str_in_fstring`: `f"{str(x)}"` where `f"{x}"` is meant; both go through the Display protocol, so the value is the same. Whether `str` at the call site is the builtin is a resolution fact, not a tree fact: only `print` is protected from rebinding, so `str` may be a local, a parameter, or an import, and a rule that needs resolution is a `semantic` entry whose fix is a suggestion. In `style` rather than clippy's `perf`, because the two spellings go through one protocol and the redundant call is spelling, not cost (semantic) | |
...
| `range_minus_one` | `range(a..=(b - 1))` where `range(a..b)` is meant; the two differ where `b - 1` underflows, which fails the original at run time, and that edge is an observable change, so the rewrite does not meet the `fix` contract and is a suggestion (syntax) | | |
| `range_plus_one` | `range(a..(b + 1))` where `range(a..=b)` is meant; the two differ where `b + 1` overflows, which fails the original at run time, and that edge is an observable change, so the rewrite does not meet the `fix` contract and is a suggestion (syntax) | | |
...
- A run in which one local name is bound twice is left as written: nothing in it is sorted, merged, or de-duplicated. The binding itself is the checker's error (`ambiguous_import_binding`, `INCAN-I0001`), not a lint, so there is no catalog entry for it; `incan fmt --check` reports the run under `fmt_import_order` with the reason "import run left as written: `X` is bound more than once", so the file is not passed off as canonical in silence. The report is a note: the file would not change, so `--check`'s exit code is unaffected, and the checker refuses the file on its own. Reordering is meaning-preserving because it cannot change resolution, and that run is the one case where it might.
...
| `fmt_nested_fstring` | this RFC (clippy `format_in_format_args`) | an f-string in a placeholder spliced into the outer string, only where lexing the spliced text yields the same literal and placeholder parts as the two-level form; left as written otherwise |
...
| `fmt_import_order` | this RFC | imports sectioned by root, sorted, merged, and de-duplicated as "Import ordering" defines; a run that binds one name twice is left as written and reported by this name under `--check` |
...
Formatter invariants apply to the class as a whole: idempotency, parse preservation, and opacity of string literal bodies (RFC 053's string-and-comment safety rule). A file that does not parse is not formatted. The same invariants bind every `fix`-mode entry `incan architect --fix` applies, which is why its output can be handed to the formatter without a second parse. The bar for a `format` entry is higher than for a `fix`, because the class runs on every file with no level to turn it off, and `fmt_nested_fstring` shows what meeting it means: the entry does not assume the splice is an identity, it proves it per construct by lexing the spliced text and the two-level form and comparing their part sequences, and it leaves any construct the proof does not cover as written. The opacity rule is why that proof is a lex-and-compare and not a reading of the string: the formatter never interprets a literal's value, so equality of the lexed parts is the only equality it may claim.
...
- Formatter output on already-formatted code does not change until a `format`-class entry lands or changes. Each such entry changes formatter output for code that matches it; each is listed in the release notes and projects see it as a one-time reformat. There is no preview period for the class, because a `format`-class entry has no level to hold at `allow`: the release note is the notice. The eight new `format`-class entries this RFC names beyond RFC 053 and the style guide (`fmt_decorator_spacing`, `fmt_parentheses`, `fmt_digit_grouping`, `fmt_fstring_prefix`, `fmt_nested_fstring`, `fmt_redundant_pass`, `fmt_bare_return`, `fmt_import_order`) are that reformat for this RFC.
...
`incan architect --fix` could apply `comparison_to_empty` or `useless_conversion`, which need types, since the engine has them. Rejected for this RFC: a `fix` entry must be applicable to any file that parses, in every configuration, so that `--fix` never depends on the checked configuration and never rewrites one branch on facts from another; the fixable set is by construction the type-independent set. Type-dependent fixes are the `unsafe` mode a later RFC may add. Resolution-dependent rewrites fall on the same side: `str_in_fstring` needs to know that `str` at the call site is the builtin, which is a resolution fact, so it is a `semantic` suggestion and not a `fix`.

### Reserving the conversion builtins so `str_in_fstring` can be a fix

An earlier draft of this RFC made `f"{str(x)}"` to `f"{x}"` a `format`-class normalization guarded by "no local binding named `str` in the file", on the assumption that the tree shows every rebinding. It does not: only `print` is protected from rebinding, and `str` may arrive as a parameter, a local, or an import, so the guard is a resolution question. Reserving `str`, `int`, `float`, and `bool` the way `print` is reserved would make the guard a tree fact and the rewrite a `fix`. Rejected: rebinding a builtin is allowed by design and stays so; the language does not change to make a rewrite syntax-decidable, and a lint that needs a resolution fact is a `semantic` entry.

### Approximating the nested f-string splice

`fmt_nested_fstring` could splice any inner f-string whose placeholder has no format spec, on the reasoning that the value is unchanged by construction. Rejected: the reasoning is about the value, and the formatter never reads a literal's value (RFC 053's opacity rule); what it can compare is what the lexer produces. So the entry proves each splice by lexing the spliced text and the two-level form and comparing their part sequences, and leaves as written every construct where the sequences differ, which is the higher bar a `format` entry carries for running on every file with no level to turn it off.
...
- **Relationship to #159.** Rules are defined against the trivia-aware tree that #159 introduces, so comment and docstring trivia are inputs to a rule rather than repair work after it. Two sets follow from the classification in the design decisions. The `format` class gains eight entries beyond RFC 053 and the style guide; the current AST-based formatter can host the six whose normalization moves no comment (`fmt_parentheses`, `fmt_digit_grouping`, `fmt_fstring_prefix`, `fmt_nested_fstring`, `fmt_redundant_pass`, `fmt_bare_return`), and the two that move trivia or reorder declarations (`fmt_decorator_spacing`, `fmt_import_order`) wait for #159. The `fix` set is seven entries; `incan architect --fix` can apply the five whose rewrite moves no comment (`needless_bool`, `needless_bool_assign`, `needless_else`, `list_init_then_append`, `while_true`) on the current tree, and the two that move a branch body's comments with the body (`if_not_else`, `redundant_else`) wait for #159.
...
- **CLI / tooling**: `incan architect` resolves levels from the manifest through the lattice, distinguishes eligibility from level, carries the catalog fields in its findings, and gains `--fix` (with `--diff`), which applies `fix`-mode findings, formats the result, re-evaluates it, and reports and exits from the final evaluation; `incan fmt --check` reports by rule, including an import run left as written, and gains `--format json`; `incan explain` resolves `INCAN-L` codes and bare rule names and renders the documentation template; `incan architect --list-rules` is a new flag on RFC 105's surface and lists the catalog with group or class and fix mode, and, for a lint entry, preview flag, eligibility, and effective level.
...
- `incan architect --fix` follows the lifecycle: it evaluates the original tree, applies every `fix`-mode finding at an effective level other than `allow` and none at `allow`, formats the result, re-evaluates it, reports the applied originals as `applied: true` and every final finding as `applied: false` in RFC 105's envelope with no separate applied-fixes array and with (rule, final span) as the identity of a final record, takes its exit status from the final evaluation alone, honors `--diff`, and passes an idempotency property test per `fix` entry and for the set as a whole, with the seven `fix` entries being `needless_bool`, `needless_bool_assign`, `needless_else`, `redundant_else`, `if_not_else`, `list_init_then_append`, and `while_true`;
- `incan fmt --check` reports by rule name, fails on the `format` class and on nothing else, reads no lint table, reports an import run that binds one name twice under `fmt_import_order` without failing on it, and emits the schema 2 report under `--format json`; `incan fmt` applies the class, the idempotency property test covers each `format`-class entry, and `fmt_nested_fstring` is covered by a test in which the lexed part sequences differ and the construct is left as written;
- `str_in_fstring` is a `semantic` entry evaluated by `incan architect` with fix mode `suggest`, and reports nothing where `str` at the call site resolves to a binding other than the builtin;
...
- **The twenty, re-sorted by kind.** The twenty type-independent rewrites an earlier draft gave the formatter are classified as representation, as structure, or as neither. Representation, and therefore the `format` class with no level, eight entries: `precedence` and `double_parens` (as `fmt_parentheses`), `unreadable_literal` and `inconsistent_digit_grouping` (as `fmt_digit_grouping`), `empty_line_after_outer_attr` (as `fmt_decorator_spacing`), `redundant_pass` (as `fmt_redundant_pass`), `needless_bare_return` (as `fmt_bare_return`), `useless_format`'s placeholder-free form (as `fmt_fstring_prefix`), `format_in_format_args` (as `fmt_nested_fstring`, under the lex-and-compare proof), and import ordering (as `fmt_import_order`). Structure, and therefore fix mode `fix` under `incan architect --fix`, seven entries: `needless_bool`, `needless_bool_assign`, `needless_else`, `redundant_else`, `if_not_else`, `list_init_then_append`, and `while_true`. Neither, and therefore `suggest`: `range_plus_one` and `range_minus_one`, whose rewrite is not an identity at an overflowing bound, and `to_string_in_format_args` (as `str_in_fstring`), whose guard is a resolution fact. Everything type-dependent stays `suggest`.
- **`range_plus_one` and `range_minus_one` are suggestions.** `range(a..(b + 1))` and `range(a..=b)` differ where `b + 1` overflows; the original fails there at run time, but integer overflow is not a documented language contract, so the difference is an observable change, and the `fix` contract (meaning preserved for every well-typed program in every configuration) is read strictly rather than loosened for the case. The `fix` set is the seven entries above.
- **`str_in_fstring` is a semantic suggestion.** A builtin name may be rebound by a local, a parameter, or an import; only `print` is protected. Whether `str(x)` in a placeholder is the builtin is therefore a resolution fact, a lint that needs a resolution fact is a `semantic` entry, and the language does not change to make a rewrite syntax-decidable. The entry sits in `style` at `warn`, with the clippy correspondence (`to_string_in_format_args`) recorded, and nothing rewrites it.
- **`fmt_nested_fstring` proves each splice.** A `format` entry runs on every file with no level to turn it off, so its bar is higher than a `fix`'s. The splice is admissible only where re-lexing the spliced text yields exactly the same sequence of literal and placeholder parts as the two-level form (expression text and format spec of each placeholder unchanged, `{{` and `}}` preserved, escapes decoding to the same text); the formatter lexes both and compares the part sequences, and leaves the construct as written otherwise. That check is the proof, and a normalization without one is not a `format` entry.
- **`incan architect --fix` has one lifecycle.** Evaluate the original tree; select and apply the eligible `fix`-mode findings whose effective level is not `allow`; run the `format` class over the result; re-evaluate the result; report every applied original as `applied: true` and every final finding as `applied: false` in RFC 105's envelope, with no separate applied-fixes array and with (rule, final span) as the identity of a final record, so a rule that still fires at the construct its fix was attempted on is one record; take the exit status from the final evaluation exclusively.
- **The documentation test proves the evaluator's contract.** A lint entry's "before" produces the finding and its "after" does not, through the engine; formatting a `format`-class entry's "before" produces its "after" and formatting the "after" leaves it unchanged; `incan architect --fix` on a `fix` entry's "before" equals the formatted "after". One template, three proofs, chosen by the declared evaluator.
- **Preview is a property of lint entries.** A `format`-class entry has no preview flag in the entry shape, in `--list-rules`, or in the rule documentation header: it has no level to hold at `allow`, so a new normalization ships as a formatter change, listed in the release notes, and a project sees a one-time reformat.
- **A duplicate import binding is the checker's.** No lint entry reports it: the checker already refuses an ambiguous binding (`ambiguous_import_binding`, `INCAN-I0001`). `fmt_import_order` leaves such a run as written, and `incan fmt --check` reports it under that name with "import run left as written: `X` is bound more than once", without failing on it.
