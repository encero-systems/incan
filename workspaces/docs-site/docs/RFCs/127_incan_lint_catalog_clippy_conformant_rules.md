...
3. **Three tools, three jobs.** `incan check` reports what the type system knows. `incan architect` evaluates the catalog and reports findings, and under `--fix` applies the catalog's safe structural fixes. `incan fmt` canonicalizes representation. None of the three duplicates another's analysis: the engine is RFC 105's, the formatter applies the `format` class and calls no lint rule, and `--fix` output is itself formatted before it is written.
...
7. **Rewrites are two.** `incan fmt` canonicalizes representation: it applies the `format` class and nothing else, so its output is the same for every project. `incan architect --fix` applies safe structural fixes: a `fix`-mode entry is decidable on the syntax tree alone, meaning-preserving for every well-typed program in every configuration, idempotent, and structure-preserving, and it is applied under the lint levels, so `allow` means not applied. No other command rewrites source. In Ruff's vocabulary both are safe fixes and every suggestion is display-only; the catalog has no unsafe tier, because the line is drawn by construction, not by judgment per rule.
...
- Define fix applicability in Ruff's terms, so a reader who knows safe, unsafe, and display-only fixes knows what `format`, `fix`, `suggest`, and `none` mean.
- Define preview gating for entries that are not yet stable, as eligibility distinct from level, and import ordering as a `format`-class normalization over Incan's namespace roots.
...
- Statement-level suppression. `@allow(...)` attaches to declarations; a narrower form is an open, non-gating question. A blanket line directive that names no rule (Ruff's bare `# noqa`) is not a question: it is rejected, because a suppression that names nothing suppresses everything.
...
The same catalog holds rules nothing rewrites. `too_many_arguments` needs a threshold and a human decision; `unwrap_used` is a policy, not a rewrite; `float_cmp` needs to know that both operands are floats. `incan architect` reports those, and the level table decides whether they are advice (`warn`) or enforcement (`deny`). The defaults are clippy's, and clippy denies `correctness`: a project that writes no `[incan.lints]` at all and contains an `eq_op` or a `never_loop` fails `incan architect` outright. That is the reference's calibration, adopted as it stands, and it is the one way a default project can fail the command with no configuration.
...
- `warn`: the rule is reported; `incan architect`'s exit code is unaffected; `incan architect --fix` applies its fix when the entry's fix mode is `fix`.
- `deny`: the rule is reported; `incan architect` exits non-zero; `incan architect --fix` applies its fix when the entry's fix mode is `fix`, after which the finding no longer exists.
...
| `fix` | safe | `incan architect --fix`, at any effective level other than `allow` | meaning preserved for every well-typed program in every configuration, comments preserved, idempotent, structure preserved except at the reported construct |
...
- A `fix` must be meaning-preserving for every well-typed program in every configuration the file can be compiled under, regardless of the types involved (a `syntax`-tier rule sees every conditional-compilation branch, not only the checked one), idempotent (`fix(fix(x)) == fix(x)`), and structure-preserving (the syntax tree after the rewrite differs from the tree before it only at the reported construct and its trivia). These are #159's invariants, stated per entry.
- A `format` entry must satisfy the same invariants and one more: it may change how the program is written and nothing about its structure. Because it has no level, it is applied to every file, so the bar is stricter, not looser.
...
Levels are ordered `allow < warn < deny < forbid`, and the effective level of a rule at a source location is a lattice join over the policy layers followed by one scoped exemption:

```text
policy_level(rule)           = workspace(rule)           if the policy table assigns the rule
                             = catalog_default(rule)     otherwise

project_level(rule)          = max(policy_level(rule), member(rule))
...
- the **policy table** is `[workspace.incan.lints]` in a workspace, and the project's own `[incan.lints]` in a project that is not a workspace member, because that table is then the top of the policy chain; `workspace(rule)` is the level it assigns the rule after priority resolution inside that one table, directly, by group, or by `all`. The policy table is the one layer that may set a rule below its catalog default, because it is the policy authority and the defaults are policy;
- `member(rule)` is the level a workspace member's `[incan.lints]` assigns the rule after priority resolution inside that one table, and is `policy_level(rule)` when the table does not assign the rule; priority resolution happens inside each manifest layer independently and never across layers;
...
- a workspace member's manifest assigns a rule a level below its policy level (the workspace table's assignment, or else the catalog default);
...
- Its effect is the `targeted_allow` of the lattice above: the named rules are `allow` for the decorated declaration and everything lexically inside it. Naming a group allows every rule in the group.
...
`--fix` is a new flag on RFC 105's command surface, added by this RFC, and it is Ruff's `check --fix`: the same evaluation as `incan architect`, followed by the application of every finding whose entry has fix mode `fix`.

- It must apply a `fix`-mode entry at a location iff the entry is eligible under the invocation and its effective level at that location is not `allow`. A finding at `allow` is neither reported nor fixed; a finding at `warn`, `deny`, or `forbid` is fixed. An entry with fix mode `suggest` or `none` is reported as under `incan architect` and never applied.
- The rewritten file must be run through the formatter, that is, the `format` class, before it is written, so the file `--fix` leaves behind is canonical. The two rewrites cannot conflict because no `fix` entry may report a construct the `format` class normalizes; there is no order to document between them.
- The output must not depend on the order in which `fix` entries are applied; a pair of entries whose rewrites do not commute is a catalog defect, not a documented ordering. Applying the set must be idempotent as a whole: `incan architect --fix` twice must leave the file as once did.
- `--fix --diff` must print the rewrite each fix would make and write nothing, and must exit non-zero when any fix is pending, so a CI job can ask whether a tree is fully fixed without changing it.
- A file with a syntax error is refused for fixing, as `incan fmt` refuses it; a file the checker rejects is fixed for `syntax`-tier entries only, which every `fix` entry is.
- The report `--fix` prints is the ordinary finding report, with each applied finding marked applied (`applied: true` in JSON), so a reader sees what changed and what remains. The exit code is computed over the findings that remain after the fixes are applied.
...
`incan fmt --check --format json` must emit the `incan check --format json` envelope, schema 2: `schema_version`, `ok`, and `diagnostics`, whose entries are diagnostic records in schema 2's shape: a stable `code`, `severity`, `phase`, `origin`, a `primary_span` with the file and `start` and `end` positions in line, column, and offset form, `message`, `notes`, `hints`, related spans, and the `incan explain` hook. Each catalog record must additionally carry `rule`, `group` (the group name, or `format` for the class), `fix` (the entry's fix mode), and, for an entry with a level, `level` (the effective level at the location) and `level_source` (the layer that set it). `severity` is presentation derived from `level`: `warn` renders as `warning`, and `deny` and `forbid` render as `error`; a `format`-class record, which has no level, renders as `warning`, because a pending representation change is not an error in the program. `ok` is `true` iff no file would change. The `phase` of a formatter record is `format`. A consumer that reads `incan check --format json` today must read this report unchanged: the shared fields keep their names and shapes, and the added fields are ignorable.
...
| --- | --- | --- | --- |
| header | (badges) | name, code, group and default level (or the `format` class), origin and clippy counterpart, fact tier, fix mode, preview flag | `code`, `title`, `severity`, `phase` |
...
| `incan architect --fix` | no finding at `deny` or `forbid` remains after the fixes are applied | at least one such finding remains, a file could not be written, or an operational error |
...
| `format_in_format_args` | | `fmt_nested_fstring`: an f-string placeholder that is itself an f-string is spliced into the outer string; only a placeholder with no format spec, where the value is unchanged by construction (syntax; format) | |
...
| `to_string_in_format_args` | | `fmt_str_in_fstring`: `f"{str(x)}"` becomes `f"{x}"`; both go through the Display protocol, so the value is unchanged. Only a placeholder with no format spec, and not in a file that declares or imports its own binding named `str`, which the tree shows (syntax; format) | |
...
| `vec_init_then_push` | | `list_init_then_append`: `xs = []` followed only by `xs.append(...)` statements, where a list literal is meant; the annotation, if any, is kept, and a run in which an appended expression mentions `xs` is reported without a fix (syntax; fix) | |
...
| `range_minus_one` | `range(a..=(b - 1))` where `range(a..b)` is meant; the two differ only where `b - 1` underflows, which fails the original at run time and is the one place the rewrite is not an identity (syntax; fix) | | |
| `range_plus_one` | `range(a..(b + 1))` where `range(a..=b)` is meant; the two differ only where `b + 1` overflows, which fails the original at run time and is the one place the rewrite is not an identity (syntax; fix) | | |
...
| `duplicate_import_binding` | `suspicious` | `incan` | syntax | none | an import run in which one local name is bound twice, which `fmt_import_order` leaves as written; the entry names the two bindings and says why the run was not sorted, beside whatever the checker says about them (a duplicate or ambiguous binding is a checker error in the configuration it is compiled under) |
...
- A run in which one local name is bound twice is left as written: nothing in it is sorted, merged, or de-duplicated, and `duplicate_import_binding` reports it. Reordering is meaning-preserving because it cannot change resolution, and that run is the one case where it might.
...
| `fmt_nested_fstring` | this RFC (clippy `format_in_format_args`) | an f-string in a placeholder without a format spec spliced into the outer string |
| `fmt_str_in_fstring` | this RFC (clippy `to_string_in_format_args`) | `str(x)` in a placeholder without a format spec written `x`; not in a file that binds its own `str` |
| `fmt_redundant_pass` | this RFC | `pass` removed from a block that has other statements |
| `fmt_bare_return` | this RFC | a bare `return` removed as the last statement of a `-> None` body; a body that was only `return` becomes `pass` |
| `fmt_import_order` | this RFC | imports sectioned by root, sorted, merged, and de-duplicated as "Import ordering" defines; a run that binds one name twice is left as written |
...
Formatter invariants apply to the class as a whole: idempotency, parse preservation, and opacity of string literal bodies (RFC 053's string-and-comment safety rule). A file that does not parse is not formatted. The same invariants bind every `fix`-mode entry `incan architect --fix` applies, which is why its output can be handed to the formatter without a second parse.
...
- **Imports and modules**: `wildcard_imports` does not translate because the parser refuses the form; `unused_imports` and `duplicate_import_binding` are Incan-side lint entries, and `fmt_import_order` is a `format`-class normalization that reads the section of an import off its root segment, as "Import ordering" defines, on the guarantee that imports are compile-time bindings with no import-time execution. `pub from module import Item` re-exports are ordinary declarations to every rule, and a separate run to `fmt_import_order`.
...
- A project with no `[incan.lints]` gets the catalog defaults. Nothing needs to be written to keep today's behavior, with one consequence stated in the guide-level explanation: `correctness` is `deny`, so a default project with a `correctness` finding fails `incan architect`.
- A preview entry reports nothing until a level table names it and `--profile experimental` makes it eligible, so a rule added to the catalog never changes a project's output on upgrade; its promotion does, and is a release-notes item.
- Formatter output on already-formatted code does not change until a `format`-class entry lands or changes. Each such entry changes formatter output for code that matches it; each is listed in the release notes and projects see it as a one-time reformat. The nine new `format`-class entries this RFC names beyond RFC 053 and the style guide (`fmt_decorator_spacing`, `fmt_parentheses`, `fmt_digit_grouping`, `fmt_fstring_prefix`, `fmt_nested_fstring`, `fmt_str_in_fstring`, `fmt_redundant_pass`, `fmt_bare_return`, `fmt_import_order`) are that reformat for this RFC.
- A `fix`-mode entry never changes a file on its own: `incan architect --fix` is opt-in per invocation, and a project that runs it sees the rewrites the levels admit.
- `INCAN-T0101` keeps its code, its text, and its position in output; its level becomes configurable. `unused_variables` and `unused_imports` are new checker by-products at `warn`, so a project sees them on upgrade as it would any new rustc lint; `wildcard_enum_match_arm` is new at clippy's `allow` default and reports nothing unless a project enables it.
...
| `preview` rules, `explicit-preview-rules`, one minor release before promotion | adapt as a catalog flag | Groups | preview entries default to `allow`, are outside `all` and their group for level purposes, are eligible only under `--profile experimental`, and are enabled by name; no manifest switch, because the manifest names rules |
| isort sections and sort order (`unsorted-imports`, `I001`) | adapt as `fmt_import_order`, a `format`-class entry | Import ordering | sections are decided by the root segment (`std`, `pub::`, `rust::`, project, parent), so `known-*` settings do not exist; imports are compile-time bindings with no import-time execution, so ordering is representation and belongs to the formatter, not to a lint; the rest of isort's settings are rejected row by row in that section |
...
| pycodestyle `E`/`W` layout codes, `dummy-variable-rgx`, rules that exist because Python has no static types | reject | catalog | the `format` class makes layout codes impossible, the underscore convention is fixed, and `incan check` owns what types know |
...
`incan architect --fix` could apply `comparison_to_empty` or `useless_conversion`, which need types, since the engine has them. Rejected for this RFC: a `fix` entry must be applicable to any file that parses, in every configuration, so that `--fix` never depends on the checked configuration and never rewrites one branch on facts from another; the fixable set is by construction the type-independent set. Type-dependent fixes are the `unsafe` mode a later RFC may add.
...
- **One rule implementation per entry, one engine.** `syntax`-tier rules are functions over the trivia-aware syntax tree; `semantic`-tier rules are functions over RFC 105's typed fact views. `incan architect` runs both tiers and, under `--fix`, applies the rewrites of `fix`-mode findings and hands the result to the formatter. `incan fmt` runs the `format` class and no lint rule; the formatter holds no rule logic beyond that class. The `fix` rewrites live beside their rules in the engine, not in the formatter crate, and the formatter is a library the engine calls on the fixed tree.
- **Relationship to #159.** Rules are defined against the trivia-aware tree that #159 introduces, so comment and docstring trivia are inputs to a rule rather than repair work after it. Two sets follow from the classification in the design decisions. The `format` class gains nine entries beyond RFC 053 and the style guide; the current AST-based formatter can host the seven whose normalization moves no comment (`fmt_parentheses`, `fmt_digit_grouping`, `fmt_fstring_prefix`, `fmt_nested_fstring`, `fmt_str_in_fstring`, `fmt_redundant_pass`, `fmt_bare_return`), and the two that move trivia or reorder declarations (`fmt_decorator_spacing`, `fmt_import_order`) wait for #159. The `fix` set is nine entries; `incan architect --fix` can apply the seven whose rewrite moves no comment (`needless_bool`, `needless_bool_assign`, `needless_else`, `list_init_then_append`, `range_plus_one`, `range_minus_one`, `while_true`) on the current tree, and the two that move a branch body's comments with the body (`if_not_else`, `redundant_else`) wait for #159.
...
- **Manifest / Oven (RFC 117)**: `[incan.lints]` and `[workspace.incan.lints]` parsing as facet tables, the `per-file-allow` subtable and its globs, the precedence lattice, and the refusals, including the refusal that names the spelling for a Ruff key or for a Cargo-shaped `lints` table; the manifest ownership map gains an `[incan.lints]` line owned by this RFC.
- **CLI / tooling**: `incan architect` resolves levels from the manifest through the lattice, distinguishes eligibility from level, carries the catalog fields in its findings, and gains `--fix` (with `--diff`), which applies `fix`-mode findings and formats the result; `incan fmt --check` reports by rule and gains `--format json`; `incan explain` resolves `INCAN-L` codes and bare rule names and renders the documentation template; `incan architect --list-rules` is a new flag on RFC 105's surface and lists the catalog with group or class, fix mode, preview flag, eligibility, and effective level.
- **LSP / formatter**: the formatter hosts the `format` class and nothing else; it reads no manifest, no workspace table, and no `@allow(...)`, and the formatter crate becomes a library the engine calls on the tree `--fix` leaves. The language server surfaces catalog findings through the engine, which is an open question below.
...
- **Not implicit:** no rule runs that is not a catalog entry; no level comes from anywhere but the defaults, the two manifest tables, and `@allow(...)`; no command rewrites source except `incan fmt`, which applies only the `format` class, and `incan architect --fix`, which applies only `fix`-mode entries; nothing a project configures changes what `incan fmt` produces.

## Acceptance criteria

This RFC is done when:

- the catalog registry holds every entry in the tables above with name, code, group and default level or `format`-class membership, origin, fact tier, evaluator, fix mode, preview flag, parameters, and documentation in the template, a conformance test checks every `clippy` and `clippy-renamed` entry against the pinned clippy lint list, and a documentation test runs every entry's example pair;
- `[incan.lints]`, `[workspace.incan.lints]`, and their `per-file-allow` subtables parse with every refusal listed above, and the lattice is covered by tests for the join of policy level and member, for the policy table lowering a catalog default and a member being unable to, for the member-below-policy refusal, for the per-path and `@allow(...)` exemptions, for `forbid` being outside their reach, for the same-priority conflict inside one table and its absence across tables, and for a preview entry being enabled by name and eligible only under `--profile experimental`;
...
- `incan architect --format json` findings carry rule, code, group, effective level, level source, and severity derived from level, and the checker's entries appear once;
- `incan architect --fix` applies every `fix`-mode entry at an effective level other than `allow` and none at `allow`, formats its output, marks applied findings `applied: true`, honors `--diff`, and passes an idempotency property test per `fix` entry and for the set as a whole;
- `incan fmt --check` reports by rule name, fails on the `format` class and on nothing else, reads no lint table, and emits the schema 2 report under `--format json`; `incan fmt` applies the class, and the idempotency property test covers each `format`-class entry;
...
- **The twenty, re-sorted by kind.** The twenty type-independent rewrites an earlier draft gave the formatter are classified as representation or as structure. Representation, and therefore the `format` class with no level: `precedence` and `double_parens` (as `fmt_parentheses`), `unreadable_literal` and `inconsistent_digit_grouping` (as `fmt_digit_grouping`), `empty_line_after_outer_attr` (as `fmt_decorator_spacing`), `redundant_pass` (as `fmt_redundant_pass`), `needless_bare_return` (as `fmt_bare_return`), `useless_format`'s placeholder-free form (as `fmt_fstring_prefix`), `to_string_in_format_args` (as `fmt_str_in_fstring`), `format_in_format_args` (as `fmt_nested_fstring`), and import ordering (as `fmt_import_order`). Structure, and therefore fix mode `fix` under `incan architect --fix`: `needless_bool`, `needless_bool_assign`, `needless_else`, `redundant_else`, `if_not_else`, `list_init_then_append`, `range_plus_one`, `range_minus_one`, and `while_true`. Everything type-dependent stays `suggest`. The rows record the one place each of `range_plus_one` and `range_minus_one` is not an identity (an overflowing bound), and the two conditions `fmt_str_in_fstring` checks on the tree (no format spec, no local `str` binding).
...
- **Precedence is a lattice.** `project_level = max(policy_level, member)` over `allow < warn < deny < forbid`, where `policy_level` is the workspace table's assignment or else the catalog default, priority is resolved inside each manifest layer independently, and `location_level = allow` under a per-path or `@allow(...)` exemption unless `project_level` is `forbid`. The catalog default is the base of the join; the policy table (the workspace's, or a standalone project's own) is the one layer that may set a rule below it. The workspace is the package-policy floor, subject to explicit scoped exemptions; a member composes monotonically; a path or a declaration exempts. This diverges from Cargo's all-or-nothing `[lints] workspace = true`, which cannot merge a member table with the workspace's, and it is what RFC 117's "narrow but not replace" means for lint policy.
...
- **Import ordering is representation.** An Incan module is declarations only, a `static` initializer is a storage cell, and an import is a compile-time binding resolved under the reserved roots (RFC 022, RFC 031), so there is no import-time execution and reordering cannot change program meaning. `fmt_import_order` sorts, groups, merges, and de-duplicates; a run that binds one name twice is left as written and reported by `duplicate_import_binding`. The section order (`std`, `pub::`, `rust::`, project, parent-relative) is this RFC's ruling. No `known-*` configuration exists, because the root segment decides the section.
...
- **Fix safety is a catalog property.** `format` and `fix` are Ruff's safe tier, split by who applies them, and `suggest` is its display-only tier, decided by the entry's constraints and not by a project setting; there is no unsafe tier, no `--unsafe-fixes`, and no `fixable`/`unfixable` or `extend-safe-fixes`/`extend-unsafe-fixes` key. A later RFC that wants type-dependent fixes adds a mode named `unsafe` with its own flag.
...
None of these gates the move to Planned. Each carries this draft's answer and becomes a design decision at that move unless review overturns it.

- One invocation or two: should `incan fmt` also print the `syntax`-tier findings it does not apply, or is `incan architect` the only reporting surface? This draft says `incan fmt` reports only the `format` class.
