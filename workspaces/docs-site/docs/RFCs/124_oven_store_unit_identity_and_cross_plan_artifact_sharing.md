...
3. **Identity completes in stages.** Where RFC 119 permits selected publisher-side link or tool work, its declared inputs and admitted receipt are known before consuming units are identified. Procedural macros remain ordinary host Rust units loaded and invoked through rustc's ABI. Downstream identities are finalised from the applicable admitted facts and receipts, and no unit compiles before its identity is final. A `build.rs` never participates in this process.
