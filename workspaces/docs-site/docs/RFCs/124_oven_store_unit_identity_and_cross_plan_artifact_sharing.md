...
1. **The unit is the object.** A compiled unit is one compiler invocation's durable output for one unit of a Loaf's build graph in one domain: a library, a procedural macro, a binary, or a test harness. Declared `cfg`, `out`, `link`, and `tool` facts are consuming-unit inputs under RFC 119; a `build.rs` is inert source-inventory evidence, never a unit, executable, or output-run unit. In rustc's own vocabulary a unit is a crate; this RFC uses the RFC 119 term throughout. The store addresses units, not plans.
...
3. **Identity completes in stages.** Where RFC 119 permits selected publisher-side link or tool work, its declared inputs and admitted receipt are known before consuming units are identified. Procedural macros remain ordinary host Rust units loaded and invoked through rustc's ABI. Downstream identities are finalized from the applicable admitted facts and receipts, and no unit compiles before its identity is final. A `build.rs` never participates in this process.
...
The current Oven store is correct and conservative in the right way: nothing is reused without a receipt match, and the receipt covers the inputs that matter. What it lacks is granularity, and the cost of that was measured during the 0.6 dependency audit. Two freshly generated projects with identical dependency closures each baked their own debug and release Loafs, four sealed artifacts of 51 to 54 MB holding the same fourteen compiled rlibs. On a maintainer's machine, twenty build directories under one development root totaled 52 GB, two of them the compiler itself at 10.8 GB and 10.4 GB a few commits apart, indistinguishable to any tool because the artifacts inside carry no identity that another directory could recognize.
...
- Stage identity completion along the RFC 119 unit graph so declared publisher-side products are admitted before the units that consume them are identified, with no second graph.
...
When RFC 125 is in place, a unit that arrives from `incan.pub` shows in `oven store status` with `imported` as its origin and the attestation that vouched for it. A later local bake of a unit with the same identity is recognized as already present and is not recompiled.

## Reference-level explanation

### Compiled units

A compiled unit is the durable output of one compiler invocation for one unit of a Loaf in one domain, together with the metadata needed to compile and link against it. Unit kinds are library, procedural macro, binary, test harness, and metadata-only (the check-only output of `oven check`, which carries the compiler's metadata and no code; its identity includes the check-only emission mode so it never substitutes for a full unit, while a full unit may satisfy a later check). Every unit must record its host or target domain as RFC 119 defines them. Declared `cfg`, `out`, `link`, and `tool` facts reach the identity of the unit that consumes them; a `build.rs` is only inert inventory evidence and cannot contribute executable output, directives, or a separate shared object.
...
- the profile facts that affect codegen: optimization level, debug-info level, codegen units, panic strategy, LTO mode, target features, and any other flag Oven passes that changes output;
...
- for units that consume declared generated, linked-library, or tool products, the receipt identity of each applicable admitted publisher-side producer and the declared `cfg`, `out`, `link`, and `tool` facts that reach compilation; and
- for units that use a procedural macro, the identity of the host Rust macro unit that rustc loads through its ordinary ABI;
- for units with linked-library inputs, the digests of the linked libraries and headers the unit links or includes.
...
Every other identity input — toolchain, target, domain, profile facts, features, artifact kind, applicable declared producer receipts, procedural-macro identities, and linked-library inputs — enters the external identity unchanged, because all of them are observable by a dependent.
...
Identity completion is staged along the RFC 119 unit graph; Oven must not introduce a second graph to do it. Declared `cfg`, committed `out`, `link`, and `tool` facts are selected before the consuming Rust unit is identified. Where RFC 119 permits a selected publisher-side link or tool unit to produce a declared input, that unit is admitted and its receipt is recorded before the consuming identity is finalized. A matching admitted receipt may be reused; otherwise only that selected publisher-side unit may execute under RFC 119 admission. Procedural macros are ordinary host Rust units: rustc loads and invokes them through its normal ABI while compiling the consumer, and their unit identities enter that consumer's identity. A `build.rs` is inert source inventory and is never selected, compiled, executed, interpreted, or treated as a provider. No unit may compile before its identity is final; the no-execution rule for resolution in RFC 117 and RFC 125 is untouched. For each final identity the store is consulted; a present, verified unit is reused, and an absent one is baked and inserted. Where a declared publisher-side result is uncacheable because an input could not be observed, every consuming unit whose identity includes that result is likewise uncacheable: it may be baked and used by the plan that produced it, but must not be inserted for reuse or exported to a registry. The plan receipt must record, per unit, the identity and the satisfaction kind: `baked`, `reused`, or `imported`, with the importing attestation reference where applicable.

If two units in one link closure share a source-qualified package identity and linkage role (the same package from the same source kind, both linked as Rust libraries into one artifact) but have different unit identities, the plan must fail before execution with a diagnostic naming both units, the requirement paths that introduced them, and the first differing identity input. No ownership or provenance declaration waives this rule: declaring who owns a closure does not make two instances' types identical or their runtime state shared. The two permitted resolutions are a **coherent closure**, in which the provider's dependencies are recompiled against the consumer's chosen dependency identities so that one unit results, or an **explicit isolation boundary** under RFC 119, in which the second instance lives behind a process or external ABI boundary across which no Rust type or runtime object passes; the isolation must be declared and receipt-visible. Two units of the same package in different linkage roles, for example a host proc-macro build and a target library build, are not a collision.

Whole-plan reuse remains permitted: a plan whose receipt identity matches a sealed plan may reuse the sealed outputs without consulting units individually. This is an optimization over the unit rules, not an exception to them.

### Store obligations

The store must hold each unit payload once, addressed by unit identity and payload digest. Payloads are immutable once inserted; the index and referrer tables change only by append and tombstone. The store must verify a payload's digest at admission (insert, import, or first use after the store detects external change to the file) and whenever a unit crosses a trust boundary such as import from a registry. Within one operation, a unit that has been verified and is held by a lease must be treated as immutable and must not be rehashed again; Oven must not recompute the digest of a transitive closure on every logical lookup. Cheap consistency checks (size, modification time, inode) may guard the warm path, with a full rehash on `oven store verify` or on policy. A payload that fails verification must be evicted and the unit treated as absent. The store must keep a name index from package name, version, domain, target, profile facts, and feature set to identities, so a planner can find candidates without scanning payloads. The store must track referrers: every receipt and lease that reaches a unit. Leases keep in-progress plans and explicitly pinned entries live. The store should materialize units into build directories by hard link or reflink where the filesystem allows and must fall back to copying where it does not.

Collection must remove only units no receipt, lease, or policy pin reaches, must be explicit or governed by a stated policy, and must report what it removed. The existing bounded-capacity behavior continues to apply: the store must refuse an insert that capacity policy cannot satisfy rather than silently evicting live units.

### Registry parity

RFC 125 assets must be addressed by unit identity as defined here. Importing an asset must verify its digest and attestation and then insert its units into the store with satisfaction kind `imported`. Exporting for publication must serialize units with their identities and receipt facts unchanged. A unit whose identity is marked machine-local must not be exported.
...
- **Refusal surfaces earlier.** Plans that today link two compiled instances of one package and happen to work will be refused at plan time until they either unify the closure or declare an isolation boundary. This is the intended behavior and will feel like a regression to anyone relying on the accident.
...
- **Oven executor**: insert baked units by identity; materialize reused units by link or copy.
...
Each of these began as an open question. Every answer is settled against code that already exists rather than against a preference, and each names what it was settled by, so a reader can check it rather than take it.

**Codegen facts enter the identity as the derived flag vector, never as the profile name.** Oven has no user-supplied codegen-flag surface: `OvenBuildIntent` carries `profile` as a *name*, and one function turns that name into the exact arguments (`release` selects `-C opt-level=3`, anything else `-C opt-level=0`, with the debug-info and strip flags decided alongside). So the question's worry — that two spellings of the same intent defeat sharing — cannot arise from a user today. It can arise from the profile name, which is why the name is provenance and not an identity input: two machines can spell one set of flags differently, and one machine can change what a profile means without renaming it. The identity therefore folds the flag vector Oven actually passes, canonicalized as sorted `key`/`value` pairs rather than as a raw argument string, so `-Copt-level=3` and `-C opt-level=3` are one fact. The rule this sets for later: when Oven gains a way to pass a flag the profile does not imply, that flag joins the identity as another normalized pair, and no flag reaches `rustc` without reaching the identity.

**Linked-library inputs of system-binding units split three ways, and the existing link vocabulary already makes the cut.** A linked-library input vendored inside the package is source, and is digested like any other source the unit compiles. A linked-library input resolved through a target-declared interop plan is digested by its resolved files, because that plan already names them and its receipt already records them — `incan inspect interop-plan` reports exactly that set, and a binding-use receipt retains the target-declared binding-to-artifact-name relation only after compilation produced that binding. A `system_library` or `framework` capability is neither: `LinkCapabilityId` defines it as *a named capability a later target resolver must select explicitly*, which is a declaration that the bytes come from the host. Such a unit folds the declared capability name and the target's declared resolution facts, and is marked **machine-local**. Machine-local means shareable across every plan, project, and worktree on this machine, and never exported as a registry asset — which is the same boundary RFC 125 needs, drawn by the fact that makes it necessary rather than by a separate policy.

**A build script is inert inventory; declared facts are consuming-unit inputs.** RFC 119 supersedes the earlier provider model. A `build.rs` may be retained to explain an adoption warning, but it is never selected, compiled, executed, interpreted, stored, or addressed as a unit. Its former observable effects are represented only by declared `cfg`, committed `out`, `link`, and `tool` facts. Those facts, plus any receipt for selected publisher-side link or tool work that produces them, enter the consuming unit's identity. This preserves the identity boundary without treating an executable script or its output as a reusable provider. A procedural macro is different: it remains an ordinary host Rust compilation unit which the selected rustc loads and invokes through the normal ABI.

**Collection is capacity-triggered at admission plus explicit, with reachability as the eligibility filter.** `OvenStoreLimits` already carries three caps — total physical bytes, per-domain physical bytes, per-domain logical bytes — admission already prunes to them, and `prune` and `preview_prune` are already the user-facing operations, the second reporting what the first would reclaim. RFC 124 adopts that rather than introducing a fourth mode. What it adds is the eligibility rule: a unit reached by a receipt, a lease, or a policy pin is never collectable, whatever the pressure, and age orders the remainder only as a tie-break among units that are already eligible. Time-based background collection is rejected: the store is shared across every worktree on a machine, and a collector running with no user present would reclaim a unit another checkout is about to reuse, with no diagnostic anyone would see.

**Two payloads at one identity: retain the first, refuse the second, mark the identity machine-local, and classify nothing.** The 0.6 dependency audit is the evidence against classification. Two independently resolved closures of identical source produced ABI-incompatible rlibs for one `tokio` version, and linking both into one executable panicked at runtime because an object created through one instance was invisible to code compiled against the other. A classifier would have had to decide that difference was benign or malign from the bytes, and the difference that mattered was not one a heuristic distinguishes from debug information or symbol ordering. So Oven records both payload digests in the receipt, so the divergence is inspectable rather than silent, refuses the second payload, and marks the identity machine-local — which makes it ineligible for registry export, because an identity this machine cannot reproduce is not one another machine should trust.

**A manifest fact is observed by compilation exactly when it reaches the invocation Oven constructs.** Enumerating facts by reading the manifest is the wrong direction and produces a list that drifts. Oven builds the `rustc` command itself, so the observed set is closed and knowable from the other end: it is the facts that appear in the argument vector and in the environment Oven exports — the crate name, the edition, each `--cfg feature=`, each `-C` flag, each `--extern` and the identity behind it, and the package values Oven exports for the environment macros a crate can read. Anything the manifest holds that does not reach that vector is provenance. Stating it this way makes the list mechanically checkable rather than a judgment: a test asserts that every identity input appears in the recorded invocation and that no input of the recorded invocation is missing from the identity, and a new flag or environment value fails that test until it is classified. The same discipline is what #1530 was a failure of one layer down — a Cargo lock retains every edge any resolve could take, the sealed authority records the one closure a bake resolved, and reading the first as if it were the second made every `rust::` import in a project resolve to nothing.
