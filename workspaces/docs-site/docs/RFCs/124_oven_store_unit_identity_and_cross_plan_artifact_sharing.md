...
2. **Identity is the complete compilation context.** A unit identity is a content digest over the unit's source digest, toolchain identity, target, profile and codegen options, resolved feature set, host or target domain, crate type, and the unit identities of every dependency it links. Because dependency identities are themselves inputs, identity is a Merkle root over the closure: two units with equal identity were produced from byte-equal inputs all the way down.
3. **Equal identity means interchangeable; nothing less does.** Oven may substitute one unit for another only when their identities are equal. Same package and version with different identity is a miss, and if both would enter one link, it is a refusal.
4. **A plan is a set of unit references plus a receipt.** The plan receipt records which unit identities satisfy which crates and how each was obtained: baked here, reused from the store, or imported from a registry asset. Reusing a plan output remains possible and is now a special case of every unit hitting.
5. **The store is content-addressed with a name index.** Unit payloads live once under their identity. A separate index maps package name, version, and plan facts to identities so a planner can ask "what do I already have for this crate under these facts" without enumerating the store.
6. **Sharing crosses every local boundary.** Plans, projects, worktrees, and profiles share units through identity. A unit compiled for a release plan in one checkout satisfies a release plan in another checkout of the same or a different project.
7. **Collection is by reachability.** A unit is live while a receipt, lease, or policy pin reaches it. Everything else is collectable, and collection is a store operation a user can invoke and inspect.
8. **Local and registry are the same identity space.** RFC 125 assets are unit-identity-addressed. Importing an asset is a store insert; publishing one is a store export. The registry never needs a second notion of what makes two compiled artifacts the same.
...
- Define store obligations: content addressing, the name index, reference tracking, leases, verification on read, and collection by reachability.
...
$ oven bake
  ...
  hint: unify the feature closure in loaf.toml, or declare the provider's closure as caller-owned (RFC 119)
```
...
- the source identity of the crate (registry checksum, or the source Loaf digest under RFC 125, or a content digest of a path source);
...
A unit identity must not include absolute paths, timestamps, hostnames, user names, or the identity of the plan that requested it. Oven must remap source paths so that the same source at two locations produces the same identity. Where the compiler's own output is not deterministic for equal inputs, Oven must record that the identity is machine-local and must not export the unit to a registry; RFC 125 inherits this rule.

Two units with equal identity must be treated as interchangeable. Oven must never substitute units whose identities differ, regardless of package name, version, or apparent compatibility.

### Plans and reuse

A plan must resolve every unit to an identity before executing anything. For each identity the store is consulted; a present, verified unit is reused, and an absent one is baked and inserted. The plan receipt must record, per unit, the identity and the satisfaction kind: `baked`, `reused`, or `imported`, with the importing attestation reference where applicable.

If two units in one link closure share a package name and version but have different identities, the plan must fail before execution with a diagnostic naming both units, the requirement paths that introduced them, and the differing identity inputs. A project may declare a caller-owned closure under RFC 119 to permit a deliberate second instance; the receipt must record that declaration.
...
The store must hold each unit payload once, addressed by identity, and must verify a payload's digest before serving it; a payload that fails verification must be evicted and the unit treated as absent. The store must keep a name index from package name, version, domain, target, profile facts, and feature set to identities, so a planner can find candidates without scanning payloads. The store must track referrers: every receipt and lease that reaches a unit. Leases keep in-progress plans and explicitly pinned entries live. The store should materialise units into build directories by hard link or reflink where the filesystem allows and must fall back to copying where it does not.
...
- **Refusal surfaces earlier.** Plans that today produce a working binary through a caller-owned closure will need to declare it, or will be refused at plan time. This is the intended behaviour and will feel like a regression to anyone relying on the accident.
...
- How should identity treat compiler nondeterminism that is detected after the fact, when two bakes of equal inputs produce different bytes on one machine?
