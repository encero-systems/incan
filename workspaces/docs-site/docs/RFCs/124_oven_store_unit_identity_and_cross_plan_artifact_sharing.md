...
3. **Identity completes in stages.** Where RFC 119 permits selected publisher-side native or tool work, its declared inputs and admitted receipt are known before consuming units are identified. Procedural macros remain ordinary host Rust units loaded and invoked through rustc's ABI. Downstream identities are finalised from the applicable admitted facts and receipts, and no unit compiles before its identity is final. A `build.rs` never participates in this process.
...
- for units that consume declared generated, native, or tool products, the receipt identity of each applicable admitted publisher-side producer and the declared `cfg`, `out`, `native`, and `tool` facts that reach compilation; and
...
Every other identity input — toolchain, target, domain, profile facts, features, artifact kind, applicable declared producer receipts, procedural-macro identities, and native inputs — enters the external identity unchanged, because all of them are observable by a dependent.
...
**Native inputs of system-binding units split three ways, and the existing link vocabulary already makes the cut.** A native input vendored inside the package is source, and is digested like any other source the unit compiles. A native input resolved through a target-declared interop plan is digested by its resolved files, because that plan already names them and its receipt already records them — `incan inspect interop-plan` reports exactly that set, and a binding-use receipt retains the target-declared binding-to-artifact-name relation only after compilation produced that binding. A `system_library` or `framework` capability is neither: `LinkCapabilityId` defines it as *a named capability a later target resolver must select explicitly*, which is a declaration that the bytes come from the host. Such a unit folds the declared capability name and the target's declared resolution facts, and is marked **machine-local**. Machine-local means shareable across every plan, project, and worktree on this machine, and never exported as a registry asset — which is the same boundary RFC 125 needs, drawn by the fact that makes it necessary rather than by a separate policy.

**A build script is inert inventory; declared facts are consuming-unit inputs.** RFC 119 supersedes the earlier provider model. A `build.rs` may be retained to explain an adoption warning, but it is never selected, compiled, executed, interpreted, stored, or addressed as a unit. Its former observable effects are represented only by declared `cfg`, committed `out`, `native`, and `tool` facts. Those facts, plus any receipt for selected publisher-side native or tool work that produces them, enter the consuming unit's identity. This preserves the identity boundary without treating an executable script or its output as a reusable provider. A procedural macro is different: it remains an ordinary host Rust compilation unit which the selected rustc loads and invokes through the normal ABI.
