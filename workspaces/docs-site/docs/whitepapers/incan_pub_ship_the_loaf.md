...
Oven's receipt model is examined against those properties as one candidate, and `incan.pub` as an exchange built on the identities it produces — without touching crates.io, and without asking any Rust project to adopt anything.
...
What none of the partial answers has is an artifact identity precise enough to trust from a stranger, produced by the build system itself rather than inferred afterwards.
