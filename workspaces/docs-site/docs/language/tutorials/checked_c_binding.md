...
You can use checked bindings today for exact scalar free functions, native integer constants, plain-structure layout declarations, opaque owned resources, inferred call-scoped resource borrows, scalar or owned-resource `Out`/`InOut` positions, NUL-terminated text input through `c.cstr(value)?`, and immediately copied UTF-8 text returned as `c.ConstPtr[c.c_char]`. The scalar bridge preserves `i8` through `i128`, `u8` through `u128`, `usize`, `f32`, and `f64`; `c.i128` and `c.u128` require the selected Clang target's `__int128` extension. Bounded byte and `f32` spans are also available when a declaration pairs their checked pointer and count/capacity arguments. Packages may lock declared artifacts, bake governed C/C++ shims, and stage selected Android/iOS runtime files through Oven Alpha. Pointer arithmetic, arbitrary dereference, escaping views, and direct callback or variadic binding declarations remain unavailable; callbacks and variadics belong behind a governed shim with a bounded C contract. Physical-device deployment, signing, and publication admission remain outside this experimental slice.
