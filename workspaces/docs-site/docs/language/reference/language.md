...
- `mut` in a function type marks a parameter whose changes the caller sees, as on a `def` parameter; see [`mut` parameters in function types](stdlib_traits/callable.md#mut-parameters-in-function-types).
