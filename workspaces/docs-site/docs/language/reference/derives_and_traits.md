...
- A `mut` parameter of a trait method, default or implementation, has the contract of a `mut` function parameter. It is a mutable binding in the body. When its type is not `int`, `float`, `bool` or a Rust type, and it is not a `*args` or `**kwargs` parameter, the method's changes to it are visible to the caller after the call, and the argument must be a binding or parameter declared `mut`, a static, `self` in a `mut self` method, or a field or element of one of those; an immutable binding, a literal or another temporary is refused with `INCAN-T0117`. With `def replace(self, mut items: list[int]) -> int` appending one element, `widget.replace(items)` on `mut items: list[int] = [1, 2]` returns `3` and leaves `items` with 3 elements; on `items: list[int] = [1, 2]` it is refused with `INCAN-T0117`. A `mut n: int` parameter accepts any argument, a literal included, and its changes stay in the method.
