...
- A `mut` parameter of a trait method, default or implementation, has the contract of a `mut` function parameter. It is a mutable binding in the body. When its type is not `int`, `float`, `bool` (or an alias of one) or a Rust type, and it is not a `*args` or `**kwargs` parameter, the method's changes to it are visible to the caller after the call, and the body cannot rebind it to a new value (`items = []` is refused with `INCAN-T0001`). When the method changes such a parameter, by writing to its elements or fields, calling a method that changes it, or passing it on to a parameter that is changed, the argument must be a binding or parameter declared `mut`, `self` in a `mut self` method, a field of one of those, or a temporary such as a literal or a call result; an immutable binding or a field of one, a list or dict element and a static are refused with `INCAN-T0117`. When the method never changes the parameter, every argument is accepted and the method receives the argument's value. With `def replace(self, mut items: list[int]) -> int` appending one element, `widget.replace(items)` on `mut items: list[int] = [1, 2]` returns `3` and leaves `items` with 3 elements; on `items: list[int] = [1, 2]` it is refused with `INCAN-T0117`; `widget.replace([5])` returns `2`. A `mut n: int` parameter accepts any argument, and its changes stay in the method.
