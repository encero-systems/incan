...
- Reading an element needs no bound on a type parameter: `items[i]` and the slice `items[a:b]` on a `list[T]`, and `table[key]` on a `dict[K, V]`, produce values of their own in a generic function, a method and a trait method, required or default, as in `def first[K](items: list[K]) -> K: return items[0]` and `def rest[K](items: list[K]) -> list[K]: return items[1:]`. A generic caller in the same project that passes its own type parameter to such a callable needs no bound either. A callable of a compiled library (`.incnlib`) that reads elements this way requires `with Clone` on the type parameter a caller in another package passes to it: `def head[T with Clone](items: list[T]) -> T: return first(items)`.
...
- A default method body names what its trait's module names. A type, an enum variant or a function the trait's module declares or imports is that declaration in every adopter, whether or not the adopting module imports it: a default `def extent(self) -> Extent: return Extent(width=self.width(), height=self.height())` in module `shapes` returns `shapes.Extent` to an adopter that imports only the trait; `return Corner.Top` and a `match` arm `Corner.Top => ...` name `shapes.Corner.Top`; `Size(n=1)` constructs the `Size` that `shapes` imports with `from units import Size`; and a call `doubled(n)` calls `shapes.doubled`, a private function of `shapes` included.
- A trait imported through a module that re-exports it (`pub from shapes import Measured` in `geometry`, then `from geometry import Measured`) is the declaring module's trait: `with Measured` adopts `shapes.Measured`, its defaults included, whether or not the trait has defaults.
- A `mut` parameter of a trait method, default or implementation, has the contract of a `mut` function parameter. It is a mutable binding in the body. When its type is not `int`, `float`, `bool` (or an alias of one) or a Rust type, and it is not a `*args` or `**kwargs` parameter, the method's changes to it are visible to the caller after the call, and the body cannot rebind it to a new value (`items = []` is refused with `INCAN-T0001`). The body uses such a parameter only directly. These are refused with `INCAN-T0001`: binding it to another name (`other = items`, `let other = items`, `mut other: list[int] = items`, or reassigning an existing name to it); a direct construction or store that holds it, that is, an element of a tuple, list, set or dict literal or of a comprehension, a field or element store (`holder.items = items`, `table["k"] = items`), an argument of a model, class or newtype construction (`Holder(items=items)`), of an enum variant (`Wrap.Held(items)`) or of `Some`, `Ok` or `Err`, and a `partial` preset (`partial extend(items=items)`); producing it as the value of a `match` arm, an `if` branch, a `break` or a `yield`; binding it to a name in a `match` arm (`match items: xs => ...`); and a closure that returns it, changes it or passes it on to a parameter that may change it. Passing it as an argument to any other call, `rows.append(items)` included, hands the callee its value and is not refused. An independent copy of a list is written `list(items)` and of a string `str(items)`; a value of any other type is copied by building a new value from it. Whether a call changes such a parameter is decided on the body that runs. A call on a value of a model, class, enum or newtype runs that type's method, its own or the trait's default when it defines none, and that body changes the parameter when it writes to its elements or fields, calls a method that changes it, or passes it on to a parameter that is changed. The variable of a `for` loop over a list parameter itself, over a field of it, or over the variable of an enclosing such loop is a view into the parameter's elements, so a change through it (`for row in items: row.append(3)`) changes the parameter; a loop over a copy (`list(items)`, `enumerate(items)`, `items.clone()`, `items[0]`) changes only the copy. A `for` loop variable is not a `mut` binding, so passing it for a parameter that the call changes is refused with `INCAN-T0117`. A call through a type parameter's bound, on `self` in a default or on a trait-typed value may run any adopter's method, so it counts as changing the parameter. For a call that changes the parameter, the argument must be a binding or parameter declared `mut`, `self` in a `mut self` method, a field of one of those, or a temporary such as a literal or a call result; an immutable binding or a field of one, a list or dict element and a static are refused with `INCAN-T0117`. For a call that does not change it, every argument is accepted and the method receives the argument's value. With `def replace(self, mut items: list[int]) -> int` appending one element, `widget.replace(items)` on `mut items: list[int] = [1, 2]` returns `3` and leaves `items` with 3 elements; on `items: list[int] = [1, 2]` it is refused with `INCAN-T0117`; `widget.replace([5])` returns `2`. With a default `def grow(self, mut items: list[int]) -> int: return len(items)`, `grower.grow(items)` in `def run[T with Grower](grower: T, items: list[int]) -> int` is refused with `INCAN-T0117`, whether or not an adopter overrides `grow`. A `mut n: int` parameter accepts any argument, and its changes stay in the method.
- A type that adopts a trait through a module derive, `@derive(codec)` over a module whose `__derives__` lists the trait, or through a derive of the trait imported by name, `from codec import Encode` with `@derive(Encode)`, adopts it as a `with` clause does: the trait's methods, defaults included, are called directly on the type (`item.tag()`) and satisfy generic bounds.
