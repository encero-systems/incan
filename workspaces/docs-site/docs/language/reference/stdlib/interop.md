...
Headers and native names are explicit in source. The verifier neither scans arbitrary headers to infer an API nor searches for a library that happens to provide a symbol. The logical library name records the link capability. A package may separately declare package-relative interop inputs and compatibility requirements under `[interop.c]` in `incan.toml`; `incan lock` records those requirements and the content-derived identities of package-owned files. In v0.5, the explicit `incan oven interop bake` command turns those locked facts plus supplied compiler/SDK evidence into a selected direct-`rustc` native plan, and `stage` creates a fixed Android/iOS runtime layout. These commands do not discover host libraries, invoke Cargo or platform build tools, sign applications, or prove physical-device deployment. See the [checked C binding how-to](../../how-to/checked_c_bindings.md#freeze-oven-interop-requirements-for-a-target) for the current declaration schema and limits.
