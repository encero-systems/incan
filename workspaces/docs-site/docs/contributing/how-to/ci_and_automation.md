...
Expect two publication phases. The command first publishes the ordinary release Loaf family under the staged package, then bakes the release policy project against that package-local family. It selects the exact `core_engine` output for `TARGET` from the structured bake report and republishes the envelope with that member. A failure in either phase stops archive creation.
