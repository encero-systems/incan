...
Import, reexport, package, stdlib, vocab, formatter, and test-batch boundaries may change visibility, initialization timing, and emitted Rust path selection, but they must not change the semantic identity of a source item. A function, method, type, static, alias, partial preset, decorator-backed callable, or exported package member should keep the same resolved callable surface, default arguments, metadata, and type identity whether it is used locally, imported directly, reexported through a facade, consumed through `pub::package`, desugared from a vocab helper, or lowered into a generated Rust test harness.

When adding or fixing a boundary-sensitive feature, resolve the source item once in the frontend/typechecker or checked manifest metadata, then pass that resolved identity forward. Checked exports carry source identity and projection metadata, and `.incnlib` manifests serialize the public identity graph so package consumers reconstruct the same provider-owned facts instead of guessing from the public spelling. An alias, facade reexport, partial preset, decorated callable, generated vocab helper, or test harness binding is a projection over the original semantic identity; it is not a copied callable summary that may drift.
