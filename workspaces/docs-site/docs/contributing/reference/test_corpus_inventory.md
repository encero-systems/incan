...
- Retire-class tests: 1145, of which twinned 74, dies 115, open 956 (neither yet).
...
### `loaves/compiler/incan_emit` (934 tests in 57 files: keep 100, retire 832, unaffected 2)
...
| `loaves/compiler/incan_emit/src/conversions.rs` | 79 | 2834 | 1578 | retire | 0/79 | 0 | required | #1561 | legacy_ir 74 | Rust conversion policy (to_string/borrow/clone plans) for emission; the duckborrower facts in Body IR are the twin surface. |
...
| `loaves/compiler/incan_emit/src/tests/static_reads_and_frozen_strings.rs` | 7 | 493 | 493 | retire (retire 7) | 7/7 | 0 | - | #1561 | codegen 6, parser 6, legacy_ir 1 | generated Rust for static collection reads and const string destinations, lowering through emission end to end. |
| `loaves/compiler/incan_emit/src/trait_bound_inference.rs` | 15 | 4439 | 703 | retire | 0/15 | 0 | - | #1561 | codegen 5, legacy_ir 15 | infers Rust trait bounds for generated generics; Rust-shape concern. |
...
Per-test overrides in `loaves/compiler/incan_emit/src/tests/static_reads_and_frozen_strings.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `static_collection_arguments_stay_readable_issue1793` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | codegen, parser | Asserts the generated Rust of a static dict lookup, insert and membership test (the reused key is cloned into the storage temporary, a last-read key moved, a literal key stored as an owned string; `get` returns `Option<i64>`); retires with `incan_emit`. The behavior is carried by the named fixture (#1793). |
| `local_dict_get_answers_with_the_stored_value` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | codegen, parser | Asserts the generated Rust of `get` on a dict that is not static storage (`.copied()` for a `Copy` value, `.cloned()` otherwise); retires with `incan_emit`. The behavior is carried by the named fixture (#1793). |
| `dict_get_copies_only_a_kept_value` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | codegen, parser | Asserts the generated Rust of in-place lookups (a generic value, a list of models in a loop, a Rust value that cannot be copied) and of kept lookups with their inferred `Clone`/`Eq + Hash` bounds; retires with `incan_emit`. The behavior is carried by the named fixture (#1793). |
| `dict_get_is_copied_when_the_dict_changes_while_it_is_read` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | codegen, parser | Asserts the generated Rust of lookups whose dict changes while the binding is read (`.cloned()` before the change), the in-place read otherwise, and union type patterns inside `Some`; retires with `incan_emit`. The behavior is carried by the named fixture (#1793). |
| `str_const_is_wrapped_at_frozen_str_destinations_issue1794` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/str_const_returned_as_frozen_str.incn` | - | codegen, parser | Asserts the generated Rust of a `str` const at `FrozenStr` destinations (wrapped in `FrozenStr::new` as a return, a union member, a `Some` payload, an argument and a binding); retires with `incan_emit`. The behavior is carried by the named fixture (#1794). |
| `frozen_str_const_is_converted_at_str_destinations_issue1794` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/str_const_returned_as_frozen_str.incn` | - | codegen, parser | Asserts the generated Rust of a `FrozenStr` const at `str` destinations (converted to an owned string as a return, a union member, a `Some` payload, an argument and a binding); retires with `incan_emit`. The behavior is carried by the named fixture (#1794). |
| `planner_wraps_a_static_str_at_frozen_str_destinations_issue1794` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/str_const_returned_as_frozen_str.incn` | - | legacy_ir | Asserts the ownership planner's conversion of a `'static` string at a `FrozenStr` destination (wrapped in `FrozenStr::new` at every Incan destination, at no Rust-facing boundary); retires with `incan_emit`. The behavior is carried by the named fixture (#1794). |
...
### `loaves/compiler/incan_frontend` (1753 tests in 85 files: keep 1753)
...
| `loaves/compiler/incan_frontend/src/typechecker/collect/stdlib_imports.rs` | 1 | 4829 | 32 | keep | - | - | - | #1561 | checker 1 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/dict_get_and_frozen_payloads.rs` | 8 | 475 | 475 | keep | - | - | - | #1561 | checker 8, parser 1 | checker facts for a dict's `get` result (the stored value, `copied`/`cloned` refused) and a `Some` payload's string instantiation (#1793, #1794); typechecker only, no emit/driver dependency. |
...
| `loaves/compiler/incan_ir/src/lower/decl/helpers.rs` | 4 | 1043 | 209 | retire (retire 4) | 1/4 | 0 | - | #1561 | legacy_ir 4 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/expr/calls.rs` | 21 | 5686 | 1240 | retire (keep 1, retire 20) | 0/20 | 0 | - | #1561 | checker 10, parser 4, legacy_ir 19 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3168 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/patterns.rs` | 4 | 1203 | 276 | retire (retire 4) | 4/4 | 0 | - | #1561 | checker 4, parser 4, legacy_ir 4 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5460 | 1358 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/tests/static_method_args.rs` | 8 | 750 | 750 | retire (retire 8) | 6/8 | 0 | - | #1561 | legacy_ir 8 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/types.rs` | 53 | 1331 | 484 | retire (retire 53) | 1/53 | 0 | - | #1561 | legacy_ir 53 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/static_method_args.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `arguments_read_after_a_static_method_call_are_kept_readable_issue1793` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | legacy_ir | Asserts the recorded argument shape on the legacy IR (an argument of a builtin method on static storage that is read after the call is handed over as a view of itself when the method reads it in place and as a copy when the method stores it, a value at its last read as it is, a string literal typed as a `'static` string); retires with `incan_ir`. The behavior, a `str` parameter still readable after `get`, `insert`, `in` and `append` on module statics, is carried by the named fixture (#1793). |
| `arguments_read_again_by_the_generated_code_are_kept_readable_issue1793` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | legacy_ir | Asserts the legacy IR read order for a static index assignment (value before key) and a receiver path that reads the argument's variable; retires with `incan_ir`. The behavior, `counts[name] = counts.get(name).unwrap_or(0) + 1` and `graph[node].contains(node)` running, is carried by the named fixture (#1793). |
| `dict_get_answers_with_the_stored_value` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | legacy_ir | Asserts the legacy IR completion of an in-place dict lookup (`copied` or `cloned`) and its absence on static storage; retires with `incan_ir`. The behavior, `get` returning the stored value on a static dict, a local bound to one and a dict parameter, is carried by the named fixture (#1793). |
| `a_dict_get_whose_result_is_only_read_stays_in_place` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | legacy_ir | Asserts the legacy IR shape of a `dict.get` whose result is only read (an in-place lookup, no copy) and of a kept result (completed with `cloned`); retires with `incan_ir`. The behavior, a generic dict read in a `match` and returned, is carried by the named fixture (#1793). |
| `a_dict_changed_while_the_binding_is_read_is_copied` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | legacy_ir | Asserts the legacy IR completion of a lookup whose dict changes or is passed on before its binding's last read (`cloned`), and the in-place read otherwise; retires with `incan_ir`. The behavior, a value read from a dict that is changed in the arm, is carried by the named fixture (#1793). |
| `a_read_only_lookup_matched_with_union_type_patterns_names_the_variants` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_dict_get_with_a_reused_str_parameter.incn` | - | legacy_ir | Asserts the legacy IR patterns of union type patterns over an in-place lookup (`Some(<union variant>)`); retires with `incan_ir`. The behavior, `int(n)`/`str(t)` arms over a `dict[str, int \| str]` lookup, is carried by the named fixture (#1793). |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `frozen_str_union_member_admits_a_static_str_const_issue1794` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/str_const_returned_as_frozen_str.incn` | - | legacy_ir | Asserts the legacy IR union-member relation (a `FrozenStr` member admits the `'static` string a `const` declared `str` is stored as); retires with `incan_ir`. The behavior, such a const returned and passed where `FrozenStr \| int` is expected and read by `isinstance` as a `str`, is carried by the named fixture (#1794). |
...
| `loaves/kernel/incan_syntax/src/diagnostics/stable.rs` | 8 | 903 | 232 | keep | - | - | - | #1561 | checker 8 | lexer, parser and diagnostics catalog; below the emitter, cannot reach codegen. Reviewed at crate level. |
