...
- Retire-class tests: 1142, of which twinned 72, dies 115, open 955 (neither yet).
- Retire-class files with open rows: 63 (a file whose retire tests are all twinned or recorded `dies` is done).
...
### `loaves/compiler/incan_emit` (935 tests in 58 files: keep 100, retire 833, unaffected 2)
...
| `loaves/compiler/incan_emit/src/codegen.rs` | 129 | 8073 | 5500 | retire (retire 129) | 6/129 | 0 | required | #1561 | codegen 122, text 52, checker 112, parser 82, legacy_ir 115 | IrCodegen entry point and emitter-side metadata; every test drives IrCodegen or emitter-owned merges (manifest type refs, native-union capture). Split with the retirement, not before. |
...
| `loaves/compiler/incan_emit/src/codegen/dependency_metadata.rs` | 6 | 1322 | 167 | retire | 0/6 | 0 | - | #1561 | checker 1, parser 5, legacy_ir 1 | which stdlib and provider items the generated project links; generated-project shape. |
...
| `loaves/compiler/incan_emit/src/emit/program.rs` | 10 | 4665 | 348 | retire | 0/10 | 0 | - | #1561 | codegen 10, text 2, checker 6, legacy_ir 10 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/src/trait_bound_inference.rs` | 15 | 4404 | 703 | retire | 0/15 | 0 | - | #1561 | codegen 5, legacy_ir 15 | infers Rust trait bounds for generated generics; Rust-shape concern. |
...
| `loaves/compiler/incan_emit/tests/default_argument_codegen_tests.rs` | 7 | 399 | 399 | retire (retire 7) | 6/7 | 0 | - | #1561 | codegen 7, checker 1, parser 7 | generated-Rust spelling of default arguments expanded outside their callable's module or package; asserts emitted paths, which retire with the Rust-source backend. |
...
| `loaves/compiler/incan_emit/tests/method_reachability_codegen_tests.rs` | 1 | 77 | 77 | retire | 1/1 | 0 | - | #1561 | codegen 1, text 1, parser 1 | generated-Rust retention of a method whose caller is found used after its owner's impl was scanned (#1765); asserts the emitted Rust item, which retires with the Rust-source backend. The behavior is carried by the named fixture. |
...
Per-test overrides in `loaves/compiler/incan_emit/tests/default_argument_codegen_tests.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `same_named_default_consts_resolve_in_their_callables_modules_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/same_named_default_consts_across_modules` | - | codegen, parser | Asserts the emitted `crate::a::SIZE` / `crate::b::SIZE` paths and that both pruned modules keep their `SIZE`; the behavior, each omitted argument taking its own module's `SIZE`, is carried by the named fixture (#1771). |
| `imported_default_const_resolves_to_its_declaring_module_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/same_named_default_consts_across_modules` | - | codegen, parser | Asserts the emitted `crate::c::SIZE` path for a default that reads a `SIZE` imported from `c`, and that the pruned `c` keeps it; the behavior is carried by the named fixture's `third()` (#1771). |
| `default_consts_of_the_calling_module_and_a_dependency_stay_apart_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/same_named_default_consts_across_modules` | - | codegen, parser | Asserts the emitted `crate::SIZE` path for the crate-root module's own default beside a dependency's `crate::c::SIZE`; the behavior is carried by the named fixture's `local()` (#1771). |
| `default_constructing_another_modules_type_reaches_its_fields_within_the_crate_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/private_default_items_across_modules` | - | codegen, parser | Asserts the emitted literal through `crate::a::Card` for a default and a method-partial preset in another module, and the `pub(crate)` field it reaches; the behavior is carried by the named fixture's `card_area()` (#1771). |
| `default_const_of_a_function_reached_through_an_alias_is_kept_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/same_named_default_consts_across_modules` | - | codegen, parser | Asserts the emitted `crate::a::SIZE` path and the kept const for calls through a symbol alias and an alias chain; the behavior is carried by the named fixture's `five()` (#1771). |
| `defaults_of_uncalled_functions_keep_nothing_issue1771` | retire | - | - | codegen, parser | Asserts generated-item pruning: a const only an unexpanded default names is not kept. Pruning generated Rust items belongs to the Rust-source backend alone, so the test dies with it (#1771). |
| `public_construction_default_reaches_a_consumer_through_the_dependency_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/public_construction_default_from_dependency` | - | codegen, checker, parser | Asserts the emitted construction a consumer receives for a dependency default that constructs a public model or class, spelled through the dependency's path and otherwise equal to the consumer's own construction; the behavior is carried by the named fixture (#1771). |
...
### `loaves/compiler/incan_frontend` (1749 tests in 86 files: keep 1749)

| File | Tests | Lines | Test lines | Disposition | Twins | Dies | Split | Owner | Signals | Notes |
|---|---:|---:|---:|---|---:|---:|---|---|---|---|
| `loaves/compiler/incan_frontend/src/api_metadata.rs` | 17 | 3766 | 944 | keep | - | - | - | #1561 | checker 17, parser 17 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
| `loaves/compiler/incan_frontend/src/library_exports.rs` | 6 | 2209 | 179 | keep | - | - | - | #1561 | checker 3 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
| `loaves/compiler/incan_frontend/src/typechecker/collect/stdlib_imports.rs` | 1 | 4865 | 32 | keep | - | - | - | #1561 | checker 1 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
| `loaves/compiler/incan_frontend/src/typechecker/stdlib_loader.rs` | 31 | 3183 | 1114 | keep | - | - | - | #1561 | parser 30 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/bounds_and_derives.rs` | 39 | 1162 | 1162 | keep | - | - | - | #1561 | checker 22, parser 11 | split of typechecker/tests.rs; typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. |
| `loaves/compiler/incan_frontend/src/typechecker/tests/calls_decorators_and_builtins.rs` | 52 | 1207 | 1207 | keep | - | - | - | #1561 | checker 28, parser 9 | split of typechecker/tests.rs; typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. |
...
### `loaves/compiler/incan_ir` (170 tests in 20 files: keep 1, retire 169)
...
| `loaves/compiler/incan_ir/src/expr.rs` | 3 | 1167 | 81 | retire | 0/3 | 0 | - | #1561 | legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lib.rs` | 5 | 793 | 136 | retire | 0/5 | 0 | - | #1561 | legacy_ir 5 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/decl/helpers.rs` | 3 | 984 | 150 | retire | 0/3 | 0 | - | #1561 | legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/decl/imports.rs` | 2 | 408 | 144 | retire (retire 2) | 1/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/decl/methods.rs` | 2 | 2325 | 164 | retire (retire 2) | 1/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/decl/mod.rs` | 3 | 716 | 238 | retire (retire 3) | 3/3 | 0 | - | #1561 | checker 3, parser 3, legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/expr/calls.rs` | 21 | 5690 | 1240 | retire (keep 1, retire 20) | 0/20 | 0 | - | #1561 | checker 10, parser 4, legacy_ir 19 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3187 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5494 | 1360 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/default_named_items.rs` | 1 | 157 | 157 | retire (retire 1) | 1/1 | 0 | - | #1561 | legacy_ir 1 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/default_owner_paths.rs` | 2 | 186 | 186 | retire (retire 2) | 2/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/dependency_call_arguments.rs` | 3 | 403 | 403 | retire (retire 3) | 3/3 | 0 | - | #1561 | checker 3, parser 3, legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/method_partial_forwarding.rs` | 1 | 140 | 140 | retire (retire 1) | 1/1 | 0 | - | #1561 | legacy_ir 1 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/types.rs` | 13 | 1892 | 343 | retire | 0/13 | 0 | - | #1561 | checker 1, legacy_ir 13 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/default_named_items.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `private_item_a_default_names_is_published_for_its_callers` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/private_default_items_from_dependency` | - | legacy_ir | Asserts the recorded shape on the legacy IR (a private const, static, function, model, class, enum or newtype a parameter default or method-partial preset names is lowered as a published item, with the fields a construction spells); retires with `incan_ir`. The behavior across a package boundary is carried by the named fixture; the same-project case by `cli/private_default_items_across_modules`. |

Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/default_owner_paths.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `module_items_a_default_names_are_spelled_through_their_module_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/same_named_default_consts_across_modules` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR (a const a default or method-partial preset reads lowers to a crate path to its declaring module, a function it calls carries that module's canonical callee path); retires with `incan_ir`. The behavior is carried by the named fixture (#1771). |
| `crate_root_and_unmapped_module_defaults_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/same_named_default_consts_across_modules` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR for the crate-root module (`crate::SIZE`) and for a module the crate does not map (unchanged); retires with `incan_ir`. The root case is carried by the named fixture's `local()` (#1771). |

Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/dependency_call_arguments.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `imported_partial_call_fills_its_targets_residual_default_issue1760` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/imported_partial_keeps_target_defaults` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR (a call through an imported partial carries its target's default for each parameter the partial leaves open, at the package root and through a nested namespace); retires with `incan_ir`. The behavior is carried by the named fixture (#1760). |
| `some_member_takes_the_dependency_owned_union_at_every_destination_issue1743` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/some_member_into_dependency_option_union` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR (a `Some(member)` whose destination is a dependency's `Option[union]` is instantiated at the dependency-owned union as a call argument, a list element, a model field at construction and by assignment, an annotated binding, a `match` arm and a returned value); retires with `incan_ir`. The behavior is carried by the named fixture (#1743). |
| `dependency_default_constructing_a_public_model_calls_the_type_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/public_construction_default_from_dependency` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR (a dependency default that constructs a public model lowers to a call whose callee is the type, reached through the dependency's public path; a helper-function default stays an ordinary call); retires with `incan_ir`. The behavior is carried by the named fixture (#1771). |

Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/method_partial_forwarding.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `method_partial_forwards_on_a_receiver_typed_by_its_model_issue1765` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/method_partial_over_model_method.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (a model method partial's generated forwarding call has its `self` receiver typed by the model, directly and through a method alias); retires with `incan_ir`. The behavior, `user.short()` printing the preset and `short(prefix="other")` overriding it, is carried by the named fixture (#1765). |

Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/stdlib_const_defaults.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `omitted_stdlib_default_names_the_const_through_its_declaring_module_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/stdlib_const_default_omitted.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (an omitted stdlib default naming a const is a path to the const's declaring module, typed as the const); retires with `incan_ir`. The behavior, `reader_digest(reader, "sha256")` without `chunk_size` printing the sha256 digest, is carried by the named fixture (#1771). |
...
### `loaves/toolchain/incan-cli` (634 tests in 44 files: keep 117, re-point 358, retire 34, unaffected 125)
...
| `loaves/toolchain/incan-cli/tests/behavior_snapshots_tests.rs` | 7 | 55 | 55 | re-point | - | - | - | #1561 | - | runs every behavior fixture of the snapshots_* areas (one libtest case per leaf area) as a program and compares its observables; the route changes under it, the fixtures do not. |
