...
- Retire-class tests: 1132, of which twinned 61, dies 115, open 956 (neither yet).
...
| `loaves/compiler/incan_emit/src/tests/trait_parameters_and_element_bounds.rs` | 2 | 163 | 163 | retire (retire 2) | 2/2 | 0 | - | #1561 | codegen 1, checker 1, parser 2, legacy_ir 1 | lowering through emission: trait-bound inference over generic element reads and the Rust shape of trait-method `mut` parameters on the legacy IR; retires with `incan_emit`. |
