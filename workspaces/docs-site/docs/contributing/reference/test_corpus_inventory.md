...
- Retire-class tests: 1111, of which twinned 39, dies 115, open 957 (neither yet).
...
### `loaves/compiler/incan_ir` (149 tests in 11 files: retire 149)
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5447 | 1353 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
### `loaves/toolchain/incan-cli` (628 tests in 44 files: keep 117, re-point 352, retire 34, unaffected 125)
...
| `loaves/toolchain/incan-cli/tests/behavior_snapshots_tests.rs` | 2 | 26 | 26 | re-point | - | - | - | #1561 | - | runs every behaviour fixture of the snapshots_* areas (one libtest case per leaf area) as a program and compares its observables; the route changes under it, the fixtures do not. |
