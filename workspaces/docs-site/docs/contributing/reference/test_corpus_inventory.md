...
- Retire-class tests: 1133, of which twinned 64, dies 115, open 954 (neither yet).
...
### `loaves/compiler/incan_emit` (929 tests in 58 files: keep 100, retire 827, unaffected 2)
...
| `loaves/compiler/incan_emit/src/codegen.rs` | 129 | 8040 | 5500 | retire (retire 129) | 6/129 | 0 | required | #1561 | codegen 122, text 52, checker 112, parser 82, legacy_ir 115 | IrCodegen entry point and emitter-side metadata; every test drives IrCodegen or emitter-owned merges (manifest type refs, native-union capture). Split with the retirement, not before. |
...
| `loaves/compiler/incan_emit/src/codegen/dependency_metadata.rs` | 6 | 1095 | 167 | retire | 0/6 | 0 | - | #1561 | checker 1, parser 5, legacy_ir 1 | which stdlib and provider items the generated project links; generated-project shape. |
...
| `loaves/compiler/incan_emit/src/emit/expressions/calls.rs` | 29 | 2811 | 1278 | retire | 0/29 | 0 | - | #1561 | codegen 29, legacy_ir 29 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/src/emit/expressions/methods.rs` | 2 | 1541 | 60 | retire (retire 2) | 2/2 | 0 | - | #1561 | codegen 2, legacy_ir 2 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/src/emit/mod.rs` | 20 | 4672 | 952 | retire | 0/20 | 0 | - | #1561 | codegen 15, checker 4, legacy_ir 18 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/tests/default_argument_codegen_tests.rs` | 1 | 45 | 45 | retire (retire 1) | 1/1 | 0 | - | #1561 | codegen 1, parser 1 | generated-Rust spelling of default arguments expanded outside their callable's module; asserts emitted paths, which retire with the Rust-source backend. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `same_named_default_consts_resolve_in_their_callables_modules_issue1771` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/same_named_default_consts_across_modules` | - | codegen, parser | Asserts the emitted `crate::a::SIZE` / `crate::b::SIZE` paths; the behavior, each omitted argument taking its own module's `SIZE`, is carried by the named fixture (#1771). |
...
| `loaves/compiler/incan_frontend/src/library_exports.rs` | 6 | 2119 | 179 | keep | - | - | - | #1561 | checker 3 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
### `loaves/compiler/incan_ir` (167 tests in 19 files: keep 1, retire 166)
...
| `loaves/compiler/incan_ir/src/lower/expr/calls.rs` | 21 | 5670 | 1240 | retire (keep 1, retire 20) | 0/20 | 0 | - | #1561 | checker 10, parser 4, legacy_ir 19 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3179 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5459 | 1359 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/default_named_items.rs` | 1 | 134 | 134 | retire (retire 1) | 1/1 | 0 | - | #1561 | legacy_ir 1 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/dependency_call_arguments.rs` | 2 | 331 | 331 | retire (retire 2) | 2/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
