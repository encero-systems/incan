...
- Retire-class tests: 1111, of which twinned 36, dies 115, open 960 (neither yet).
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 32 | 5412 | 1310 | retire (retire 32) | 2/32 | 0 | - | #1561 | checker 27, parser 28, legacy_ir 32 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `str_over_an_operator_expression_lowers_a_grouped_argument_issue1726` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/snapshots_collections_and_strings/str_over_arithmetic_expression.incn` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR (an operator-shaped `str(...)` argument is handed to the emitter as one grouped value in its own type; an atomic argument stays as written); retires with `incan_ir`. The behaviour, `str(a + b)` rendering the whole sum, is carried by the named fixture (#1726). |
| `derive_vocabulary_imports_lower_to_no_ir_import_issue1727` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/snapshots_stdlib/std_derives_collection_adapters_with_imported_clone.incn` | - | checker, parser, legacy_ir | Asserts the recorded import fact on the legacy IR (an import of a derivable trait's stdlib stub lowers to no IR import; the source-owned protocol and callable imports stay; the expanded adapter default and an aliased `Eq` bound spell the derive-owned Rust traits); retires with `incan_ir`. The behaviour, the FallibleIterator adapters driven with derived callables beside `from std.derives.copying import Clone`, is carried by the named fixture (#1727). |
...
| `loaves/toolchain/incan-cli/tests/behavior_snapshots_tests.rs` | 2 | 25 | 25 | re-point | - | - | - | #1561 | - | runs every behaviour fixture of the snapshots_* areas (one libtest case per leaf area) as a program and compares its observables; the route changes under it, the fixtures do not. |
