...
- Retire-class tests: 1109, of which twinned 28, dies 116, open 965 (neither yet).
...
| `loaves/compiler/incan_driver/src/cargo_policy.rs` | 4 | 270 | 92 | retire (retire 4) | 0/4 | 2 | - | #1561 | - | Two tests die with the Cargo command line (arg ordering, `INCAN_CARGO_ARGS`); two are open: the `INCAN_LOCKED/FROZEN/OFFLINE` env defaults, the `--no-locked/--no-offline/--no-frozen` negations and frozen ⇒ locked+offline feed the Oven lock policy (`loaves/compiler/incan_driver/src/lock/resolution.rs` reads `CargoPolicy.locked \|\| .frozen` for the strict `oven.lock` check; probed: `INCAN_LOCKED=1 incan build` with no lock → exit 1 `oven.lock is missing`, `--no-locked` → exit 0), which is not a Cargo field. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `cargo_policy_resolves_env_defaults_and_frozen_implication` | retire | - | - | - | OPEN (maintainer): do the `INCAN_LOCKED/FROZEN/OFFLINE` env defaults and the frozen ⇒ locked+offline implication survive as oven.lock policy inputs? If yes, the twin is a re-point CLI test (env set + missing/stale lock → refusal); not fixture-expressible (env + flags). If no, dies. |
...
| `cargo_policy_cli_disable_flags_override_env_defaults` | retire | - | - | - | OPEN (maintainer): do the `--no-locked/--no-offline/--no-frozen` negations of the env defaults survive as oven.lock policy inputs? Same twin shape as the sibling row. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `explicitly_baked_project_reuses_release_json_authority_without_cargo` | retire | `dies` | end-to-end probes of the generated-Cargo boundary: a normal build refusing `--generated-cargo-target-dir`, sealed-input reuse across projects and after source drift proven by a guarded `cargo` executable that must never launch, `--locked` refusing an `oven.lock` whose Cargo payload drifted after a `[rust-dependencies]` edit, and concurrent normal builds ignoring Cargo's `RUSTC_WRAPPER` while leaving no generated-Cargo state behind. Every assertion is whether Cargo (or a Cargo control) was consulted or refused; without a generated project none of it exists. The sealed project-output replay these probes ride on is Oven's own behaviour (unaffected class); one test carries surviving observables and is recorded as an open override. | build_run | OPEN. Beyond the Cargo-boundary assertions (dies) this test pins four surviving observables: (i) `reused sealed project Loaf` on `build --locked` after a bake — carried by `rfc031_pub_import_integration_tests.rs` (re-point); (ii) edit-after-bake: no replay, the stderr warning `the sealed project output … no longer matches this source tree`, and the build still succeeds — asserted nowhere else; maintainer: does the warning survive? If yes, twin = a re-point CLI test (bake → edit → build), not a fixture shape; (iii) `--locked` after a manifest edit → `oven.lock is out of date` — carried by `cli_workspace_and_lock_tests.rs`, `cli_interop_target_tests.rs`; (iv) `incan test tests --fail-on-empty` on a baked project using `std.json` (#1056) — carried by no other test. |
...
| `a_package_projects_a_call_into_its_own_sibling_module` | retire | `dies` | asserts the RFC 120 `__incan_v1_` projected wrapper name, not the source spelling `c.bumped()`, in a package's generated `lib.rs` (#1174 emitted-name recoverability); the emitted-name projection and the legacy `incan_ir` lowering that required it die with generated Rust. The package-calls-its-own-sibling behaviour is reachable only through a `--lib`/SDK component build, which is not a run the fixture format can express. The surviving observable — a package whose library calls a sibling module's method builds — is carried by the SDK provider prewarm every suite root performs. | build_run | - |
