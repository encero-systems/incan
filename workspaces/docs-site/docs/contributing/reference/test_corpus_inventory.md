...
- Retire-class tests: 1136, of which twinned 66, dies 115, open 955 (neither yet).
...
### `loaves/compiler/incan_emit` (931 tests in 57 files: keep 100, retire 829, unaffected 2)
...
| `loaves/compiler/incan_emit/src/tests/static_reads_and_frozen_strings.rs` | 4 | 260 | 260 | retire (retire 4) | 4/4 | 0 | - | #1561 | codegen 3, parser 3, legacy_ir 1 | generated Rust for static collection reads and const string destinations, lowering through emission end to end. |
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/static_reads_and_frozen_payloads.rs` | 4 | 163 | 163 | keep | - | - | - | #1561 | checker 4 | checker facts for a static dict's `get` result and a `Some` payload's string instantiation (#1793, #1794); typechecker only, no emit/driver dependency. |
...
### `loaves/compiler/incan_ir` (168 tests in 18 files: keep 1, retire 167)
...
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3166 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/tests/static_method_args.rs` | 2 | 225 | 225 | retire (retire 2) | 1/2 | 0 | - | #1561 | legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
