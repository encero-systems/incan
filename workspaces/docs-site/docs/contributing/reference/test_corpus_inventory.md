...
- Retire-class tests: 1139, of which twinned 68, dies 115, open 956 (neither yet).
...
| `loaves/compiler/incan_emit/src/codegen/dependency_metadata.rs` | 7 | 1111 | 184 | retire (retire 7) | 1/7 | 0 | - | #1561 | checker 1, parser 6, legacy_ir 6 | which stdlib and provider items the generated project links; generated-project shape. |
| `loaves/compiler/incan_emit/src/conversions.rs` | 79 | 2818 | 1578 | retire | 0/79 | 0 | required | #1561 | legacy_ir 74 | Rust conversion policy (to_string/borrow/clone plans) for emission; the duckborrower facts in Body IR are the twin surface. |
| `loaves/compiler/incan_emit/src/emit/decls/functions.rs` | 3 | 2016 | 115 | retire | 0/3 | 0 | - | #1561 | codegen 3, legacy_ir 3 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/src/tests/trait_parameters_and_element_bounds.rs` | 5 | 358 | 358 | retire (retire 5) | 5/5 | 0 | - | #1561 | codegen 3, checker 2, parser 5, legacy_ir 2 | lowering through emission: trait-bound inference over generic element reads and the Rust shape of trait-method `mut` parameters on the legacy IR; retires with `incan_emit`. |
| `loaves/compiler/incan_emit/src/trait_bound_inference.rs` | 15 | 4507 | 703 | retire | 0/15 | 0 | - | #1561 | codegen 5, legacy_ir 15 | infers Rust trait bounds for generated generics; Rust-shape concern. |
...
Per-test overrides in `loaves/compiler/incan_emit/src/codegen/dependency_metadata.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `selected_trait_keeps_its_default_body_helpers_issue1759` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_reaches_its_module_names_across_modules` | - | parser, legacy_ir | Asserts that a selected trait keeps the helper functions its defaults call among the dependency module's emitted items; retires with the Rust-source backend. The behavior, an adopter in another module printing the result of a default that calls the trait module's private helper, is carried by the named fixture (#1759). |
...
Per-test overrides in `loaves/compiler/incan_emit/src/tests/trait_parameters_and_element_bounds.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `index_read_of_a_type_parameter_element_states_clone_issue1756` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/generic_list_index_without_clone_bound.incn` | - | checker, parser, legacy_ir | Asserts the inferred `Clone` bound an element read of a type-parameter list or dict value needs; retires with `incan_emit`. The behavior, `first[K]` returning `items[0]` for int and str elements, is carried by the named fixture (#1756). |
| `trait_method_and_slice_element_copies_state_clone_on_every_signature_issue1756` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/generic_list_index_without_clone_bound.incn` | - | checker, parser, legacy_ir | Asserts the inferred `Clone` bound a list slice and a trait method's element read or slice need on the function, the trait slot and each implementation; retires with `incan_emit`. The behavior, `rest[K]` slicing and a trait's `pick[K]`/`tail[K]` defaults for int and str elements, is carried by the named fixture (#1756). |
| `trait_method_mut_parameter_keeps_one_shape_across_slot_impl_and_wrapper_issue1773` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_method_mutable_parameter.incn` | - | codegen, parser | Asserts generated Rust text for a trait method's `mut` parameters (slot, default, implementation, wrapper, call site); retires with the Rust-source backend. The behavior, a trait default appending to its `mut` list parameter and the caller seeing the change, is carried by the named fixture (#1773). |
| `scalar_mut_parameter_is_a_mutable_local_in_every_body_issue1773` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_method_mutable_parameter.incn` | - | codegen, parser | Asserts generated Rust text for a scalar `mut` parameter changed in a function, a method and a trait default; retires with the Rust-source backend. The behavior, a trait default incrementing its `mut int` parameter, is carried by the named fixture (#1773). |
| `mut_arguments_follow_the_checker_facts_issue1773` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_method_mutable_parameter.incn` | - | codegen, parser | Asserts generated Rust text for `mut` arguments through a generic bound, a copied immutable binding and an `int` alias; retires with the Rust-source backend. The behavior, a generic call appending through a trait bound, an unchanged `mut` parameter taking an immutable list, and an `int` alias staying the callee's own copy, is carried by the named fixture (#1773). |
...
| `loaves/compiler/incan_frontend/src/typechecker/check_expr/calls/rust_boundary.rs` | 43 | 2531 | 1425 | keep | - | - | - | #1561 | text 2, checker 43 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/mut_parameters.rs` | 5 | 315 | 315 | keep | - | - | - | #1561 | checker 5, parser 4 | typechecker: `mut` parameters as mutable bindings and the INCAN-T0117 refusal of immutable arguments; no emit/driver dependency. |
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/pub_imports_trait_adoptions.rs` | 9 | 798 | 798 | keep | - | - | - | #1561 | checker 9, parser 3 | split of typechecker/tests.rs; typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. |
...
### `loaves/compiler/incan_ir` (169 tests in 16 files: keep 1, retire 168)
...
| `loaves/compiler/incan_ir/src/lower/decl/helpers.rs` | 3 | 1013 | 150 | retire | 0/3 | 0 | - | #1561 | legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/decl/methods.rs` | 2 | 2326 | 164 | retire (retire 2) | 1/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/decl/mod.rs` | 3 | 718 | 238 | retire (retire 3) | 3/3 | 0 | - | #1561 | checker 3, parser 3, legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/decl/traits.rs` | 3 | 375 | 45 | retire | 0/3 | 0 | - | #1561 | legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/calls.rs` | 21 | 5663 | 1240 | retire (keep 1, retire 20) | 0/20 | 0 | - | #1561 | checker 10, parser 4, legacy_ir 19 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3243 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/patterns.rs` | 4 | 1204 | 276 | retire (retire 4) | 4/4 | 0 | - | #1561 | checker 4, parser 4, legacy_ir 4 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5798 | 1356 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/imported_trait_adoption_scope.rs` | 7 | 439 | 439 | retire (retire 7) | 5/7 | 0 | - | #1561 | checker 6, parser 7, legacy_ir 7 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/types.rs` | 13 | 1899 | 343 | retire | 0/13 | 0 | - | #1561 | checker 1, legacy_ir 13 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/imported_trait_adoption_scope.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `imported_trait_default_names_its_module_types_by_path_issue1759` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_names_its_own_module_types` | - | checker, parser, legacy_ir | Asserts on the legacy IR that an imported trait default expanded into an adopter names its own module's model by the defining-module Rust path; retires with `incan_ir`. The behavior, the adopter printing the area of the model the default returns without importing it, is carried by the named fixture (#1759). |
| `derive_bundle_trait_counts_as_adopted_for_direct_calls_issue1792` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/module_derive_default_method_direct_call` | - | checker, parser, legacy_ir | Asserts on the legacy IR that a direct call of a derive-bundle trait's method is routed to the recoverable projection the derived implementation provides; retires with `incan_ir`. The behavior, `item.tag()` printing the default's result under the plain and the aliased module binding, is carried by the named fixture (#1792). |
| `imported_trait_default_reaches_its_module_names_issue1759` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_reaches_its_module_names_across_modules` | - | checker, parser, legacy_ir | Asserts on the legacy IR that an imported trait default expanded into an adopter reads its module's enum variant, matches on it, constructs a model its module imports and calls its module's helper, each by the declaring module's Rust path; retires with `incan_ir`. The behavior, the adopter printing the helper-built size, the matched corner and its own label, is carried by the named fixture (#1759). |
| `reexported_trait_binds_to_its_declaration_issue1759` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_reaches_its_module_names_across_modules` | - | checker, parser, legacy_ir | Asserts on the legacy IR that a trait imported through a re-exporting module, with and without defaults, gets the adopter's methods and its expanded defaults in its implementation; retires with `incan_ir`. The behavior, an adopter importing both traits from the re-exporting module and printing their results, is carried by the named fixture (#1759). |
| `trait_default_helpers_are_visible_to_the_crate_issue1759` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/trait_default_reaches_its_module_names_across_modules` | - | checker, parser, legacy_ir | Asserts on the legacy IR that a private function a public trait's default calls is lowered with crate visibility and an uncalled one stays private; retires with `incan_ir`. The behavior, an adopter in another module printing the result of a default that calls the trait module's private helper, is carried by the named fixture (#1759). |
