...
| `loaves/kernel/incan_syntax/src/diagnostics/stable.rs` | 6 | 656 | 149 | keep | - | - | - | #1561 | checker 6 | lexer, parser and diagnostics catalog; below the emitter, cannot reach codegen. Reviewed at crate level. |
...
| `loaves/toolchain/incan-cli/tests/behavior_snapshots_tests.rs` | 4 | 39 | 39 | re-point | - | - | - | #1561 | - | runs every behavior fixture of the snapshots_* areas (one libtest case per leaf area) as a program and compares its observables; the route changes under it, the fixtures do not. |
