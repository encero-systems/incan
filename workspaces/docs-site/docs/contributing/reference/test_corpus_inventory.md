...
- Retire-class tests: 1144, of which twinned 75, dies 115, open 954 (neither yet).
- Retire-class files with open rows: 65 (a file whose retire tests are all twinned or recorded `dies` is done).
...
### `loaves/compiler/incan_emit` (933 tests in 58 files: keep 100, retire 831, unaffected 2)
...
| `loaves/compiler/incan_emit/src/codegen.rs` | 129 | 8050 | 5511 | retire (retire 129) | 8/129 | 0 | required | #1561 | codegen 122, text 52, checker 112, parser 82, legacy_ir 115 | IrCodegen entry point and emitter-side metadata; every test drives IrCodegen or emitter-owned merges (manifest type refs, native-union capture). Split with the retirement, not before. |
...
| `loaves/compiler/incan_emit/src/emit/native_unions.rs` | 8 | 2072 | 1082 | retire (retire 8) | 1/8 | 0 | - | #1561 | codegen 6, checker 7, parser 7, legacy_ir 6 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/src/tests/dependency_method_unions.rs` | 1 | 94 | 94 | retire (retire 1) | 1/1 | 0 | - | #1561 | codegen 1 | lowering through emission end to end. |
...
| `loaves/compiler/incan_emit/tests/stdlib_generated_rust_snapshot_tests.rs` | 14 | 446 | 446 | retire (retire 14) | 1/14 | 0 | - | #1561 | codegen 14, snapshot 12, parser 14 | insta snapshots of generated stdlib Rust. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `top_level_qualified_alias_preserves_target_path` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/module_member_alias_calls_the_member.incn` | - | codegen, checker, parser, legacy_ir | twinned by the retired test's program run with a `main`: a public and a private alias of a `std.math` member reached through `import std.math as math` call the member. The program failed to build (E0425) until the alias lowered as an import of the member (#1764). |
...
| `test_relative_from_import_uses_super_prefix` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/relative_import_from_a_nested_module` | - | codegen, generated_text, checker, parser, legacy_ir | twinned by the retired test's layout run to completion: `store/relative.incn` reads a field through `from ..db.schema import Database`, and a sibling uses the `super::` spelling. The retired test pinned `use super::db::schema::Database`, one level short (E0433); the import now names the module the checker bound by its crate-absolute path (#1766). |
...
Per-test overrides in `loaves/compiler/incan_emit/src/emit/native_unions.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `publication_captures_nominal_method_return_unions` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_method_returning_a_union` | - | codegen, checker, parser, legacy_ir | twinned by the program the corpus work parked as a candidate: a dependency model method returning `int \| str` is called for both members and each result is passed to a consumer `int \| str` parameter. The run failed (E0308) until a dependency method's union result kept the provider's carrier (#1797); the captured wrapper name dies with #654. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `dependency_method_union_results_convert_into_the_consumers_unions_issue1797` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_method_returning_a_union` | - | codegen | Asserts the emitted conversions from the provider's union wrapper into the consumer's, bare and element by element, and the option's member patterns through the provider's wrapper; dies with the emitter (#654). The behavior is carried by the named fixture (#1797). |

Per-test overrides in `loaves/compiler/incan_emit/src/tests/reexported_projections.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `alias_declaration_facades_bind_the_projection_they_reexport_issue1750` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/facade_reexports_an_alias_without_its_target` | - | codegen | Asserts the emitted `use` lines of each facade (the projection bound publicly once, beside every public name); dies with the emitter (#654). The behavior, consumers calling every name, is carried by the named fixture (#1750). |
| `private_import_beside_a_public_reexport_binds_the_projection_publicly_issue1750` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/facade_reexports_an_alias_without_its_target` | - | codegen | Asserts the emitted `use` lines (the projection bound publicly, with no private binding beside it); dies with the emitter (#654). The behavior is carried by the named fixture's `private_then_public` facade (#1750). |
| `module_member_alias_beside_a_direct_import_binds_publicly_issue1764` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/module_member_alias_beside_a_direct_import` | - | codegen | Asserts the emitted `use` lines (the member's projection bound publicly under the alias's name, with no private binding beside it); dies with the emitter (#654). The behavior is carried by the named fixture (#1764). |
| `module_reexport_of_a_package_renamed_export_binds_the_projection_issue1751` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_renamed_export_reexported_under_a_new_name` | - | codegen | Asserts the emitted `use` lines (a module re-exporting a package's renamed export binds the projection publicly beside the further name); dies with the emitter (#654). The behavior, a sibling module calling the further name, is carried by the named fixture (#1751). |
...
Per-test overrides in `loaves/compiler/incan_emit/tests/stdlib_generated_rust_snapshot_tests.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `std_root_prelude_trait_import_is_refused` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/refused_prelude_trait_imported_from_the_std_root.incn` | - | codegen, parser | twinned by the test's program as a refusal: the std root exports only its submodules, so `from std import Debug, Eq, Clone, …` is refused at check time (INCAN-I0001) rather than emitting re-exports no standard-library facade defines (E0432) (#1767). |
...
| `loaves/compiler/incan_frontend/src/typechecker/collect/stdlib_imports.rs` | 1 | 4832 | 32 | keep | - | - | - | #1561 | checker 1 | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
### `loaves/compiler/incan_ir` (174 tests in 18 files: keep 1, retire 173)
...
| `loaves/compiler/incan_ir/src/lower/decl/imports.rs` | 2 | 492 | 144 | retire (retire 2) | 1/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/decl/mod.rs` | 3 | 723 | 238 | retire (retire 3) | 3/3 | 0 | - | #1561 | checker 3, parser 3, legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/expr/calls.rs` | 21 | 5639 | 1240 | retire (keep 1, retire 20) | 0/20 | 0 | - | #1561 | checker 10, parser 4, legacy_ir 19 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3191 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5479 | 1358 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/import_paths.rs` | 6 | 198 | 198 | retire (retire 6) | 4/6 | 0 | - | #1561 | checker 4, parser 4, legacy_ir 6 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/pub_method_results.rs` | 1 | 183 | 183 | retire (retire 1) | 1/1 | 0 | - | #1561 | checker 1, legacy_ir 1 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/reexported_projections.rs` | 5 | 143 | 143 | retire (retire 5) | 4/5 | 0 | - | #1561 | legacy_ir 5 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/import_paths.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `relative_import_lowers_to_the_crate_absolute_module_the_checker_bound_issue1766` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/relative_import_from_a_nested_module` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR (a relative import reaches the emitter as the crate-absolute path of the module the checker bound); retires with `incan_ir`. The behavior, `from ..db.schema import Database` in `store/relative.incn` printing `relative:7`, is carried by the named fixture (#1766). |
| `rust_style_parent_import_lowers_to_the_same_module_issue1766` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/relative_import_from_a_nested_module` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR for the `super::` spelling of the same climb; retires with `incan_ir`. The behavior is carried by the named fixture's `store/rust_style.incn` (#1766). |
| `grandparent_relative_import_lowers_to_the_root_module_issue1766` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/relative_import_from_a_nested_module` | - | checker, parser, legacy_ir | Asserts the lowered shape on the legacy IR for `...`, two directories up from `app/store/`; retires with `incan_ir`. The behavior is carried by the named fixture's `app/store/nested.incn` (#1766). |
| `module_member_alias_lowers_to_an_import_of_the_member_issue1764` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/module_member_alias_calls_the_member.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (an alias of a module member lowers to an import of that member under the alias's name and visibility); retires with `incan_ir`. The behavior, a public and a private alias of a `std.math` member calling it, is carried by the named fixture (#1764). |

Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/pub_method_results.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `pub_dependency_method_results_keep_the_providers_union_issue1797` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_method_returning_a_union` | - | checker, legacy_ir | Asserts the lowered shape on the legacy IR (a `pub::` dependency method's result keeps the provider's `int \| str` carrier, bare, in a list and in an option); retires with `incan_ir`. The behavior, the consumer passing each result to its own `int \| str` positions and matching the option by member, is carried by the named fixture (#1797). |

Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/reexported_projections.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `alias_declaration_reexported_without_its_target_binds_the_projection_issue1750` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/facade_reexports_an_alias_without_its_target` | - | legacy_ir | Asserts the lowered import item (a re-exported `alias` declaration, and a renaming second hop, spelled by the target's name with the alias's name as its alias); retires with `incan_ir`. The behavior, consumers of both facades calling the alias, is carried by the named fixture (#1750). |
| `target_reexported_beside_its_alias_binds_the_projection_first_issue1750` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/facade_reexports_an_alias_without_its_target` | - | legacy_ir | Asserts the lowered import order (a re-exported target comes ahead of its re-exported alias in either source order); retires with `incan_ir`. The behavior is carried by the named fixture's `both_names` facade, which lists the alias first (#1750). |
| `private_import_before_a_public_reexport_stands_behind_it_issue1750` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/facade_reexports_an_alias_without_its_target` | - | legacy_ir | Asserts the lowered import order (a public re-export of an alias comes ahead of a private import of its target); retires with `incan_ir`. The behavior is carried by the named fixture's `private_then_public` facade (#1750). |
| `module_member_alias_beside_a_direct_import_binds_publicly_issue1764` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/module_member_alias_beside_a_direct_import` | - | legacy_ir | Asserts the lowered import order (the public import an alias of a module member lowers to comes ahead of a private import of the same member); retires with `incan_ir`. The behavior, a consumer calling the alias, is carried by the named fixture (#1764). |
...
### `loaves/kernel/incan_syntax` (310 tests in 15 files: keep 310)
...
| `loaves/kernel/incan_syntax/src/parser/tests/modules_and_imports.rs` | 48 | 911 | 911 | keep | - | - | - | #1561 | checker 31, parser 48 | split of parser/tests.rs; lexer, parser and diagnostics catalog; below the emitter, cannot reach codegen. Reviewed at crate level. |
