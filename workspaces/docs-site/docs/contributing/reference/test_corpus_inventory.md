...
| re-point | 496 | 50 | 430 |
| retire | 1135 | 79 | 0 |
| unaffected | 1440 | 134 | 5 |
| unreviewed | 0 | 0 | 0 |
| **Total** | **6227** | **472** | **442** |

- Retire-class tests: 1135, of which twinned 65, dies 115, open 955 (neither yet).
...
| `loaves/compiler/incan_emit/src/emit/expressions/builtins.rs` | 5 | 1099 | 174 | retire | 0/5 | 0 | - | #1561 | codegen 3, legacy_ir 4 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/src/emit/expressions/indexing.rs` | 2 | 496 | 68 | retire | 0/2 | 0 | - | #1561 | codegen 2, legacy_ir 2 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/src/emit/types.rs` | 3 | 701 | 61 | retire | 0/3 | 0 | - | #1561 | codegen 3, legacy_ir 2 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
| `loaves/compiler/incan_emit/src/ownership.rs` | 46 | 2041 | 895 | retire (retire 46) | 1/46 | 0 | - | #1561 | legacy_ir 46 | emitter-side ownership and argument plans (borrow/clone/move for generated Rust); twins belong to Body IR ownership facts. |
...
| `loaves/compiler/incan_emit/src/tests/emitted_spellings.rs` | 4 | 194 | 194 | retire (retire 4) | 4/4 | 0 | - | #1561 | codegen 4, parser 4 | asserts the Rust text emission gives keyword identifiers, a program type named like a runtime surface type, `set()` over a generator and `const` `FrozenDict` reads; generated Rust dies with #654. |
...
Per-test overrides in `loaves/compiler/incan_emit/src/tests/emitted_spellings.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `host_keyword_identifiers_are_raw_in_every_position_issue1775` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/host_keyword_identifiers.incn` | - | codegen, parser | Asserts generated Rust text; the behavior, keyword-named fields, parameters, variants and type parameters running and reflection and JSON reporting `impl`, is carried by the named fixture (#1775). |
| `program_type_named_like_a_runtime_surface_type_keeps_its_own_spelling_issue1770` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/user_model_named_field_info.incn` | - | codegen, parser | Asserts generated Rust text; the behavior, `FieldInfo(label="custom").label` printing `custom`, is carried by the named fixture (#1770). |
| `set_constructor_over_a_generator_collects_through_the_iterator_trait_issue1744` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/set_constructor_over_generator.incn` | - | codegen, parser | Asserts generated Rust text; the behavior is carried by the named fixture (#1744). |
| `const_frozen_dict_reads_use_the_frozen_lookup_issue1757` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/const_frozen_collection_reads.incn` | - | codegen, parser | Asserts generated Rust text; the behavior is carried by the named fixture (#1757). |
...
### `loaves/compiler/incan_frontend` (1747 tests in 84 files: keep 1747)
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/collections_strings_and_bytes.rs` | 49 | 1104 | 1104 | keep | - | - | - | #1561 | checker 44, parser 4 | split of typechecker/tests.rs; typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. |
...
### `loaves/compiler/incan_ir` (167 tests in 16 files: keep 1, retire 166)
...
| `loaves/compiler/incan_ir/src/expr.rs` | 3 | 1177 | 81 | retire | 0/3 | 0 | - | #1561 | legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3183 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5450 | 1356 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/collection_sources_and_static_reads.rs` | 4 | 230 | 230 | retire (retire 4) | 4/4 | 0 | - | #1561 | legacy_ir 4 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/types.rs` | 53 | 1348 | 496 | retire | 0/53 | 0 | - | #1561 | legacy_ir 53 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/collection_sources_and_static_reads.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `annotated_local_bound_from_a_static_aliases_its_storage_issue1777` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/static_alias_with_type_annotation.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (a local annotated with its static's type binds the static's storage alias and carries no Rust annotation); retires with `incan_ir`. The behavior, `count: int = COUNT` printing the static's value and an annotated list alias appending to the static, is carried by the named fixture (#1777). |
| `frozen_dict_index_carries_the_value_type_issue1757` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/const_frozen_collection_reads.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (a `const` `FrozenDict` index read typed as the value type, `contains_key` as keyed membership); retires with `incan_ir`. The behavior, `TABLE["names"]` and `TABLE.contains_key(...)` answering, is carried by the named fixture (#1757). |
| `comprehension_over_frozen_text_iterates_owned_items_issue1757` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/const_frozen_collection_reads.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (a comprehension over a `const` `FrozenList[str]` iterates the source's `list(...)` conversion); retires with `incan_ir`. The behavior, `" ".join([name for name in NAMES])` printing `alpha beta`, is carried by the named fixture (#1757). |
| `set_constructor_over_a_generator_plans_the_iterator_trait_issue1744` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/set_constructor_over_generator.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (`set(generator)` plans collection through the `Iterator` trait); retires with `incan_ir`. The behavior, `set(numbers(3))` holding three values, is carried by the named fixture (#1744). |
...
??? note "Unaffected crates (1151 tests in 90 files)"
