...
- Retire-class tests: 1114, of which twinned 42, dies 115, open 957 (neither yet).
...
### `loaves/compiler/incan_frontend` (1710 tests in 83 files: keep 1710)
...
### `loaves/compiler/incan_ir` (152 tests in 13 files: retire 152)
...
| `loaves/compiler/incan_ir/src/lower/expr/calls.rs` | 20 | 5419 | 1079 | retire | 0/20 | 0 | - | #1561 | checker 9, parser 4, legacy_ir 18 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3148 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5458 | 1356 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `str_over_an_operator_expression_lowers_a_grouped_argument_issue1726` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/snapshots_collections_and_strings/str_over_arithmetic_expression.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (an operator-shaped `str(...)` argument is handed to the emitter as one grouped value in its own type; an atomic argument stays as written); retires with `incan_ir`. The behaviour, `str(a + b)` rendering the whole sum, is carried by the named fixture (#1726). |

Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/derive_vocabulary_imports.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `derive_vocabulary_imports_lower_to_no_ir_import_issue1727` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/snapshots_stdlib/std_derives_collection_adapters_with_imported_clone.incn` | - | legacy_ir | Asserts the recorded import fact on the legacy IR (an import of a derivable trait's stdlib stub lowers to no IR import; the source-owned protocol and callable imports stay; the expanded adapter default and an aliased `Eq` bound spell the derive-owned Rust traits); retires with `incan_ir`. The behaviour, the FallibleIterator adapters driven with derived callables beside `from std.derives.copying import Clone`, is carried by the named fixture (#1727). |
...
### `loaves/kernel/incan_lang` (108 tests in 21 files: keep 108)
...
| `loaves/kernel/incan_lang/src/lang/stdlib.rs` | 11 | 1445 | 472 | keep | - | - | - | #1561 | checker 1 | language registry (stdlib inventory, interop metadata, version); below the emitter. Reviewed at crate level. |
...
| `loaves/kernel/incan_syntax/src/diagnostics/stable.rs` | 6 | 656 | 149 | keep | - | - | - | #1561 | checker 6 | lexer, parser and diagnostics catalogue; below the emitter, cannot reach codegen. Reviewed at crate level. |
...
### `loaves/toolchain/incan-cli` (630 tests in 44 files: keep 117, re-point 354, retire 34, unaffected 125)
...
| `loaves/toolchain/incan-cli/tests/behavior_snapshots_tests.rs` | 4 | 39 | 39 | re-point | - | - | - | #1561 | - | runs every behaviour fixture of the snapshots_* areas (one libtest case per leaf area) as a program and compares its observables; the route changes under it, the fixtures do not. |
