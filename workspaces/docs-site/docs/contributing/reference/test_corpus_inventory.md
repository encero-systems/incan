...
- Retire-class tests: 1126, of which twinned 59, dies 115, open 952 (neither yet).
...
### `loaves/compiler/incan_emit` (925 tests in 56 files: keep 100, retire 823, unaffected 2)
...
| `loaves/compiler/incan_emit/src/codegen.rs` | 127 | 7947 | 5408 | retire (retire 127) | 6/127 | 0 | required | #1561 | codegen 120, text 51, checker 110, parser 80, legacy_ir 113 | IrCodegen entry point and emitter-side metadata; every test drives IrCodegen or emitter-owned merges (manifest type refs, native-union capture). Split with the retirement, not before. |
...
### `loaves/compiler/incan_frontend` (1753 tests in 84 files: keep 1753)
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/calls_decorators_and_builtins.rs` | 55 | 1367 | 1367 | keep | - | - | - | #1561 | checker 31, parser 9 | split of typechecker/tests.rs; typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. |
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/collections_strings_and_bytes.rs` | 50 | 1124 | 1124 | keep | - | - | - | #1561 | checker 45, parser 4 | split of typechecker/tests.rs; typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. |
...
| `loaves/compiler/incan_frontend/src/typechecker/tests/statements_and_bindings.rs` | 72 | 1293 | 1293 | keep | - | - | - | #1561 | checker 69, parser 2 | split of typechecker/tests.rs; typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. |
...
| `loaves/compiler/incan_frontend/src/vocab_ast_bridge.rs` | 12 | 1906 | 491 | keep | - | - | - | #1561 | - | typechecker, Body IR, library manifests, provider plans; no emit/driver dependency. Reviewed at crate level; generated-text hits are Incan source or diagnostics. |
...
### `loaves/compiler/incan_ir` (164 tests in 16 files: keep 1, retire 163)
...
| `loaves/compiler/incan_ir/src/lower/expr/calls.rs` | 21 | 5627 | 1240 | retire (keep 1, retire 20) | 0/20 | 0 | - | #1561 | checker 10, parser 4, legacy_ir 19 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3172 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5451 | 1357 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/display_operands.rs` | 2 | 121 | 121 | retire (retire 2) | 2/2 | 0 | - | #1561 | legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
Per-test overrides in `loaves/compiler/incan_ir/src/lower/tests/display_operands.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `print_arguments_render_structural_values_like_interpolation_issue1748` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/display_positions_render_collections_alike.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (a structural `print`/`println` argument is handed to the emitter as the one-part f-string rendering, a scalar stays as written); retires with `incan_ir`. The behavior, `print(items)` printing `[1, 2, 3]` like `f"{items}"`, is carried by the named fixture (#1748). |
| `str_of_a_structural_value_is_its_interpolation_issue1748` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/display_positions_render_collections_alike.incn` | - | legacy_ir | Asserts the recorded shape on the legacy IR (`str(items)` becomes the one-part f-string rendering, `str(count)` keeps the builtin conversion); retires with `incan_ir`. The behavior, `str(items)` printing `[1, 2, 3]`, is carried by the named fixture (#1748). |
...
| `loaves/kernel/incan_lang/src/lang/conventions.rs` | 1 | 68 | 13 | keep | - | - | - | #1561 | - | language registry (stdlib inventory, interop metadata, version); below the emitter. Reviewed at crate level. |
...
| `loaves/kernel/incan_syntax/src/diagnostics/stable.rs` | 8 | 955 | 272 | keep | - | - | - | #1561 | checker 8 | lexer, parser and diagnostics catalog; below the emitter, cannot reach codegen. Reviewed at crate level. |
