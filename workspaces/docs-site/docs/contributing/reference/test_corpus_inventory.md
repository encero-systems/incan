...
- Retire-class tests: 1135, of which twinned 68, dies 115, open 952 (neither yet).
...
### `loaves/compiler/incan_emit` (930 tests in 56 files: keep 100, retire 828, unaffected 2)
...
| `loaves/compiler/incan_emit/src/codegen.rs` | 129 | 8051 | 5512 | retire (retire 129) | 8/129 | 0 | required | #1561 | codegen 122, text 52, checker 112, parser 82, legacy_ir 115 | IrCodegen entry point and emitter-side metadata; every test drives IrCodegen or emitter-owned merges (manifest type refs, native-union capture). Split with the retirement, not before. |
...
| `loaves/compiler/incan_emit/src/emit/decls/mod.rs` | 8 | 1395 | 396 | retire (retire 8) | 3/8 | 0 | - | #1561 | codegen 5, checker 2, legacy_ir 5 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `loaves/compiler/incan_emit/tests/stdlib_generated_rust_snapshot_tests.rs` | 14 | 447 | 447 | retire (retire 14) | 1/14 | 0 | - | #1561 | codegen 14, snapshot 12, parser 14 | insta snapshots of generated stdlib Rust. |
...
Per-test overrides in `loaves/compiler/incan_emit/src/emit/decls/mod.rs`:

| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `alias_declaration_reexport_without_its_target_binds_the_projection_issue1750` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/facade_reexports_an_alias_without_its_target` | - | codegen, legacy_ir | Asserts the emitted `use` lines (a facade re-exporting only an `alias` declaration, and a renaming second hop, bind the projection beside the alias's name); dies with the emitter (#654). The behavior, consumers of both facades calling the alias, is carried by the named fixture (#1750). |
| `alias_declaration_reexport_beside_its_target_binds_the_projection_once_issue1750` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/facade_reexports_an_alias_without_its_target` | - | codegen, legacy_ir | Asserts the emitted `use` lines (a facade re-exporting an alias beside its target binds the projection once, in either order); dies with the emitter (#654). The behavior is carried by the named fixture's `both_names` facade, which lists the alias first (#1750). |
| `renamed_package_reexport_binds_the_projection_issue1751` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_renamed_export_reexported_under_a_new_name` | - | codegen, legacy_ir | Asserts the emitted `use` lines (a `pub::` re-export of a dependency's renamed function under a further name binds the projection); dies with the emitter (#654). The behavior, a consumer calling the further name, is carried by the named fixture (#1751). |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `std_root_prelude_import_snapshot` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli/refused_prelude_trait_imported_from_the_std_root.incn` | - | codegen, parser | twinned by the snapshot's program as a refusal: the std root exports only its submodules, so `from std import Debug, Eq, Clone, …` is refused at check time (INCAN-I0001) instead of emitting `pub use crate::__incan_std::Debug;` and seven more re-exports no facade defines (E0432). The test now asserts that refusal (#1767). |
...
### `loaves/compiler/incan_ir` (168 tests in 17 files: keep 1, retire 167)
...
| `loaves/compiler/incan_ir/src/lower/decl/imports.rs` | 2 | 490 | 144 | retire (retire 2) | 1/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/decl/mod.rs` | 3 | 722 | 238 | retire (retire 3) | 3/3 | 0 | - | #1561 | checker 3, parser 3, legacy_ir 3 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/expr/mod.rs` | 15 | 3185 | 351 | retire | 0/15 | 0 | - | #1561 | checker 3, legacy_ir 15 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5504 | 1357 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/import_paths.rs` | 5 | 177 | 177 | retire (retire 5) | 3/5 | 0 | - | #1561 | checker 3, parser 3, legacy_ir 5 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
| `loaves/compiler/incan_ir/src/lower/tests/pub_method_results.rs` | 1 | 95 | 95 | retire (retire 1) | 1/1 | 0 | - | #1561 | checker 1, legacy_ir 1 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `pub_dependency_method_result_keeps_the_providers_union_issue1797` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_method_returning_a_union` | - | checker, legacy_ir | Asserts the recorded shape on the legacy IR (a `pub::` dependency method's union result takes the provider-owned carrier); retires with `incan_ir`. The behavior, the consumer passing `Box.answer()` to its own `int \| str` parameter, is carried by the named fixture (#1797). |
