...
- Retire-class tests: 1134, of which twinned 64, dies 115, open 955 (neither yet).
...
### `loaves/compiler/incan_emit` (929 tests in 56 files: keep 100, retire 827, unaffected 2)
...
| `loaves/compiler/incan_emit/src/codegen.rs` | 131 | 8185 | 5600 | retire (retire 131) | 7/131 | 0 | required | #1561 | codegen 124, text 52, checker 114, parser 84, legacy_ir 117 | IrCodegen entry point and emitter-side metadata; every test drives IrCodegen or emitter-owned merges (manifest type refs, native-union capture). Split with the retirement, not before. |
...
| `loaves/compiler/incan_emit/src/emit/native_unions.rs` | 8 | 2072 | 1082 | retire (retire 8) | 2/8 | 0 | - | #1561 | codegen 6, checker 7, parser 7, legacy_ir 6 | emitter unit tests; frozen with the emitter under #1561, deleted by #654 once twinned. |
...
| `same_named_nominals_in_two_modules_get_one_union_wrapper_each_issue1796` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_same_named_declarations_in_two_modules` | - | codegen, checker, parser, legacy_ir | Asserts the generated Rust (one wrapper per module, each carrying its own module's `Product`); dies with the Rust-source backend (#654). The behavior is carried by the named fixture (#1796). |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `emitted_union_publication_is_scoped_to_module_and_span` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_same_named_declarations_in_two_modules` | - | codegen, checker, parser, legacy_ir | twinned by the program the corpus work parked as a candidate: a dependency whose `first` and `second` modules declare `Product` and `Answer = Product \| int` at the same source positions bakes and each module's union serves its own `Product`. The bake stopped at the publication invariant until each module's union got its own wrapper (#1796); the `rust_name` spellings and the manifest capture die with #654. |
| `native_union_local_nominals_keep_module_identity_through_forwarding` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_same_named_declarations_in_two_modules` | - | codegen, checker, parser, legacy_ir | twinned by the same candidate program: each module's `Product` keeps its module identity, observed as each `label` printing its own module's payload. The forwarding plan dies with #654 (#1796). |
...
| `loaves/compiler/incan_ir/src/lower/mod.rs` | 33 | 5477 | 1358 | retire (retire 33) | 2/33 | 0 | - | #1561 | checker 28, parser 29, legacy_ir 33 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/lower/tests/union_member_identity.rs` | 2 | 101 | 101 | retire (retire 2) | 1/2 | 0 | - | #1561 | checker 2, parser 2, legacy_ir 2 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| `loaves/compiler/incan_ir/src/types.rs` | 53 | 1425 | 506 | retire | 0/53 | 0 | - | #1561 | legacy_ir 53 | the Rust-source backend's own lowering (`AstLowering`, `IrProgram`, `IrType`, Rust name spellings). Retire because its only consumers are `incan_emit` and the driver's `backend/ir` re-export, and `replacement/**` and `shadow/**` import nothing from it; flips to keep if #654 keeps the generated-project inspection path or the keep definition is read to include this lowering. Twins belong in Body IR (loaves/compiler/incan_frontend/src/body_ir/tests/). Reviewed at crate level. |
...
| Test | Disposition | Twin | Dies | Lanes | Notes |
|---|---|---|---|---|---|
| `shared_nominal_union_members_are_spelled_by_their_declaring_module_issue1796` | retire | `loaves/compiler/incan_test_support/fixtures/behavior/cli_dependencies/dependency_same_named_declarations_in_two_modules` | - | checker, parser, legacy_ir | Asserts the recorded shape on the legacy IR (a union member nominal two modules of the crate declare is spelled by its declaring module); retires with `incan_ir`. The behavior, a dependency whose two modules each declare `Product` and `Answer = Product \| int` building and serving both unions, is carried by the named fixture (#1796). |
