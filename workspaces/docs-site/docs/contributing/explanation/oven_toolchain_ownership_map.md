...
| Oven-shaped commands: `lock`, `init`/`new`, `env`/`version`, `workspace inspect`, vocab companion crate compilation | `src/cli/commands/{lock,init,lifecycle,workspace}.rs`, `loaves/compiler/incan_provider/src/vocab_extraction.rs` | Dependency/lock/registry lifecycle, manifest/workspace selection, environments/project mutation — all named explicitly under "oven" in RFC 118's own command-ownership table | 488/14, 315/16, 103/4, 44/1, 294/14 |
...
| Test discovery, module graph, reporting, types | `loaves/compiler/incan_driver/src/testing/{discovery,module_graph,types}.rs`, `src/cli/test_runner/reporter.rs` | Semantic source-level test discovery and result presentation, no Oven/project state | ~2,740 combined | `discovery` 828/33, `module_graph` 127/3, `reporter` 0/0 ⚠️, `types` 0/0 ⚠️ |
...
|---|---|---|
| LSP protocol implementation, diagnostics conversion, call-site generic-arg helpers | `src/lsp/{mod,diagnostics,call_site_type_args,backend}.rs` (with service preparation described above) | `diagnostics` 78/4, `call_site_type_args` 47/4, `backend` 7,027/48 (whole file) |
