...
| Manifest (`loaf.toml`) parsing & shape validation | `src/manifest.rs` | TOML deserialization + `deny_unknown_fields` + structural checks; sits upstream of the resolver rather than doing resolution | 2,505 | 538 / 28 |
...
| `commands/common.rs` | retired | Was a grab-bag straddling both sides. Decomposed along the boundary: module collection, the compilation session, typecheck orchestration, project discovery, Cargo policy, and rust-inspect workspace prep are `src/driver/*`; SDK-store identity, inventory discovery, SDK build, and project requirements are `src/provider/*`. Nothing under `src/driver` or `src/provider` imports `crate::cli`. | — |
