...
| `loaves/oven/` | Project and lock models (`oven_model`), receipts/stores/process containment (`oven_store`), native planning and execution (`oven_rustc`), receipt-bound native interop over it (`oven_interop`), and the RFC 125 contract crate (`oven_registry`, empty while the RFC is a draft); `oven_cargo_compat` is the last layout skeleton, filled when `legacy_cargo`'s edges into `loaf` and `rustc` are inverted. |
