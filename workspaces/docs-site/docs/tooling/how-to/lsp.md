...
This runs `cargo build -p incan-cli -p incan --features incan/lsp` and then links `~/.cargo/bin/incan` → `target/debug/incan` and `~/.cargo/bin/incan-lsp` → `target/debug/incan-lsp` when that binary exists. Set `INCAN_SKIP_CARGO_BIN_LINK=1` to skip linking, or rely on CI defaults (linking is off when `CI` is set).
...
When the file type-checks successfully, hover also previews checked public API metadata for public declarations, public partial callable presets, fields on public models/classes, checked public methods, and public enum variants. These previews use the same checked metadata extractor as `incan tools metadata api`, so they can include raw docstrings, checked signatures, partial target/preset provenance, field visibility and aliases/descriptions, enum backing values, derives, trait adoption, and safe const values. Private model and class fields are labeled private in their field preview without becoming accessible outside the declaring type. Hovering a canonical or supported-alias `@describe` decorator additionally shows its checked registry membership: registry, key, descriptor, and described subject. For value enums, enum and variant hover details show the backing type and variant raw value where available.
