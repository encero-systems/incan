...
A compiler built from a source checkout has no installed inventory. `incan check` publishes the checkout's component providers into the provider store on demand, under an identity derived from the standard-library sources and the compiler. `incan run`, `incan build`, `incan oven bake`, test collection and formatting never build providers, but they reuse the inventory `incan check` published for the same identity, so every command reads the same components, including the syntax a component's vocabulary adds, such as the checked C `binding` declaration from `std.interop`. Until that identity has been published, those commands see no inventory and fall back to the standard-library sources, where a component's vocabulary is not available.
