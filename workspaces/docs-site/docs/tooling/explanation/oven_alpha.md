...
<p>Compiler-suite replay reports live case completions and durations on stderr, with captured failure diagnostics and a 30-second heartbeat naming outstanding work. Its JSON report keeps process success separate from complete case evidence: missing records are unknown coverage, never zero-duration successes. Every partition retains case durations and full native transcripts; see the benchmark protocol below for artifact names and interpretation.</p>
