...
<p>The compiler-suite family is two things with two identities. The suite itself — the roots, the compiler that runs them, the Loaf compatibility they select — is keyed on the compiler's own sources, so every compiler edit misses it and is prepared again. The third-party foundation the suite links, the registry and vendored closure, is keyed on its own inputs only: the private foundation manifest, its lock, the checked-in patches, target, host, profile, the rustc identity and the Cargo version. A compiler edit therefore misses the suite but keeps the foundation key, and preparation selects the existing family by that key — from the compiler-suite store, or from a mirror through the same verifying import a mirrored Loaf uses — before it would ask Cargo to compile anything. Only a changed lock, manifest, patch, target, host, profile or toolchain derives a different key. The planning-only Cargo calls that read the unit graph and metadata still run on a suite miss; what the foundation key removes is the compilation.</p>
<p>Compiler-suite replay reports live case completions and durations on stderr, with captured failure diagnostics and a 30-second heartbeat naming outstanding work. Its JSON report keeps process success separate from complete case evidence: missing records are unknown coverage, never zero-duration successes. Phase progress continues through scratch cleanup and report publication. Every partition retains case durations, full native transcripts and an outer timing record covering archive creation and output cleanup; see the benchmark protocol below for artifact names and timing boundaries.</p>
...
<p>The developer store defaults to <code>$INCAN_HOME/oven/store/v2</code> (or <code>~/.incan/oven/store/v2</code>). Its everyday policy allows 12 GiB of aggregate physical allocation, with 12 GiB physical and 6 GiB logical per compatibility domain, enough for two large interop projects to share one home without evicting each other. The compiler-suite policy is 16 GiB aggregate physical, 6 GiB domain physical, and 4 GiB domain logical. Before an explicit bake compiles anything it reserves a 4 GiB staging floor, reclaiming least-recently-used inactive entries to reach it and saying which ones went; an active lease is never reclaimed, and an operator-supplied limit is never silently expanded.</p>
