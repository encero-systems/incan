...
Release packaging copies only the selected, relocatable provider payload into `share/incan/sdk` inside the installed toolchain. It excludes producer source, mutable Cargo targets, per-component lockfile copies, and components outside the distribution profile. Every archive has a `.profile.json` evidence sidecar recording the profile, component count, provider payload bytes, and complete archive bytes. Normal Oven consumers select those installed immutable artifacts through a receipt-compatible Loaf without copying a standard-library source tree or mutable provider cache into every project.

`oven.lock` format 2 records the expanded SDK component selection, exact provider identities and digests, public feature closure and activation reasons, dependency feature edges, and implementation-facet closure that can affect checking or generated output. Its embedded Cargo projection remains compatibility evidence for explicit publication, while normal Oven consumers derive receipt identity and select a sealed direct-`rustc` closure without materializing that projection. `--locked` rejects semantic drift or a compatibility manifest that would change the projection; `--frozen` adds the existing offline and no-mutation policy. Neither mode downloads a missing component.

Inside an RFC 077 workspace, the canonical root lock keeps one member-attributed semantic graph for every project and one workspace-wide backend dependency closure. Generated member roots retain their own Incan identity and target names while Oven receipts bind them to the compatible sealed closure selected for that root. `incan workspace inspect` exposes the locked member roots, feature closures, SDK profiles, component selections, and provider facts in both its human summary and JSON projection instead of flattening semantic selections into one ambient workspace-wide selection.
