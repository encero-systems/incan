...
`incan.lock` format 2 records the expanded SDK component selection, exact provider identities and digests, public feature closure and activation reasons, dependency feature edges, and implementation-facet closure that can affect checking or generated output. For a manifest project, its embedded Cargo payload and generated Cargo manifests use one canonical dependency closure across all declared scripts and tests. `--locked` rejects semantic drift or a Cargo manifest that would change that payload; `--frozen` adds the existing offline and no-mutation policy. Neither mode downloads a missing component.

Inside an RFC 077 workspace, the canonical root lock keeps one member-attributed semantic graph for every project and one workspace-wide backend dependency closure. Generated member projects retain their own Incan identity and target names while using the internal Cargo package identity and dependency closure that own the root lock. `incan workspace inspect` exposes the locked member roots, feature closures, SDK profiles, component selections, and provider facts in both its human summary and JSON projection instead of flattening semantic selections into one ambient workspace-wide selection.
