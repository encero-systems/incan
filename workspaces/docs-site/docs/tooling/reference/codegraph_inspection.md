...
These C records are language-contract facts only. They do not resolve an Oven requirement to a local compiler or SDK, read a lock receipt, fetch or stage an artifact, build a shim, prove that a library is present at runtime, or expose an editor navigation result. Use `incan inspect bindings` for the focused declaration review and `[interop.c]` with `incan lock` for declared and locked package inputs.
...
`examples/pro/codegraph_importer` is a runnable Incan-authored consumer of this JSONL contract. It accepts the v1 through v8 snapshot envelopes, counts the currently known record kinds, preserves unknown future kinds as opaque, and prints a deterministic JSON summary.
...
- `declaration`: source language, top-level declaration kind, name, visibility, type parameters, optional signature, optional compiler-owned `canonical_identity` and `stable_identity`, semantic and documentation digests when proven, source span, provenance, and degraded flag.
- `import`: source language, import kind, path, imported items, alias, visibility, and per-binding local spelling plus canonical and stable target identities where checked, source span, provenance, and degraded flag.
- `export`: public symbol exported by a declaration or public import, carrying the canonical and stable exported declaration identities when checked.
...
- `reference`: source-level name references inside declaration bodies, including identifier, field, `self`, surface-path, and `type` forms. A `type` reference is one the compiler proved from checked facts at an expression's span rather than from a syntax node, so an expression whose type names several declarations produces one record per named declaration at that same span; a consumer indexing references by span alone therefore sees more than one entry there, each with its own identity. A successfully resolved reference carries its structured `canonical_identity` and, when the required stable context is proven, `stable_identity`, unchanged through imports, aliases, and re-exports. Where the compiler proved the declaring owner, it also carries `canonical_owner`, the closest checked declaring owner, which is independent of whether export-local navigation linkage exists; the field is absent when no owner was proven. `target_id` additionally points at an emitted declaration record when that exact identity is present in the same export; otherwise it remains `null` without erasing the identity.
- `call`: source-level call expressions inside declaration bodies, including function, method, constructor, and surface-symbol calls. A uniquely selected callable carries its structured `canonical_identity`, `stable_identity` when the required stable context is proven, and `canonical_owner` where the declaring owner was proven; failed or ambiguous selection remains identityless. `target_id` is the optional same-export declaration link, not semantic authority.
- `containment`: parent-child relationship between file, module, declaration, import, C binding, reference, or call records.
- `diagnostic`: stable diagnostic code, severity, phase, compiler origin, message, primary span, notes, hints, optional structured expected/actual values, labeled related spans, canonical related declarations, and explain command.
...
Schema 8 may attach `stable_identity` to declaration, import-binding, export, reference, and call records. The field is omitted when the compiler cannot prove an identity; absence is not a key. When present, the object contains `namespace`, `origin`, `declaration_name`, `declaration_kind`, `location`, and an optional canonical `signature`. The signature distinguishes declarations that otherwise share the same module-level identity fields.

`location` is a tagged object with one of these wire shapes:

```json
{"kind":"module_level"}
{"kind":"nested","owner":{"namespace":"value","origin":{"kind":"module","path":["main"]},"declaration_name":"outer","declaration_kind":"function","location":{"kind":"module_level"}},"binding_ordinal":0}
```

`module_level` means the declaration is unique at module, package, crate, or registry level. `nested` recursively names the nearest edit-stable named owner. Its zero-based `binding_ordinal` distinguishes same-name, same-kind bindings in that owner; inserting or reordering another indistinguishable binding can therefore rekey later ordinals. Comments, unrelated names, and traversal-only scope-table indices do not participate in the identity.

Schema 7 used a `nested: bool` field inside `stable_identity`. Schema 8 replaces that field with the required tagged `location`, so a reader that interprets stable identities must branch on the header schema and decode the matching shape. The field itself remains optional on its containing record for compatibility with records where the producer could not prove an identity; that omission does not make a schema-7 stable identity valid under schema 8.
