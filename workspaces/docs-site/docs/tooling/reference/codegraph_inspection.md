...
`incan inspect codegraph` exports deterministic JSONL records for the source structure the compiler can see without asking a downstream tool to scrape `.incn` text. The 0.4 surface is deliberately small: it emits files, modules, top-level declarations, imports, public exports, containment edges, source spans, provenance, and diagnostics. This is the first durable RFC 106 codegraph slice under the broader RFC 102 semantic inspection surface.

Use it when an editor, CI job, architecture review tool, or agent needs basic Incan structure with compiler-owned provenance. Do not treat it as a graph database, full reference index, call graph, or stable generated-Rust ABI. The command reports source and syntax facts today, with diagnostics in tolerant mode; later releases can add richer checked relationships on the same record stream.
...
The first record is always a `header` record. It includes the schema version, compiler version, strict or tolerant mode, requested root path, optional package identity from `incan.toml`, and whether the export is degraded. Subsequent records describe source files, modules, declarations, imports, exports, containment relationships, and diagnostics. Consumers should treat unknown future record kinds as opaque records rather than failing closed.
...
- `header`: export schema, compiler version, mode, root, package identity, and degraded flag.
- `file`: source file path, byte size, provenance, and degraded flag.
- `module`: module path, parent file id, source span, provenance, and degraded flag.
- `declaration`: top-level declaration kind, name, visibility, type parameters, optional signature, source span, provenance, and degraded flag.
- `import`: import kind, path, imported items, alias, visibility, source span, provenance, and degraded flag.
- `export`: public symbol exported by a declaration or public import.
- `containment`: parent-child relationship between file, module, declaration, or import records.
