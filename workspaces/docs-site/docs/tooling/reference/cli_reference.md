...
`bake` is the project-facing preparation command. It records debug and release receipts, reports `toolchain_loaf` when a profile selects that immutable full-stdlib Loaf directly, and reports `reused` on an exact warm project Loaf. A project that needs a closure outside the installed standard-library envelope may explicitly invoke the bounded compatibility baker and publish a receipt-bound `<identity>.loaf/loaf.json` in the policy-bounded local store. It never compiles a final library, and normal build/test/run never invoke Cargo or use a fallback.
