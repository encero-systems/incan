...
- Builds the generated Rust project and prints the binary path. Generated source and the published final binary stay under the project `target/incan/` tree, while rebuildable Cargo dependency and intermediate output uses the managed cache described below.
