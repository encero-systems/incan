...
- `INCAN-T0112`: A decorator chain of a `self` method whose shapes name the receiver is one the compiler cannot pass the receiver through: the decorator is imported or reached through a value or a method, a shape that names the receiver is written through a type alias, a `return` gives something other than the accepted callable or a function of the module named directly, a function of the chain is `pub`, or a function of the chain is used outside it; see [Decorators](../../language/reference/language.md#decorators).
