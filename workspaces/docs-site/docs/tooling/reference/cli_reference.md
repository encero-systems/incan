...
- `INCAN-T0119`: A `for` loop takes the items out of a list, because its body awaits, returns, assigns or passes each item and the item can be neither copied nor cloned (a `JoinHandle[T]`, for one), and the list is read inside or after the loop, or an enclosing loop runs the `for` loop again over a list built outside it; see [`JoinHandle[T]`](../../language/reference/stdlib/async.md#joinhandlet).
