...
- `INCAN-T0119`: A `for` loop takes the task handles out of a list, because its body awaits, returns, yields, assigns, passes on, breaks with, places in a new collection, or iterates in a nested loop that takes its items, each `JoinHandle[T]` item, and the list is then read inside or after the loop, a closure that captured the list before the loop is used inside or after it, or an enclosing loop runs the `for` loop again over a list built outside it; see [`JoinHandle[T]`](../../language/reference/stdlib/async.md#joinhandlet).
