...
impl FromStr for Cfg {
    ...
    fn from_str(s: &str) -> Result<Cfg, Self::Err> {
        ...
        if let Some(rest) = p.rest() {
            return Err(ParseError::new(p.t.orig, UnterminatedExpression(rest.to_string())));
        }
        Ok(e)
    }
}
...
impl<'a> Parser<'a> {
    ...
    fn expr(&mut self) -> Result<CfgExpr, ParseError> {
        match self.peek() {
            Some(Ok(Token::Ident(false, op @ "all"))) | Some(Ok(Token::Ident(false, op @ "any"))) => {
                self.t.next();
                let mut e = Vec::new();
    ...
    fn eat(&mut self, token: &Token<'a>) -> Result<(), ParseError> {
        match self.t.next() {
            ...
            Some(Err(e)) => Err(e),
            None => Err(ParseError::new(self.t.orig, IncompleteExpr(token.classify()))),
        }
    }
                    ...
                    let (start, raw) = if ch == 'r' {
                        if let Some(&(_pos, '#')) = self.s.peek() {
                            ...
                            if let Some((start, ch)) = self.s.next() {
                                ...
                                return Some(Err(ParseError::new(self.orig, IncompleteExpr("identifier"))));
                            }
                        } else {
