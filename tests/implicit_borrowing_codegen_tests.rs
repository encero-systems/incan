...
const OVEN_COMPILER_SUITE_CAPABILITY_ENV: &str = "INCAN_OVEN_COMPILER_SUITE_CAPABILITY";

#[derive(Deserialize)]
struct CompilerSuiteCapability {
    rustc: std::path::PathBuf,
    dependency_search_paths: Vec<std::path::PathBuf>,
    externs: std::collections::BTreeMap<String, std::path::PathBuf>,
}

fn compiler_suite_capability() -> Result<Option<CompilerSuiteCapability>, Box<dyn std::error::Error>> {
    let Some(payload) = std::env::var_os(OVEN_COMPILER_SUITE_CAPABILITY_ENV).filter(|value| !value.is_empty()) else {
        return Ok(None);
    };
    let capability = serde_json::from_str::<CompilerSuiteCapability>(&payload.to_string_lossy())?;
    Ok(Some(capability))
}
...
"#,
    ...
    let capability = compiler_suite_capability()?;
