...
| `build_pub_consumer_imports_public_alias_of_imported_item_issue617`, `build_lib_materializes_facade_decorator_metadata_projection_issue695`, in `tests/cli_provider_boundary_tests.rs` and `tests/cli_language_regression_tests.rs`, and `tools_metadata_api_reports_public_import_aliases` in `tests/cli_surface_tests.rs` | Current public-alias projection and checked API metadata derived from source. | A metadata assertion over the versioned package/caller-facet contract, including export identity, supported type projection, and explicit unsupported states rather than generated-Rust names or private compiler structures. |
...
| `compiled_provider_preserves_shared_rust_interop_contracts_issues834_835_961` in `tests/integration_tests.rs` and the Rust metadata/callback coverage in `tests/cli_rust_interop_tests.rs` | Current provider and compiler interop planning across a legacy compiled path. | A caller-facet case that proves supported Rust-host call shapes and refuses non-representable exports or boundary types before publishing an invalid consumer artifact. |
