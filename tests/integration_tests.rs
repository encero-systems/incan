...
"#,
    ...
    fn test_std_datetime_surface_runs_with_std_time_runtime_boundary() -> Result<(), Box<dyn std::error::Error>> {
        let runtime_source =
            std::fs::read_to_string(repo_root().join("crates/incan_stdlib/stdlib/datetime/runtime.incn"))?;
        let mut civil_sources = Vec::new();
        civil_sources.push(std::fs::read_to_string(
            repo_root().join("crates/incan_stdlib/stdlib/datetime/civil.incn"),
        )?);
        for entry in std::fs::read_dir(repo_root().join("crates/incan_stdlib/stdlib/datetime/civil"))? {
            let entry = entry?;
...
fn std_toml_manifest_and_lock_roundtrip_through_compiled_sdk() -> Result<(), Box<dyn std::error::Error>> {
    for (source, expected_stdout, uses_facade) in [
        ...
        (
            include_str!("fixtures/valid/std_toml_module_import.incn").to_owned(),
