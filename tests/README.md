...
Integration roots live in the package they exercise, under that package's `tests/` directory; unit tests live inline in the source files they test (`#[cfg(test)]` modules). No root lives in this directory any more: the rings hold theirs, the command-line roots sit with the `incan` binary in `loaves/toolchain/incan-cli`, and the language-server root with `incan-lsp`. What remains here is `fixtures/`.
...
| `loaves/toolchain/incan-lsp/tests/` | RFC 081 embedded-fragment conformance, parser to emission, both formatter modes and semantic highlighting | `cargo test -p incan-lsp --test rfc081_embedded_conformance` |
