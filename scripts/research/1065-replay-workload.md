...
2. Obtain each compiled root's exact libtest inventory and preserve it beside the report. Start with current two threads and warm receipt-compatible providers. Measure cold provider preparation separately so startup is not mislabeled as test execution.
