...
def twin_coverage(file: ScannedFile, entry: dict) -> tuple[int, int]:
    """`(with_twin, retire_total)` for the retire-class tests of a file."""
    retire = [key for key in file.keys if effective_disposition(entry, key) == "retire"]
    with_twin = sum(1 for key in retire if effective_twin(entry, key))
    return with_twin, len(retire)
...
def file_row(file: ScannedFile, entry: dict | None) -> str:
    ...
    with_twin, retire_total = twin_coverage(file, entry)
    twins = f"{with_twin}/{retire_total}" if retire_total else "-"
    owner = f"#{entry['owner']}" if entry.get("owner") else "-"
    return (
        f"| `{file.path}` | {len(file.tests)} | {file.lines} | {file.test_lines} | {disposition_cell} | {twins} | "
        f"{split_cell(entry)} | {owner} | {signals_cell(file)} | {md_cell(entry.get('notes', ''))} |"
    )
...
def override_rows(file: ScannedFile, entry: dict) -> list[str]:
    ...
    overrides = entry.get("tests", {})
    rows = [
        f"| Test | Disposition | Twin | Lanes | Notes |",
        f"|---|---|---|---|---|",
    ]
    for test in file.tests:
        ...
        rows.append(
            ...
            f"{code(effective_twin(entry, test.qualified))} | {lanes} | {md_cell(override.get('notes', ''))} |"
        )
    return rows
...
def render(corpus: Corpus, dispositions: dict) -> str:
    ...
    # ---- Retire twin coverage and split flags across the corpus ----
    retire_with_twin = 0
    retire_total = 0
    zero_twin_files = []
    split_files = []
    ...
    for file in corpus.files:
        ...
        if entry is None:
            continue
        with_twin, retire_count = twin_coverage(file, entry)
        retire_with_twin += with_twin
        retire_total += retire_count
        if retire_count and with_twin == 0:
            zero_twin_files.append(file.path)
    ...
    out.append("")
    out.append(f"- Retire-class tests with a named twin: {retire_with_twin}/{retire_total}.")
    out.append(f"- Retire-class files with no twin at all: {len(zero_twin_files)} (the `Twins` column reads `0/n`).")
    out.append(
