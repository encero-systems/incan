...
pub(crate) struct LoafEnvelopeExpectation<'a> {
    ...
    pub(crate) generation_identity: &'a str,
    /// Release-family compatibility evidence the local bake computed.
    pub(crate) evidence: &'a BTreeMap<String, String>,
    /// Checked member list, in envelope order.
    pub(crate) members: &'a [LoafMemberExpectation],
}

/// The checked specification one envelope member must carry.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct LoafMemberExpectation {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct MirroredLoafEnvelope {
...
pub(crate) fn import_loaf_envelope_from_mirrors(
    output: &Path,
    scratch: &Path,
...
mod tests {
    ...
    use super::*;
    use crate::oven::loaf::{OVEN_LOAF_ENVELOPE_MANIFEST_SCHEMA_VERSION, OVEN_LOAF_SCHEMA_VERSION, OvenLoafAccounting};
