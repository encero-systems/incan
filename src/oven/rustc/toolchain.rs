...
pub(crate) fn incan_owned_rustup_home() -> Option<PathBuf> {
...
pub(crate) fn incan_owned_rustup_home_in(
...
pub(crate) const INCAN_OWNED_CHANNEL_POINTER: &str = "incan-channel.txt";
...
pub(crate) fn incan_owned_tool(rust_root: &Path, tool: &str) -> Option<PathBuf> {
...
pub(crate) fn rustc_probe_memo() -> &'static Mutex<HashMap<RustcFileStamp, RustcProbe>> {
...
pub(crate) type RustcFileStamp = (PathBuf, u64, Option<std::time::SystemTime>);

/// Probe one regular Rust compiler with `-vV`, once per observed compiler file per process.
pub(crate) fn rustc_probe(rustc: &Path) -> Result<RustcProbe, OvenRustcError> {
...
pub(crate) fn rustc_sysroot(rustc: &Path) -> Result<PathBuf, OvenRustcError> {
    let rustc = verified_regular_file(rustc, "rustc")?;
    let mut command = Command::new(&rustc);
...
pub(crate) fn rustc_dynamic_library_environment_with_caller_owned_paths(
    rustc: &Path,
...
pub(crate) fn expected_artifacts(
    manifest: &OvenRustcArtifactManifest,
