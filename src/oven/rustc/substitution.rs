...
pub(crate) const BASE_UNIT_MANIFEST_SCHEMA: &str = "incan-oven-base-units/1";

/// Environment variable naming the base-unit manifest a wrapped Cargo run may substitute from.
pub(crate) const BASE_UNITS_ENV: &str = "INCAN_OVEN_BASE_UNITS";
...
pub(crate) struct RustcUnitRequest {
    ...
    pub(crate) externs: BTreeMap<String, Option<PathBuf>>,
}

/// Why an invocation could not be read as a substitutable unit request.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum RustcUnitRequestError {
...
impl RustcUnitRequest {
    ...
    pub(crate) fn parse<A, E>(args: &[A], env: E) -> Result<Self, RustcUnitRequestError>
    where
        A: AsRef<OsStr>,
    ...
    pub(crate) fn is_substitutable_kind(&self) -> bool {
    ...
    pub(crate) fn expected_artifact_name(&self, crate_type: &str) -> Option<String> {
        match crate_type {
...
pub(crate) struct BaseUnitManifest {
    ...
    pub(crate) units: Vec<BaseUnit>,
}

/// Why a request was not answered from the base.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum SubstitutionMiss {
