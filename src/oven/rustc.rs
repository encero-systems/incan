...
pub(crate) use compiled_unit::*;
pub use diagnostics::*;
pub(crate) use inspection::*;
pub(crate) use registry_leaf::*;
#[allow(
    unused_imports,
...
pub(crate) struct OvenTrustedRustcSearchRoot<'a> {
...
impl OvenRustcArtifactManifest {
    ...
    pub(crate) fn capture_source_search_closure(
        &self,
        paths: &[String],
    ...
    pub(crate) fn source_search_closure(&self, key: &str) -> Result<OvenRustcSourceSearchClosure, OvenRustcError> {
        self.validate_shape(&self.intent)?;
    ...
    pub(crate) fn materialize_trusted_store_composed_with_search_roots(
        &self,
        roots: &[OvenTrustedRustcArtifactRoot<'_>],
    ...
    fn validate_source_search_roles(&self) -> Result<(), OvenRustcError> {
    ...
    fn source_search_roles_at_root(
        &self,
        root: &Path,
    ) -> Result<Option<OvenRustcSourcePathProjection>, OvenRustcError> {
        let declared = expected_artifacts(self)?;
...
mod tests {
    ...
    fn jec_frozen_environment_contains_only_admitted_plan_values() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let frozen = super::direct_compiler::freeze_direct_rustc_environment(inherited, &compile_environment)
    ...
    fn jec_ignores_unrepresentable_ambient_names_and_clears_ambient_execution() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        assert_eq!(
            super::direct_compiler::freeze_direct_rustc_environment(inherited, &BTreeMap::new()),
            Some(BTreeMap::new())
        );
        let mut command = Command::new("/usr/bin/env");
        command.env("AMBIENT_VALUE", "ambient");
        super::direct_compiler::apply_direct_rustc_execution_environment(&mut command, Some(&BTreeMap::new()))?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let frozen_environment = super::direct_compiler::freeze_direct_rustc_environment(
        ...
        super::direct_compiler::apply_direct_rustc_execution_environment(&mut cacheable, Some(&frozen_environment))?;
        ...
        super::direct_compiler::apply_direct_rustc_execution_environment(&mut uncacheable, Some(&frozen_environment))?;
    ...
    fn jec_dep_info_keeps_only_logical_files_and_secret_digests() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let observed = super::direct_compiler::parse_rustc_dep_info(bytes, &environment)?;
        assert_eq!(
            observed.files,
            [
        ...
        assert!(super::direct_compiler::parse_rustc_dep_info(mismatched, &environment).is_err());
        Ok(())
    }
...
"#,
        ...
        let super::direct_compiler::OvenDirectRustcCompilerRetention::Unavailable { reason } = unavailable else {
    ...
    fn jec_bound_rlib_is_byte_identical_after_source_relocation() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let compiler = super::direct_compiler::OvenDirectRustcCompilerEvidence {
        ...
        let bindings = super::direct_compiler::OvenDirectRustcJecBindings {
        ...
        for name in ["first", "second"] {
            ...
            let prepared = super::direct_compiler::OvenPreparedDirectRustcLibrary {
                rustc: rustc.clone(),
                ...
                frozen_environment: Some(
                    super::direct_compiler::freeze_direct_rustc_environment(std::env::vars_os(), &BTreeMap::new())
                        .ok_or("test environment was not representable")?,
                ),
            };
            let compiled = prepared.bind(&output, &bindings)?.compile()?;
            outputs.push(fs::read(&compiled.bake.output)?);
            let super::direct_compiler::OvenRustcDepInfoOutcome::Observed(observation) = compiled.observation else {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        for fixture in [&debug, &release] {
            let view = super::native_input::OvenNativeInputView::from_store_payload(&fixture.owner)?;
    ...
    fn native_input_view_refuses_foreign_foundation_role_and_alias_collision() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        let view = super::native_input::OvenNativeInputView::from_store_payload(&first.owner)?;
        let other = super::native_input::OvenNativeInputView::from_store_payload(&second.owner)?;
        let foreign = other
            .registry_candidates("generated-root")?
            .pop()
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(super::native_input::OvenNativeInputView::from_store_payload(&first.owner).is_err());
        ...
        assert!(super::native_input::OvenNativeInputView::from_store_payload(&first.owner).is_ok());
        ...
        assert!(
            super::native_input::OvenNativeInputView::from_store_payload(&first.owner).is_err(),
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        legacy.schema_version = super::OVEN_RUSTC_LEGACY_ARTIFACT_MANIFEST_SCHEMA_VERSION - 1;
    ...
    fn source_search_fixture(receipt: &crate::oven::OvenReceipt) -> Result<OvenRustcArtifactManifest, OvenRustcError> {
        let mut artifacts = empty_manifest(receipt);
    ...
    #[test]
    fn source_search_roles_validate_wire_evidence_and_preserve_legacy_projection()
    -> Result<(), Box<dyn std::error::Error>> {
        let project = tempfile::tempdir()?;
        let receipt = intent(project.path())?;
        ...
        for invalid_paths in [vec!["../host"], vec!["unlisted"], vec!["target", "target"]] {
            ...
            invalid.entrypoint_dependency_search_paths.insert(
                "generated-root".to_string(),
                invalid.capture_source_search_closure(
                    &invalid_paths.into_iter().map(str::to_string).collect::<Vec<_>>(),
                )?,
            );
        ...
        legacy.schema_version = super::OVEN_RUSTC_LEGACY_ARTIFACT_MANIFEST_SCHEMA_VERSION;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let projected = super::trusted_artifact_plan_for_source_evidence(&materialized, &composed, "generated-root")?;
        ...
        invalid
            ...
            .legacy_projections[0]
            .dependency_search_paths
            .push(super::OvenRustcSourceSearchDirectory {
    ...
    fn source_search_roles_bind_separate_roots_and_keep_caller_paths() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let fragments = roots
            ...
            .zip(&members)
            .zip(&directories)
            .map(|((root, members), paths)| super::OvenTrustedRustcArtifactRoot {
                artifact_root: root,
        ...
        let projected = super::trusted_artifact_plan_for_source_evidence(&plan, &artifacts, "generated-root")?;
        ...
        let projected_twice =
            super::trusted_artifact_plan_for_source_evidence(&projected, &artifacts, "generated-root")?;
        ...
        let repeated = super::trusted_artifact_plan_for_source_evidence(&with_provider, &artifacts, "generated-root")?;
        ...
        assert!(super::trusted_artifact_plan_for_source_evidence(&plan, &artifacts, "generated-root").is_err());
        Ok(())
    }
    ...
    fn source_search_roles_keep_same_spelled_directories_with_their_owners() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for _ in 0..2 {
            ...
            let roots = [
                super::OvenTrustedRustcArtifactRoot {
                    artifact_root: first.path(),
            ...
            let projected = super::trusted_artifact_plan_for_source_evidence(&plan, &combined, "generated-root")?;
            assert!(projected.dependency_search_paths.contains(&second_root.join("host")));
            assert!(
            ...
            assert_eq!(
                projected,
                super::trusted_artifact_plan_for_source_evidence(&projected, &combined, "generated-root")?
            );
            ...
            assert!(
                super::trusted_artifact_plan_for_source_evidence(&plan, &changed_contract, "generated-root").is_err()
            );
            ...
            let repeated = super::trusted_artifact_plan_for_source_evidence(&caller, &combined, "generated-root")?;
    ...
    fn source_search_roles_follow_exact_extension_partition_members() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let plan =
            artifacts.materialize_trusted_store_composed_with_search_roots(&roots, &candidates, &receipt.intent)?;
        let projected = super::trusted_artifact_plan_for_source_evidence(&plan, &artifacts, "generated-root")?;
        assert_eq!(
    ...
    #[cfg(unix)]
    #[test]
    fn source_search_roles_keep_host_macro_metadata_without_a_direct_macro_grant()
    -> Result<(), Box<dyn std::error::Error>> {
        let project = tempfile::tempdir()?;
        let output = tempfile::tempdir()?;
        let artifact_root = tempfile::tempdir()?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let projected =
            super::trusted_artifact_plan_for_source(&plan, &artifacts, &selected_artifacts, "generated-root")?;
