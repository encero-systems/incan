...
pub(crate) fn cargo_registry_checksums(
    cargo_lock: &[u8],
...
pub(crate) fn stage_registry_source(
    staging: &Path,
    package: &str,
    version: &str,
    registry: &str,
...
pub(crate) fn stage_registry_source_directory(
    staging: &Path,
    package: &str,
    version: &str,
    registry: &str,
...
pub(crate) fn copy_registry_source_tree(
    source_root: &Path,
...
pub(crate) fn publisher_registry_source_catalog(
...
pub(crate) struct PublisherRegistryLeafCatalogRequest<'a> {
    pub(crate) outputs: &'a [CargoInvocationOutput],
    pub(crate) metadata: &'a CargoMetadata,
    pub(crate) cargo_lock: &'a [u8],
    pub(crate) staging: &'a Path,
    pub(crate) intent: &'a OvenBuildIntent,
    pub(crate) rustc_host: &'a str,
    pub(crate) externs: &'a [OvenRustcArtifactExtern],
    pub(crate) supporting_artifacts: &'a [OvenRustcSupportingArtifact],
    pub(crate) inspection_packages: Option<&'a [OvenLegacyCargoInspectionPackage]>,
}

/// Build the immutable registry-leaf catalog from one explicit publisher result.
pub(crate) fn publisher_registry_leaf_catalog(
