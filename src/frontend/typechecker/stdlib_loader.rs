...
//! ## Limitations
//!
//! - Only top-level `def` declarations are extracted (not methods on classes/models).
...
pub(crate) struct StdlibAstCache {
    /// Map from module path (dot-joined) to extracted function signatures.
    cache: HashMap<String, Vec<(String, FunctionInfo)>>,
}
...
impl StdlibAstCache {
    ...
    pub fn lookup_function(&mut self, module_path: &[String], function_name: &str) -> Option<FunctionInfo> {
        ...
        if !self.cache.contains_key(&key) {
            if let Some(fns) = load_stdlib_module_functions(module_path) {
                self.cache.insert(key.clone(), fns);
            } else {
...
/// Load and parse a stdlib `.incn` file, extracting function signatures.
///
/// Returns `None` if the file cannot be found or parsed.
fn load_stdlib_module_functions(module_path: &[String]) -> Option<Vec<(String, FunctionInfo)>> {
    ...
    Some(extract_function_signatures(&program))
}
...
/// 2. `$CARGO_MANIFEST_DIR/<relative>` (development builds)
/// 3. `./<relative>` relative to CWD
fn find_stdlib_file(relative: &str) -> Option<PathBuf> {
    ...
    // 2. Development builds (CARGO_MANIFEST_DIR points to repo root)
...
mod tests {
    ...
    fn test_load_testing_module() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let fns = fns.ok_or("failed to load stdlib/testing.incn")?;
        ...
        assert_eq!(assert_eq_info.params.len(), 2);
