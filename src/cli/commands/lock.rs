...
fn hash_lock_dependency_preheat_file(hasher: &mut Sha256, base: &Path, path: &Path) -> io::Result<()> {
    ...
    let contents = fs::read(path).map_err(|err| {
        io::Error::new(
            err.kind(),
            format!("failed to read preheat fingerprint input {}: {err}", path.display()),
        )
    })?;
...
/// Add all generated Rust source files to the dependency-preheat fingerprint in a stable order.
fn hash_lock_dependency_preheat_sources(hasher: &mut Sha256, base: &Path) -> io::Result<()> {
    let src_dir = base.join("src");
    let mut paths = Vec::new();
    collect_generated_rust_source_files(&src_dir, &mut paths)?;
    if paths.is_empty() {
        return Err(io::Error::new(
            io::ErrorKind::NotFound,
            format!("no generated Rust source files found under {}", src_dir.display()),
        ));
    }
    paths.sort();
    for path in paths {
        hash_lock_dependency_preheat_file(hasher, base, &path)?;
    }
    Ok(())
}

/// Recursively collect generated Rust files from one preheat workspace source tree.
fn collect_generated_rust_source_files(dir: &Path, output: &mut Vec<PathBuf>) -> io::Result<()> {
    for entry in fs::read_dir(dir).map_err(|err| {
        io::Error::new(
            err.kind(),
            format!(
                "failed to read generated Rust source directory {}: {err}",
                dir.display()
            ),
        )
    })? {
        ...
        let path = entry.path();
        if path.is_dir() {
            collect_generated_rust_source_files(&path, output)?;
...
fn compute_dependency_preheat_fingerprint(
    ...
    hash_lock_dependency_preheat_sources(&mut hasher, lock_dir)?;
...
mod tests {
    ...
    fn library_dependency_preheat_fingerprint_uses_separate_profile_domain() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_ne!(
            ...
            "generated-library preheat fingerprint must include nested generated Rust source files"
        );
