...
pub fn init_project(path: &Path, options: InitOptions<'_>) -> CliResult<ExitCode> {
    ...
    let manifest = WritableManifest {
        project: Some(ProjectSection {
            ...
            requires_incan: Some(default_requires_incan_constraint()),
...
"#,
    ...
    let test_content = format!(
        r#"from main import greeting

from std.testing import assert_eq

def test_greeting() -> None:
    assert_eq(greeting(), "Hello from {name}!")
"#,
        name = metadata.name
    );
    ...
    println!("Release it: incan build --release");
...
fn default_requires_incan_constraint() -> String {
    match semver::Version::parse(crate::version::INCAN_VERSION) {
        Ok(version) => {
        ...
        Err(_) => format!(">={}", crate::version::INCAN_VERSION),
    }
}
