...
use crate::frontend::api_metadata::{ApiDeclaration, ApiFunction, ApiPartial, CheckedApiMetadataPackage};
...
use crate::driver::metadata_packages::{
...
pub fn write_feature_inventory_reference(path: &Path) -> CliResult<()> {
    let source = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("loaves/stdlib/core/src/features.incn");
...
"#,
    ...
    fn write_test_incan_lock(project_root: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        IncanLock::new(crate::version::INCAN_VERSION, fingerprint, features, cargo_lock_payload)
