...
impl<'a> IrCodegen<'a> {
    ...
    pub fn try_generate_module(&mut self, module_name: &str, program: &Program) -> Result<String, GenerationError> {
        ...
        let type_info = {
            ...
            self.configure_typechecker(&mut tc);
            let typecheck_deps =
                Self::imported_dependency_modules_for_program(program, &dependency_modules, Some(module_name));
            let result = tc.check_with_imports_allow_private(program, &typecheck_deps);
            let type_info = match result {
                Ok(()) => tc.type_info().clone(),
                Err(errors) => return Err(Self::typecheck_errors_for_module(module_name, errors)),
            };
            self.capture_typechecker_stdlib_cache(&tc);
            type_info
        };
        ...
        for (dep_name, dep_ast, dep_path_segments) in dependency_modules.clone() {
            ...
            let dep_type_info = {
                ...
                self.configure_typechecker(&mut tc);
                let typecheck_deps =
                    Self::imported_dependency_modules_for_program(dep_ast, &dependency_modules, Some(dep_key.as_str()));
                let result = tc.check_with_imports_allow_private(dep_ast, &typecheck_deps);
                let type_info = match result {
                    Ok(()) => tc.type_info().clone(),
                    Err(errors) => return Err(Self::typecheck_errors_for_module(&dep_key, errors)),
                };
                self.capture_typechecker_stdlib_cache(&tc);
                type_info
            };
