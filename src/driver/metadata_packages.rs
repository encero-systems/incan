...
pub(crate) fn collect_api_metadata_package(path: &Path) -> CliResult<CheckedApiMetadataPackage> {
    ...
    for (idx, module) in modules.iter().enumerate() {
        ...
        crate::driver::modules::register_module_path_segments(&mut checker, &modules);
...
pub(crate) fn resolve_metadata_entry_path(path: &Path) -> CliResult<PathBuf> {
