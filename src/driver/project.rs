...
pub(crate) fn resolve_stdlib_module_source_path(module_path: &[String]) -> CliResult<PathBuf> {
    ...
    if let Some(stdlib_dir) = crate::toolchain_layout::find_stdlib_source_dir() {
...
pub(crate) fn read_source_for_diagnostics(file_path: &str) -> Result<String, CliDiagnosticFailure> {
