...
use crate::driver::build::backend_selection::{default_backend_receipt_path, write_backend_receipt};
use crate::driver::build::library_exports::{resolve_library_project_root, validate_library_entrypoint};
use crate::driver::build::library_outputs::library_publication_receipts;
use crate::driver::build::output_paths::{
...
use crate::driver::build::plan_authority::explicit_bake_profiles;
use crate::driver::build::source_authority::{
...
use crate::driver::lock::resolution::validate_oven_lock_policy;
use crate::driver::oven_store::open_default_oven_store;
...
use crate::oven::store::OvenStore;
...
pub(crate) fn completed_output_default_backend_receipt(
    output: &OvenStoredProjectOutput,
) -> Option<BackendExecutionReceipt> {
...
pub(crate) fn materialize_completed_executable_output(
    project_root: &Path,
    output: &OvenStoredProjectOutput,
...
pub(crate) fn completed_executable_output_report(
    project_root: &Path,
    output: &OvenStoredProjectOutput,
    backend_receipt: &BackendExecutionReceipt,
    total_start: Instant,
...
pub(crate) fn completed_library_output_report(
    project_root: &Path,
    outputs: &[OvenStoredProjectOutput],
    total_start: Instant,
) -> CliResult<crate::driver::build_report::BuildReport> {
    ...
    let report = BuildReportDraft {
        ...
        artifacts,
        dependencies: crate::driver::build_report::BuildDependencyReport {
            rust: Vec::new(),
            rust_dev: Vec::new(),
            incan: Vec::new(),
            stdlib_features: Vec::new(),
        },
        semantic: crate::driver::build_report::BuildSemanticReport {
            sdk: None,
            packages: Vec::new(),
        ...
        interop: crate::driver::build_report::BuildInteropReport {
...
pub(crate) fn caller_project_output_path(project_root: &Path, relative_path: &str) -> CliResult<PathBuf> {
...
pub(crate) fn digest_project_output_projection_file(path: &Path) -> CliResult<(u64, String)> {
...
pub(crate) fn verify_stored_project_output_native(output: &OvenStoredProjectOutput) -> CliResult<()> {
    let native = output
        .payload
        .files
        .iter()
...
pub(crate) fn project_output_projection_is_current(
    project_root: &Path,
    output: &OvenStoredProjectOutput,
) -> CliResult<bool> {
...
pub(crate) fn select_default_project_output(
    file_path: &str,
...
pub(crate) fn select_current_sealed_project_output(
    store: &OvenStore,
    project_root: &Path,
    entrypoint: &Path,
    target: OvenBakeProjectTarget,
...
pub(crate) fn warn_for_completed_output_lock_fingerprint_drift<'a>(
    project_root: &Path,
...
pub(crate) fn select_default_library_project_outputs(
...
pub(crate) fn select_coherent_library_outputs(
...
pub(crate) fn materialize_project_output(project_root: &Path, output: &OvenStoredProjectOutput) -> CliResult<()> {
...
pub(crate) fn materialize_completed_library_outputs<T>(
    project_root: &Path,
    outputs: &[OvenStoredProjectOutput],
...
mod tests {
    use super::*;
    use crate::driver::build::test_support::fixture_project_output_publication;
    use std::fs;
    use std::path::Path;
    use std::time::Instant;
    ...
    use crate::driver::build::backend_selection::default_backend_receipt_path;
    use crate::driver::build::output_selection::{
    ...
    use crate::driver::build_report::BUILD_REPORT_SCHEMA_VERSION;
    use crate::driver::error::{CliError, CliResult};
    ...
    use crate::oven::store::{OvenArtifactKind, OvenStore};
