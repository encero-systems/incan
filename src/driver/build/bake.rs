//! Baking a project's targets: the executable and library bakes, their classification, and the explicit
//! `incan oven bake --project` entry.
...
use crate::driver::build::caller_owned::has_caller_owned_project_libraries;
use crate::driver::build::library_exports::resolve_library_project_root;
use crate::driver::build::library_outputs::{
    library_publication_receipts, packaged_library_loaf_store_root, write_library_manifest_artifacts,
};
use crate::driver::build::library_project::prepare_library_project;
...
use crate::driver::build::output_selection::project_output_report_snapshot;
use crate::driver::build::oven_project::{prepare_oven_project, remove_completed_generated_cargo_lock};
use crate::driver::build::package_loafs::{export_selected_package_loaf, write_packaged_library_loaf_manifest};
...
use crate::driver::build::publication::{
...
use crate::driver::build_report::artifact_report;
use crate::driver::cargo_policy::{CargoPolicy, enforce_project_toolchain_constraint};
use crate::driver::error::{CliError, CliResult, oven_rustc_error};
use crate::driver::lock::PublishedOvenProjectLock;
use crate::driver::lock::resolution::publish_oven_project_lock;
...
use crate::oven::store::OvenArtifactKind;
use crate::oven::write_receipt;
...
pub(crate) fn bake_oven_project(
...
pub(crate) fn bake_oven_library(
...
pub(crate) fn select_default_executable_project_output(
    file_path: &str,
...
pub(crate) fn discover_oven_executable_entrypoints(manifest: &ProjectManifest) -> CliResult<BTreeMap<String, PathBuf>> {
...
pub(crate) fn discover_oven_bake_project_targets(
    project_root: &Path,
) -> CliResult<Vec<(OvenBakeProjectTarget, PathBuf)>> {
...
mod tests {
    ...
    use crate::driver::build::source_authority::project_bake_receipt_path;
    use crate::driver::build::{
