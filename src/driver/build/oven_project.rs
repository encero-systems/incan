...
use crate::driver::build::backend_selection::{finalize_backend_receipt, select_and_resolve_backend};
use crate::driver::build::bake::discover_oven_bake_project_targets;
use crate::driver::build::caller_owned::{append_oven_interop_execution_build_inputs, oven_caller_owned_libraries};
use crate::driver::build::library_exports::resolve_library_project_root;
use crate::driver::build::output_selection::load_current_project_registry_source_authorities;
use crate::driver::build::plan_authority::{
...
use crate::driver::build::plan_selection::{
...
use crate::driver::build::provider_compilation::{
...
use crate::driver::build::rust_extern::multi_file_output_identity;
use crate::driver::build::source_authority::{interop_bootstrap_receipt_path, prepared_oven_receipt_path};
...
use crate::driver::build_unit::oven_build_unit_inputs;
use crate::driver::cargo_policy::{CargoPolicy, enforce_project_toolchain_constraint};
use crate::driver::error::{CliError, CliResult, oven_plan_error, oven_rustc_error};
#[cfg(feature = "rust_inspect")]
use crate::driver::lock::OvenRustInspectSourceAuthorityRequest;
#[cfg(feature = "rust_inspect")]
use crate::driver::lock::RustInspectWorkspaceRequest;
#[cfg(feature = "rust_inspect")]
...
use crate::driver::project::{collect_incan_source_files, resolve_project_root, validate_output_dir};
#[cfg(feature = "rust_inspect")]
...
#[cfg(feature = "rust_inspect")]
use crate::driver::rust_inspect_workspace::collect_rust_inspect_query_paths_from_programs;
use crate::driver::session::CompilationSession;
use crate::frontend::{ParsedModule, diagnostics};
...
use crate::oven::plan::OvenDirectRustcPlanSelection;
use crate::oven::plan::composition::compose_selected_packaged_provider_plan;
use crate::oven::plan::selection::select_packaged_provider_plans;
...
pub(crate) fn prepare_oven_project(
    file_path: &str,
...
pub(crate) fn select_oven_direct_rustc_plan_with_materialization(
    store: &OvenStore,
    receipt: &crate::oven::OvenReceipt,
    registry_dependencies: &[DependencySpec],
...
pub(crate) fn bake_generated_project_compatibility_plan(
    store: &OvenStore,
    receipt: &crate::oven::OvenReceipt,
...
pub(crate) fn remove_completed_generated_cargo_lock(generated_project: &Path) -> CliResult<()> {
...
pub(crate) fn project_extension_base_loaf(receipt: &crate::oven::OvenReceipt) -> CliResult<Option<OvenToolchainLoaf>> {
...
mod tests {
    ...
    use crate::driver::build::OvenBakeProjectTarget;
    use crate::oven::interop::OVEN_INTEROP_EXECUTION_RECEIPT_INPUT;
    use crate::oven::store::OvenStore;
