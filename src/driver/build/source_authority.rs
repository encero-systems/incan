...
use crate::driver::build::bake::discover_oven_executable_entrypoints;
use crate::driver::build::package_loafs::read_packaged_library_loaf_manifest;
...
use crate::driver::project::{effective_project_manifest_for_exact_root, resolve_source_root};
...
use crate::manifest::{DependencySource, GitReference, LOAF_MANIFEST_FILENAME, ProjectManifest};
...
use crate::oven::{digest_bytes, digest_project_source_tree};
...
impl ProjectSourceAuthorityDigester {
    ...
    pub(crate) fn digest(&mut self, project_root: &Path) -> CliResult<String> {
    ...
    #[cfg(test)]
    pub(crate) fn record_project_scan(&mut self, canonical_root: &Path) {
        *self
            .project_scan_counts
    ...
    #[cfg(not(test))]
    pub(crate) fn record_project_scan(&mut self, _canonical_root: &Path) {}

    /// Digest one local Rust package together with only the Cargo-workspace facts that it actually inherits.
    pub(crate) fn digest_rust_path_crate_authority(
        package_root: &Path,
    ...
    pub(crate) fn digest_project_node(&mut self, root: &Path, visiting: &mut HashSet<PathBuf>) -> CliResult<String> {
...
pub(crate) fn digest_baked_project_source_authority(project_root: &Path) -> CliResult<String> {
...
pub(crate) fn canonical_baked_project_lock_path(project_root: &Path) -> CliResult<PathBuf> {
...
pub(crate) fn baked_project_lock_dependencies_fingerprint(project_root: &Path) -> CliResult<Option<String>> {
...
pub(crate) fn project_bake_receipt_path(
    project_root: &Path,
...
pub(crate) fn interop_bootstrap_receipt_path(
    project_root: &Path,
    rust_target: &str,
    target: OvenBakeProjectTarget,
    entrypoint: &Path,
...
pub(crate) fn prepared_oven_receipt_path(
    project_root: &Path,
...
mod tests {
    use super::*;
    use crate::driver::build::test_support::{
        fixture_project_output_publication, fixture_project_output_publication_for,
    };
    use std::collections::{BTreeMap, BTreeSet};
    use std::fs;

    use crate::driver::build::bake::discover_oven_bake_project_targets;
    use crate::driver::build::library_outputs::packaged_library_loaf_store_root;
    use crate::driver::build::output_materialization::{
        select_coherent_library_outputs, warn_for_completed_output_lock_fingerprint_drift,
    };
    use crate::driver::build::output_paths::packaged_library_metadata_files;
    use crate::driver::build::output_selection::{
        matching_baked_project_outputs_with_source_authority, select_baked_project_output,
        select_current_debug_project_outputs,
    };
    use crate::driver::build::package_loafs::write_packaged_library_loaf_manifest;
    use crate::driver::build::plan_authority::read_verified_caller_owned_provider_receipt;
    use crate::driver::build::publication::publish_project_output_loaf;
    use crate::driver::build::reuse::try_reuse_baked_project;
    use crate::driver::build::{
    ...
    use crate::manifest::{LOAF_MANIFEST_FILENAME, ProjectManifest};
    use crate::oven::interop::{
    ...
    use crate::oven::plan::OvenPackagedLibraryLoafEntry;
    use crate::oven::rustc::{
        OvenProjectInspectionAuthorityRef, resolve_active_rustc, rustc_host_target, rustc_identity,
    };
    use crate::oven::store::{OvenArtifactKind, OvenArtifactPublishRequest, OvenStore};
    use crate::oven::{OvenGeneratedProjectRequest, digest_bytes, receipt_generated_project, write_receipt};
    use crate::oven_interop::locked_oven_interop_targets;
