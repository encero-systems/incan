...
use crate::driver::build::BackendSelectionOptions;
use crate::driver::build::replacement::ReplacementModuleInputs;
use crate::driver::build::rust_extern::module_source_identity;
...
pub(crate) fn backend_shadow_comparison(selection: &BackendSelection) -> ShadowComparisonState {
...
/// visible refusal.
pub(crate) fn select_and_resolve_backend(
...
pub(crate) fn finalize_backend_receipt(
...
pub(crate) const REPLACEMENT_EXECUTION_REPORT_SCHEMA_VERSION: &str = "incan.replacement_execution.v1";

/// Return the compiler-owned project-relative destination for a backend-selection execution receipt.
pub(crate) fn default_backend_receipt_path(project_root: &Path) -> PathBuf {
...
pub(crate) fn write_backend_receipt(
...
/// boundary.
pub(crate) fn replacement_profile_cli_error(error: ReplacementExecutionError, entrypoint: &Path) -> CliError {
...
pub(crate) fn replacement_refusal_source<'a>(
...
pub(crate) fn refuse_replacement_profile<T>(
    selection: &BackendSelection,
    error: ReplacementExecutionError,
    entrypoint: &Path,
) -> CliResult<T> {
...
pub(crate) fn resolve_available_replacement_execution(selection: &BackendSelection) -> CliResult<BackendKind> {
