//! Selecting the receipt-bound plan a command executes against, and the test-dependency envelope a test run
//! adds to it.
...
use crate::driver::build::oven_project::{
...
use crate::driver::build::package_loafs::import_checked_packaged_library_loaf;
use crate::driver::build::plan_authority::CallerOwnedProviderRegistryClosure;
use crate::driver::build::provider_compilation::checked_test_dependency_package_profiles;
...
use crate::driver::build_unit::promoted_oven_test_dependencies;
...
use crate::oven::rustc::{OvenRegistryLeafAuthority, OvenRustcArtifactPlan, resolve_active_rustc};
use crate::oven::store::OvenStore;
use crate::oven::{OvenGeneratedProjectRequest, digest_dependency_specs, receipt_generated_project};
...
pub(crate) fn canonical_project_inspection_dependencies(
...
pub(crate) fn prepare_oven_test_dependency_envelope(
    store: &OvenStore,
    project_root: &Path,
    resolved: &ResolvedDependencies,
...
pub(crate) fn select_or_bake_generated_project_plan(
...
pub(crate) fn receipt_requires_final_interop_plan(receipt: &crate::oven::OvenReceipt) -> bool {
    receipt
        .sources
        .build_unit_inputs
...
pub(crate) fn interop_final_plan_required_error() -> CliError {
    CliError::failure(
...
pub(crate) fn select_published_project_plan(
    store: &OvenStore,
    receipt: &crate::oven::OvenReceipt,
    materialization: OvenToolchainMaterialization,
) -> CliResult<Option<OvenDirectRustcPlanPreparation>> {
...
pub(crate) fn format_oven_registry_dependency_requirements(dependencies: &[DependencySpec]) -> String {
...
pub(crate) fn registry_leaf_authority_for_plan_selection(
...
/// no Cargo fallback.
pub(crate) fn caller_owned_provider_registry_conflict(
...
pub(crate) fn provider_registry_conflict_reason(package: &str, pinned_by: Option<&Path>) -> String {
    match pinned_by {
        Some(root) => format!(
...
pub(crate) fn oven_native_closure_refusal(crate_name: &str, reason: &str) -> CliError {
    CliError::failure(format!(
...
mod tests {
    ...
    use crate::backend::ProjectGenerator;
    use crate::dependency_resolver::ResolvedDependencies;
    use crate::driver::build::output_paths::project_inspection_test_dependency_roots;
    use crate::driver::build::{OvenProjectDependencySurface, OvenProjectPlanMode, OvenToolchainMaterialization};
    use crate::driver::build_unit::promoted_oven_test_dependencies;
    use crate::manifest::{DependencySource, DependencySpec};
    use crate::oven::interop::OVEN_INTEROP_EXECUTION_RECEIPT_INPUT;
    use crate::oven::loaf::{OVEN_NESTED_DEPENDENCY_MISS_SUMMARY, OVEN_NO_IMPLICIT_DEPENDENCY_BUILD};
    use crate::oven::plan::OvenDirectRustcPlanSelection;
