...
use crate::driver::build::source_authority::canonical_baked_project_lock_path;
use crate::driver::build::{OVEN_PACKAGED_LIBRARY_LOAF_STORE_RELATIVE_PATH, PreparedLibraryProject};
use crate::driver::build_report::artifact_report;
use crate::driver::error::{CliError, CliResult};
use crate::driver::project::validate_output_dir;
use crate::manifest::{DependencySource, DependencySpec};
use crate::provider::vocab_extraction::PendingDesugarerArtifact;
...
pub(crate) fn dependency_artifact_skips_canonical_lock(artifact_only: bool, sdk_provider_build: bool) -> bool {
...
pub(crate) fn library_rust_inspection_required(artifact_only: bool, metadata_query_paths: &[String]) -> bool {
...
pub(crate) fn remove_generated_library_self_dependencies(resolved: &mut ResolvedDependencies, project_root: &Path) {
...
pub(crate) fn library_output_path(project_root: &Path, output_dir: Option<&str>) -> CliResult<PathBuf> {
    match output_dir {
        Some(output) => {
...
pub(crate) fn library_publication_receipts(project_root: &Path) -> CliResult<Vec<PathBuf>> {
...
pub(crate) fn write_library_manifest_artifacts(prepared: &mut PreparedLibraryProject) -> CliResult<()> {
...
pub(crate) fn packaged_library_loaf_store_root(artifact_root: &Path) -> PathBuf {
...
pub(crate) fn package_desugarer_artifact(out_dir: &Path, artifact: Option<&PendingDesugarerArtifact>) -> CliResult<()> {
