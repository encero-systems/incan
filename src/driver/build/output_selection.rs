...
use crate::driver::build::publication::stored_project_output_from_parts;
...
use crate::driver::build_report::BuildReport;
...
/// misses.
#[cfg(test)]
pub(crate) fn select_baked_project_output(
    store: &OvenStore,
    project_root: &Path,
    entrypoint: &Path,
...
pub(crate) fn baked_project_owner_identity(project_root: &Path) -> CliResult<String> {
...
pub(crate) fn has_stale_baked_project_output(
    store: &OvenStore,
    project_root: &Path,
    entrypoint: &Path,
    target: OvenBakeProjectTarget,
...
pub(crate) fn select_baked_project_output_with_source_authority(
    store: &OvenStore,
    project_root: &Path,
    entrypoint: &Path,
    target: OvenBakeProjectTarget,
...
pub(crate) fn matching_baked_project_outputs_with_source_authority(
    store: &OvenStore,
    project_root: &Path,
    entrypoint: &Path,
    target: OvenBakeProjectTarget,
...
pub(crate) fn current_project_output_receipt(
    project_root: &Path,
    target: OvenBakeProjectTarget,
    entrypoint: &Path,
    profile: &str,
...
pub(crate) fn select_current_debug_project_outputs(
    store: &OvenStore,
    project_root: &Path,
...
pub(crate) fn make_project_output_report_portable(value: &mut serde_json::Value, project_root: &Path) -> CliResult<()> {
...
pub(crate) fn restore_project_output_report_paths(value: &mut serde_json::Value, project_root: &Path) -> CliResult<()> {
...
pub(crate) fn project_output_report_snapshot(
    project_root: &Path,
    report: &BuildReport,
...
mod tests {
    ...
    use crate::driver::build::publication::{project_output_payload_for_bake, publish_project_output_loaf};
    use crate::driver::build::source_authority::{
        baked_project_lock_dependencies_fingerprint, digest_baked_project_source_authority, project_bake_receipt_path,
    };
    use crate::driver::build::{
