...
/// `rust.module()` path references an unknown crate (not a standard library facet and not in `loaf.toml`).
pub fn unresolved_rust_module_crate(crate_name: &str, path: &str, span: Span) -> CompileError {
    ...
    .with_hint(
        "The first segment of the path must be a standard library facet such as `incan_std_core` or a crate declared in `loaf.toml [dependencies]`",
    )
