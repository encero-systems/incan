...
/// different types (`list[int]` and `list[str]`; `list[int]` and `list[i64]` are one type) leave none to give it. A
/// value built only from literals and builtin empty constructors is evaluated once per target instead and never
...
pub fn chained_assignment_value_has_no_one_type(
    ...
    CompileError::type_error(
        format!(
            "The targets of this chained assignment have different types ('{first_target_ty}' and \
