...
//!
//! This module defines [`CompileError`] — the single error type threaded through the lexer, parser, and typechecker —
//! along with its severity ([`ErrorKind`]) and plain-text rendering helpers.
...
///
/// Keeping this payload out of the common [`CompileError`] layout matters because parser APIs return `Result<_,
/// CompileError>` pervasively. The indirection preserves a compact error variant for ordinary syntax and type errors
/// while still retaining rich facts when a producer supplies them.
...
///
/// Every diagnostic produced by the Incan compiler is represented as a `CompileError`. The [`errors`](super::errors)
/// and [`lints`](super::lints) catalog modules provide constructor functions that build well-formatted instances with
