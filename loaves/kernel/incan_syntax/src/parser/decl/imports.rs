...
impl<'a> Parser<'a> {
    ...
    /// Parse a Rust crate path after `rust::` (or `rust.` recovery) Returns (crate_name, optional_path_within_crate)
    /// Examples:
    ...
    /// - Parenthesized list: `(\n    ItemA,\n    ItemB as alias,\n    ItemC,\n)`
    ///
    /// The lexer's `bracket_depth` tracking suppresses `Newline`/`Indent`/`Dedent` tokens inside `(...)`, so no
    /// explicit newline handling is needed here — multi-line layouts parse identically to single-line layouts.
    ///
    /// When `rust_item_names` is true (`from rust::... import ...`), imported symbols may be Incan keywords matching
    /// Rust items (e.g. Substrait's `type` module): use [`Self::identifier_or_any_keyword`]. Incan `from module import
    /// ...` keeps strict identifiers.
