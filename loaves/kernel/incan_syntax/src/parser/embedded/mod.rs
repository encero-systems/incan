...
/// Column-0 line prefixes this scan treats as unambiguous proof a fragment's computed body range swallowed a real
/// subsequent top-level declaration (see [`find_swallowed_top_level_declaration`]'s rustdoc for why).
...
///
/// Deliberately starts scanning *after* `raw`'s own first line, never at `offset == 0`: `raw` is sliced starting right
/// after the block's `Indent` token, so its first line alone -- unlike every subsequent line -- never carries the
/// block's leading indentation in this string, even when the fragment is correctly bounded. Checking it here would
/// false-positive on any legitimate fragment whose very first line happens to start with one of
/// `TOP_LEVEL_DECLARATION_STARTS`'s words (for example `RawText` content that begins with the literal text `"def "`).
