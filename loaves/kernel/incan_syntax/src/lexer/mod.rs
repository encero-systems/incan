...
impl<'a> Lexer<'a> {
    ...
    ///
    /// Returns a vector of tokens on success, or a vector of errors on failure. The token stream always ends with an
    /// `Eof` token.
    ...
    /// Emit a closing bracket token and decrement bracket depth. Produces an error if there's no matching opening
    /// bracket.
...
///
/// This exists for RFC 081 (`#1023`) descriptor-gated embedded fragments: see [`Lexer::tokenize_tolerant`] for why a
/// caller that wants embedded-fragment support (via `parser::parse_with_source`) needs a token stream that survives
/// interior "unexpected character" errors from a claimed fragment's raw content, rather than [`lex`]'s all-or-nothing
/// contract. `Indent`/`Dedent` boundaries remain correct even where other tokens are missing.
///
