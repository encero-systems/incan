...
/// Tuple assignment to lvalue expressions: `arr[i], arr[j] = arr[j], arr[i]` Used for swaps and multi-target
/// assignments where targets are not new bindings.
#[derive(Debug, Clone, PartialEq)]
...
impl Condition {
    ...
    ///
    /// For `if let` / `while let`, this spans from the pattern start through the scrutinee expression so downstream
    /// diagnostics and tooling can treat the let-pattern condition as one surface unit.
    #[must_use]
