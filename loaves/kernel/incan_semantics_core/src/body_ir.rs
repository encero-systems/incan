...
impl Body {
    ...
    ///
    /// The walk recurses into every statement kind that carries a nested [`Block`] -- [`StatementKind::If`],
    /// [`StatementKind::Loop`], and each [`StatementKind::Race`] arm -- but does not recurse into a
    /// [`Rvalue::Closure`]'s [`ClosureBody`] or a [`Rvalue::Generator`]'s [`GeneratorBody`]. A yield nested in either
    /// value does not make the *enclosing* function body a generator, matching how the existing backend's own
    /// `body_contains_yield` walker never descends into nested closure/function literals.
...
///
/// Deliberately lighter than [`Body`]: it carries no `decl_id`/`name`/`scopes`/`runtime_requirements`/`panic_facts` of
...
///
/// A dict is built from an ordered sequence of *effects*, not from a finished mapping: keys are expressions evaluated
/// at runtime, their evaluation order is observable, and a duplicate key is legal and meaningful (a later entry
/// overwrites an earlier one). That is why [`Rvalue::Dict`] carries this ordered list rather than a map — a map would
/// have to collapse duplicates at construction, which is exactly the override semantics that must survive, and it could
/// not hold a spread whose keys are unknown until runtime.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub enum DictEntry {
...
pub enum ArgumentBinding {
    ...
    Resolved {
        ...
        /// its own. Recording the omission here is still what frees a consumer from diffing the operand vector against
        /// the declaration, and it is why no defaulted slot carries an [`OwnershipFact`]: this call site evaluates
        /// nothing for it. Making those defaults evaluable is
...
/// closure/partial is never represented as [`Self::Named`] with a fabricated source name; it is a [`Self::Local`]
/// operand, carrying the lexical-environment ownership decision the caller made.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
pub enum IterProtocol {
    ...
    UserDefined {
        ...
        next_method: String,
        /// Whether `next_method` returns a fallible `Result[Option[T], E]` rather than a plain `Option[T]` (`for item
        /// in iterable?:`, RFC 115). When `true`, [`StatementKind::IterNext`]'s implicit poll also carries an implicit
        /// early-return-with-conversion on the failure variant, mirroring [`StatementKind::TryPropagate`]'s own
        /// `From`/`Into` semantics, before the ordinary exhausted-vs-produced-a-value branch applies to the success
        /// payload.
        fallible: bool,
    },
}
