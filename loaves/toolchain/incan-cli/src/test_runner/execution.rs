...
use incan_driver::lock::OvenLockValidationRequest;
use incan_driver::lock::resolution::validate_oven_lock_policy_with_session;
use incan_driver::testing::infer_test_project_root_without_manifest;
use incan_driver::testing::module_graph::collect_source_modules_for_test;
use incan_driver::testing::types::{FixtureScope, TestInfo, TestResult};
...
) -> crate::CliResult<Arc<OvenTestCommandContext>> {
...
fn command_session_for_execution_unit<'a>(
    ...
    representative_test: &Path,
) -> Result<&'a incan_driver::session::CompilationSession, String> {
...
pub(super) fn validate_oven_test_lock_policy(
    session: &incan_driver::session::CompilationSession,
    representative_test: &Path,
...
) -> Vec<incan_provider::dependency_resolver::InlineRustImport> {
    let mut inline_imports = incan_driver::modules::collect_rust_dependency_uses(test_module, true);
    for module in source_modules {
        inline_imports.extend(incan_driver::modules::collect_rust_dependency_uses(module, false));
    }
    inline_imports
}
...
fn has_fixture_decorator(
    decorators: &[incan_frontend::ast::Spanned<incan_frontend::ast::Decorator>],
...
fn parse_test_batch_sources(
    ...
    compilation_session: &incan_driver::session::CompilationSession,
...
fn literal_type(expr: &Spanned<Expr>) -> Option<Type> {
    match &expr.node {
        Expr::Literal(incan_frontend::ast::Literal::Int(_)) => Some(Type::Simple("int".to_string())),
        Expr::Literal(incan_frontend::ast::Literal::Float(_)) => Some(Type::Simple("float".to_string())),
        Expr::Literal(incan_frontend::ast::Literal::Bool(_)) => Some(Type::Simple("bool".to_string())),
        Expr::Literal(incan_frontend::ast::Literal::String(_)) => Some(Type::Simple("str".to_string())),
        Expr::Literal(incan_frontend::ast::Literal::None) => Some(Type::Unit),
        _ => None,
    }
}
        ...
        Expr::Match(scrutinee, arms) => {
            expr_references_name(&scrutinee.node, name)
                || arms.iter().any(|arm| match &arm.node.body {
                    incan_frontend::ast::MatchBody::Expr(expr) => expr_references_name(&expr.node, name),
                    incan_frontend::ast::MatchBody::Block(body) => body_references_name(body, name),
                })
        }
        Expr::If(if_expr) => {
    ...
    match stmt {
        ...
        Statement::If(if_stmt) => {
            (match &if_stmt.condition {
                incan_frontend::ast::Condition::Expr(expr) => expr_references_name(&expr.node, name),
                incan_frontend::ast::Condition::Let { value, .. } => expr_references_name(&value.node, name),
...
fn capture_candidates(
    func: &incan_frontend::ast::FunctionDecl,
...
fn merge_test_runner_dependencies(
    ...
    dev_dependencies: &[oven_model::manifest::DependencySpec],
) -> Result<Vec<oven_model::manifest::DependencySpec>, String> {
    incan_driver::build_unit::promoted_oven_test_dependencies(&ResolvedDependencies {
...
fn collect_fixture_execution_info(
    ast: &incan_frontend::ast::Program,
    ...
    ast.declarations
        .iter()
        .filter_map(|decl| {
            if let incan_frontend::ast::Declaration::Function(func) = &decl.node {
                ...
                for decorator in &func.decorators {
                    ...
                    for arg in &decorator.node.args {
                        if let incan_frontend::ast::DecoratorArg::Named(name, value) = arg
                            && name == &semantics.fixture_scope_arg
                            && let incan_frontend::ast::DecoratorArgValue::Expr(expr) = value
                            && let Expr::Literal(incan_frontend::ast::Literal::String(value)) = &expr.node
                        {
...
fn run_file_tests_batch_oven(
    ...
    let source_root = incan_driver::project::resolve_source_root(&project_root, manifest.as_ref());
    ...
    let mut requirements = match incan_provider::requirements::collect_project_requirements(
        &dependency_modules,
        &library_manifest_index,
    ) {
    ...
    let mut resolved =
        match resolve_reachable_dependencies(manifest.as_ref(), &inline_imports, true, &feature_selection) {
            ...
            Err(errors) => {
                let sources = incan_driver::modules::build_source_map(&dependency_modules);
                return failure(
                    errors
                        .iter()
                        .map(|error| incan_driver::modules::format_dependency_error(error, &sources))
                        .collect::<String>(),
                );
            }
        };
    ...
    let mut caller_owned_libraries =
        match incan_driver::build::caller_owned::oven_caller_owned_libraries(&provider_plan, "debug") {
            Ok(libraries) => libraries,
            Err(error) => return failure(error.message),
        };
    if let Err(error) =
        incan_provider::inventory::extend_requirements_with_provider_plan(&mut requirements, &provider_plan)
    {
        return failure(error.message);
    }
    if let Err(error) =
        incan_provider::requirements::merge_project_requirement_dependencies(&mut resolved, &requirements)
    {
        return failure(error.message);
    }
    ...
    let inline_libraries_to_materialize =
        incan_driver::build::plan_authority::declared_rust_libraries_missing_from_selected_plan(
            &inline_path_dependencies,
            &artifact_plan,
        );
...
"#;
        ...
        semantics.marker_kinds.insert(
            "fixture".to_string(),
            incan_frontend::testing_markers::TestingMarkerKind::Fixture,
        );
    ...
    fn oven_test_seed_compatibility_records_only_used_sdk_capabilities() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let inputs = incan_driver::build_unit::oven_build_unit_inputs(
            &provider_plan,
    ...
    fn frozen_oven_test_validation_rejects_a_missing_lock_before_scheduling() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        let session = incan_driver::session::CompilationSession::discover_for_oven(
            &test_file,
    ...
    fn oven_test_execution_units_share_one_command_session() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let session = Arc::new(incan_driver::session::CompilationSession::discover_for_oven(
            &first_test,
            &FeatureSelection::default(),
            None,
        )?);
        ...
        let context = OvenTestCommandContext {
            ...
            session,
            store: oven_store::store::OvenStore::new(
                store_root.path(),
...
"#;
    ...
    fn nested_pub_import_dedupe_preserves_exact_child_identity_issue948() -> Result<(), String> {
        ...
        let tokens = incan_frontend::lexer::lex(source).map_err(|errors| format!("{errors:?}"))?;
        let mut program = incan_frontend::parser::parse(&tokens).map_err(|errors| format!("{errors:?}"))?;
    ...
    fn embedded_fragment_expr(nodes: Vec<incan_frontend::ast::EmbeddedNode>, source_text: String) -> Expr {
        use incan_frontend::ast::EmbeddedFragmentExpr;
    ...
    fn embedded_fragment_expr_with_hole(hole_name: &str) -> Expr {
        use incan_frontend::ast::EmbeddedNode;
