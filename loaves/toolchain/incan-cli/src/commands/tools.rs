...
use incan_frontend::api_metadata::{ApiDeclaration, ApiFunction, ApiPartial, CheckedApiMetadataPackage};
...
use incan_frontend::library_manifest::{LibraryManifest, ParamExport, ParamKindExport, TypeRef};
...
use incan_frontend::registry_metadata::{
    ...
    CheckedRegistryMetadataPackage, CheckedRegistryPackageIdentity,
};
use oven_model::manifest::ProjectManifest;

use incan_driver::metadata_packages::{
    collect_api_metadata_package, collect_registry_metadata_package, resolve_metadata_entry_path,
};
use incan_driver::project::{discover_effective_project_manifest, resolve_project_root};
...
pub fn write_feature_inventory_reference(path: &Path) -> CliResult<()> {
    let source = oven_model::toolchain_layout::development_root().join("loaves/stdlib/core/src/features.incn");
...
impl CargoCommandInfo {
    ...
    fn collect() -> Self {
        if env::var_os(oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_RUSTC_ENV)
...
mod tests {
    ...
    use incan_frontend::api_metadata::{CHECKED_API_METADATA_SCHEMA_VERSION, CheckedApiPackageIdentity};
    ...
    fn registry_selector_accepts_package_qualified_and_module_local_identities() {
        let candidate = RegistryInspectionCandidate {
            ...
            registry: CheckedRegistryDefinition {
                ...
                subjects: vec![incan_frontend::registry_metadata::CheckedRegistrySubjectKind::Function],
    ...
    fn std_feature_inventory_is_checked_and_generates_reference() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(rendered.contains("loaves/stdlib/core/src/features.incn"));
        Ok(())
    }
...
"#,
        ...
        assert_eq!(
            ...
            incan_frontend::registry_metadata::CheckedRegistryProvenance::CheckedDeclaration
        );
        Ok(())
    }
...
"#,
    ...
    fn write_test_incan_lock(project_root: &Path) -> Result<(), Box<dyn std::error::Error>> {
        let cargo_lock_payload =
            fs::read_to_string(oven_model::toolchain_layout::development_root().join("Cargo.lock"))?;
