...
//!
//! This module is deliberately thin around the pure policy modules in [`oven_model::project_lifecycle`]. It owns
...
use oven_model::project_lifecycle::env::{EnvConfigError, EnvConfigSet, EnvRunPreview, ResolvedEnv, resolve_cwd};
use oven_model::project_lifecycle::toolchain::{ToolchainCompatibility, ToolchainConstraintSet};
use oven_model::project_lifecycle::version::{VersionBump, VersionChange, VersionRequest};
...
pub fn env_run(
    ...
    incan_driver::cargo_policy::enforce_toolchain_constraints(&preview.resolved_env.requires_incan)?;
...
fn load_env_context(project: Option<&Path>) -> CliResult<EnvContext> {
    let (project_root, manifest_path, manifest_content, manifest) = if let Some(project) = project {
        ...
        let manifest = incan_driver::project::discover_effective_project_manifest(&project_root)?
...
fn discover_lifecycle_manifest() -> CliResult<ProjectManifest> {
    ...
    incan_driver::project::discover_effective_project_manifest(&cwd)?.ok_or_else(|| {
