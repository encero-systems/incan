//! Issue #1385: a canonical form in the capability catalogue compiles as written.
//!
...
//!
//! This root reads the live catalogue rather than restating its forms, so it cannot drift from what the reference
...
/// Catalogue entries whose canonical forms are self-contained enough to compile as one program.
///
/// Each entry names the `const <name>_forms` binding in the live catalogue. An entry belongs here once every one
...
/// Read one `const <name>: FrozenList[str] = [...]` form list out of the live catalogue.
fn catalogue_forms(name: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    let body = declaration
        ...
        .ok_or_else(|| format!("the catalogue declares no `{name}` form list"))?;
    let forms = quoted
...
fn self_contained_catalogue_forms_compile_as_written_issue1385() -> Result<(), Box<dyn std::error::Error>> {
    for entry in SELF_CONTAINED_ENTRIES {
        ...
        let project_name = "catalogue_forms";
        ...
        if !incan_test_support::oven_compiler_suite_is_active() {
            ...
            assert_success(&bake, &format!("prepare the `{entry}` catalogue-forms fixture"));
        }
        let build = run_incan(tmp.path(), &["build", "src/main.incn"])?;
        assert!(
            build.status.success(),
            "the `{entry}` catalogue forms must compile as written.\nprogram:\n{source}\nstdout:\n{}\nstderr:\n{}",
        ...
        assert!(
            run.status.success(),
            "the `{entry}` catalogue-forms program must run.\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&run.stdout),
            String::from_utf8_lossy(&run.stderr)
        );
        assert_eq!(String::from_utf8(run.stdout)?.trim(), "catalogue forms ok");
    }
    Ok(())
}
