...
use oven_store::store_mirror::configured_mirrors;
use oven_store::{
...
pub use incan_driver::oven_store::{
        ...
        match options.base_receipt.as_ref() {
            ...
            None => {
                let (receipt, path, cargo_process_started) =
                    incan_driver::build::oven_project::prepare_oven_interop_bootstrap(
                        &locked.project_root,
...
pub fn oven_run_compiler_libtests(options: OvenCompilerLibtestsRunCommandOptions) -> CliResult<ExitCode> {
    ...
    let (external_toolchain_data_root, _external_toolchain_generation) = if suite.schema_version >= 14 {
        ...
        let data_root = oven_model::toolchain_layout::compiler_owned_oven_data_root().ok_or_else(|| {
            CliError::failure(
...
pub(crate) struct CompilerSuiteShardExecution {
...
pub(crate) struct CompilerSuiteFoundationExecution {
    stored: OvenStoreExecutionPayload,
...
pub(crate) struct CompilerSuiteChildrenReport {
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        for succeed in [true, false] {
            let scratch = oven_rustc::loaf::LoafTemporaryDirectory::create(parent.path(), "scratch-")?;
            let path = scratch.path().to_path_buf();
            let result = super::run_compiler_suite_with_scratch(scratch, || {
                fs::write(path.join("fixture"), b"owned child scratch")
                    .map_err(|error| crate::CliError::failure(error.to_string()))?;
                if succeed {
                    Ok(7)
                } else {
                    Err(crate::CliError::failure("preparation failed"))
                }
            });
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let error = super::run_compiler_suite_with_scratch(scratch, || -> crate::CliResult<()> {
            Err(crate::CliError::failure("child preparation failed"))
        })
        .err()
    ...
    fn storage_policy_has_bounded_defaults() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(limits.max_physical_bytes, 12 * 1024 * 1024 * 1024);
    ...
    fn compiler_suite_limits_generated_rust_closure_to_its_consumers() {
        ...
        assert!(
            !OvenCompilerSuiteTargetCapabilities::for_target(
                "incan-cli",
                "test",
                "loaves/toolchain/incan-cli/tests/toolchain_installer_tests.rs"
            )
            .generated_rust_closure
        );
        assert!(
        ...
        let mut environment = BTreeMap::from([
            ...
            (
                oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_CAPABILITY_ENV.to_string(),
                "warning-capability".to_string(),
            ),
            (
                oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_VOCAB_CAPABILITY_ENV.to_string(),
        ...
        assert!(!environment.contains_key(oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_CAPABILITY_ENV));
        assert!(!environment.contains_key(oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_VOCAB_CAPABILITY_ENV));
        assert_eq!(
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            environment.get(oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_FIXTURE_RUSTC_REAL_ENV),
            Some(&"/fixture/real/rustc".to_string())
        );

        target.package_name = "incan-cli".to_string();
        target.target_kind = "test".to_string();
        target.target_name = "cli_provider_boundary_tests".to_string();
        target.source_relative_path = "loaves/toolchain/incan-cli/tests/cli_provider_boundary_tests.rs".to_string();
        ...
        assert_eq!(
            ordinary_environment.get(oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_EXPLICIT_BAKE_CARGO_ENV),
    ...
    fn compiler_suite_fixture_cargo_preserves_the_receipt_selected_rustc() -> Result<(), Box<dyn std::error::Error>> {
        ...
        explicit
            .env(
                oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_FIXTURE_CARGO_REAL_ENV,
                &fixture.real,
            )
            .env(
                oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_FIXTURE_RUSTC_REAL_ENV,
                &fixture.real_rustc,
            )
            .env(
                oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_FIXTURE_CARGO_LOG_ENV,
                &fixture.log,
            )
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let warning_capability = oven_store::compiler_suite_env::OvenCompilerSuiteCapability::decode(
            &environment[oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_CAPABILITY_ENV],
        )?;
        ...
        let vocab_capability = oven_store::compiler_suite_env::OvenCompilerSuiteCapability::decode(
            &environment[oven_store::compiler_suite_env::OVEN_COMPILER_SUITE_VOCAB_CAPABILITY_ENV],
        )?;
...
"#,
    ...
    fn command_surface_runs_a_stored_native_test_without_a_cargo_consumer() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let receipt_model: oven_store::OvenReceipt = serde_json::from_slice(&receipt_data)?;
