...
| `compiled_provider_preserves_shared_rust_interop_contracts_issues834_835_961` in `loaves/toolchain/incan-cli/tests/rfc031_pub_import_integration_tests.rs` and the Rust metadata/callback coverage in `loaves/toolchain/incan-cli/tests/cli_rust_interop_tests.rs` | Current provider and compiler interop planning across a legacy compiled path. | A caller-facet case that proves supported Rust-host call shapes and refuses non-representable exports or boundary types before publishing an invalid consumer artifact. |
