...
`behavior/` is the behaviour-fixture family of the #1561 test-corpus cutover: Incan programs whose leading comment block declares the observables a run must show, discovered and run by `behavior_fixtures` from one root per area under `loaves/toolchain/incan-cli/tests/`; its `README.md` describes the format.
