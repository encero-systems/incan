...
pub enum SetConstructorIteration {
...
#[derive(Debug, Clone)]
pub struct CarriedNativeUnion(pub Box<incan_frontend::library_manifest::NativeUnionExport>);
...
impl std::ops::Deref for CarriedNativeUnion {
    type Target = incan_frontend::library_manifest::NativeUnionExport;
...
impl IrType {
    ...
    pub fn provider_localized(&self, library: &str) -> Self {
    ...
    pub fn iterator_item_type(&self) -> Option<&IrType> {
        match self {
    ...
    pub fn is_iterator_protocol(&self) -> bool {
...
pub fn is_string_storage_type(ty: &IrType) -> bool {
    matches!(
        ty,
...
pub fn isinstance_union_variant_indices(union_ty: &IrType, target_ty: &IrType) -> Option<Vec<usize>> {
...
pub fn ir_type_from_projected_manifest(
    ty: &incan_frontend::library_manifest::TypeRef,
    ordinary: &impl Fn(&incan_frontend::library_manifest::TypeRef) -> IrType,
) -> IrType {
    use incan_frontend::library_manifest::TypeRef;
...
mod tests {
    ...
    fn an_external_union_keeps_its_identity_across_artifact_digests() {
        fn union_of(digest: &str, projected: bool) -> IrType {
            let mut descriptor = incan_frontend::library_manifest::NativeUnionExport {
                owner: incan_frontend::library_manifest::NativeUnionOwnerExport::SelectedArtifact(
            ...
            if projected {
                descriptor.checked_projection =
                    Some(Box::new(incan_frontend::library_manifest::NativeUnionProjection {
