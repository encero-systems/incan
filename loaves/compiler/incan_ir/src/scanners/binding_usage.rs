...
use crate::stmt::{AssignTarget, IrStmt, IrStmtKind};

#[derive(Clone, Copy)]
pub struct BindingUseScan {
    pub used: bool,
    pub shadowed_after: bool,
}

/// Scan local rest statements, outer leaked-binding tails, and an optional final block value for one binding.
pub fn binding_use_scan(
    local_rest: &[IrStmt],
...
pub fn stmt_list_binding_use_scan(stmts: &[IrStmt], binding_name: &str) -> BindingUseScan {
    for stmt in stmts {
...
pub fn expr_uses_binding_name(expr: &IrExpr, binding_name: &str) -> bool {
    match &expr.kind {
