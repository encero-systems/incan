...
impl AstLowering {
    ...
    pub(in crate::lower) fn define_for_pattern_bindings(&mut self, pattern: &ast::Pattern, ty: &IrType) {
        match pattern {
    ...
    pub fn lower_statements(&mut self, stmts: &[Spanned<ast::Statement>]) -> Result<Vec<IrStmt>, LoweringError> {
        let mut read_counts = HashMap::new();
        for s in stmts {
    ...
    pub fn lower_statement(&mut self, stmt: &ast::Statement, stmt_span: ast::Span) -> Result<IrStmt, LoweringError> {
        let kind = match stmt {
    ...
    pub fn count_match_arm_ident_reads(&self, arm: &ast::MatchArm) -> HashMap<String, usize> {
    ...
    pub fn count_expr_ident_reads(&self, expr: &ast::Expr, counts: &mut HashMap<String, usize>) {
        match expr {
