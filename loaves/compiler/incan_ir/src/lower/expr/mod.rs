...
use incan_frontend::symbols::{ImplementationTraitBoundOriginInfo, ImplementationTypeParamInfo, ResolvedType};
...
impl AstLowering {
    ...
    fn apply_checked_c_argument_accesses(function: &IrCheckedCFunction, args: &mut [IrCallArg]) {
        for (index, argument) in args.iter_mut().enumerate() {
            ...
            let variable_access = match access {
                incan_frontend::typechecker::CResourceAccess::Owned => VarAccess::Move,
                incan_frontend::typechecker::CResourceAccess::Borrowed => VarAccess::CAbiBorrow,
                incan_frontend::typechecker::CResourceAccess::BorrowedMut => VarAccess::BorrowMut,
            };
    ...
    pub fn lower_resolved_method_dispatch(
        &self,
    ...
    pub fn lower_expr_spanned(&mut self, expr: &Spanned<ast::Expr>) -> Result<TypedExpr, LoweringError> {
        ...
        // A trait default body keeps the defining module's AST spans while it is expanded into an adopter. Expression
        // facts are keyed only by span, so an adopter-local fact at the same offsets is not authority for that body.
        // Keep declaration identities and all non-expression semantic artifacts active; only this ambiguous span map
        // is excluded while the default's defining substitutions are active.
        if self.active_trait_type_substitutions.is_empty()
    ...
    pub fn lower_expr(&mut self, expr: &ast::Expr, expr_span: ast::Span) -> Result<TypedExpr, LoweringError> {
        let (kind, ty) = match expr {
            ...
            ast::Expr::Surface(surface_expr) => {
                use incan_frontend::semantics_registry::semantics_registry;
...
mod tests {
    ...
    fn lowering_resolving_a_package_method(
        ...
        let mut type_info = incan_frontend::typechecker::TypeCheckInfo::default();
    ...
    fn imported_method_chain_dispatches_trait_through_public_library() {
        ...
        let dispatch = lowering.lower_resolved_method_dispatch(
            ResolvedMethodDispatch::Trait {
                ...
                type_args: vec![
                    incan_frontend::symbols::ResolvedType::Int,
                    incan_frontend::symbols::ResolvedType::Str,
                ],
                implementation_type_params: Vec::new(),
                receiver_is_mutable: false,
            },
