...
/// projection -- and never the bare set spelling the checked path ends with. `selected` is `Some` exactly when the
...
impl AstLowering {
    ...
    pub fn lower_checked_c_output_slot_constructor(
        &mut self,
        call_span: ast::Span,
        args: &[ast::CallArg],
        ...
        let function = TypedExpr::new(
            IrExprKind::AssociatedFunction {
                ...
                function_name: match slot.mode {
                    incan_frontend::typechecker::COutputMode::Out => "uninit".to_string(),
                    incan_frontend::typechecker::COutputMode::InOut => "from_incan_value".to_string(),
                },
            },
            IrType::Unknown,
        );
    ...
    pub fn lower_checked_c_string_constructor(
        &mut self,
        call_span: ast::Span,
        args: &[ast::CallArg],
    ) -> Result<Option<(IrExprKind, IrType)>, LoweringError> {
    ...
    pub fn lower_checked_c_span_constructor(
        &mut self,
        call_span: ast::Span,
        args: &[ast::CallArg],
    ) -> Result<Option<(IrExprKind, IrType)>, LoweringError> {
    ...
    pub(in crate::lower::expr) fn explicit_builtin_member_name(callee: &ast::Spanned<ast::Expr>) -> Option<&str> {
    ...
    pub(in crate::lower::expr) fn is_explicit_builtin_namespace_expr(expr: &ast::Spanned<ast::Expr>) -> bool {
    ...
    pub(in crate::lower) fn lower_pub_default_expr(
        &mut self,
        library: &str,
    ...
    pub(in crate::lower) fn callable_signature_for_imported_stdlib_type_method_path(
        &mut self,
        path: &[String],
        method_name: &str,
    ) -> Result<Option<FunctionSignature>, LoweringError> {
    ...
    pub(in crate::lower) fn sdk_provider_crate_for_type(&self, type_name: &str) -> Option<String> {
    ...
    pub(in crate::lower) fn sdk_provider_path_for_type(&self, type_name: &str) -> Option<String> {
        if self.struct_names.contains_key(type_name)
            || self.enum_names.contains_key(type_name)
            || self.class_decls.contains_key(type_name)
            || self.trait_decls.contains_key(type_name)
    ...
    pub(in crate::lower) fn callable_signature_for_compiled_provider_type_method(
        &mut self,
        receiver_ty: &IrType,
        method_name: &str,
    ...
    pub(in crate::lower) fn compiled_provider_method_reference_name(
        &self,
        call_span: ast::Span,
        receiver_ty: &IrType,
        method_name: &str,
    ...
    pub(in crate::lower) fn callable_signature_for_imported_pub_type_method(
        &mut self,
        library: &str,
        receiver_ty: &IrType,
        method_name: &str,
    ...
    fn public_dependency_type_path(receiver_ty: &IrType, library: &str) -> Option<Vec<String>> {
        ...
        let (dependency, public_name) = incan_frontend::typechecker::split_canonical_public_library_type_name(name)?;
    ...
    pub(in crate::lower) fn nominal_receiver_type_name(receiver_ty: &IrType) -> Option<&str> {
        match receiver_ty {
    ...
    fn callable_signature_from_compiled_provider_method_export(
        ...
        FunctionSignature {
            params: method
                ...
                .collect(),
            return_type: self.lower_resolved_type(
                &incan_frontend::library_manifest::resolved_type_from_manifest_type_ref(&method.return_type),
            ),
        }
    }
    ...
    fn callable_signature_from_compiled_provider_function_export(
        ...
        FunctionSignature {
            params: function
                ...
                .collect(),
            return_type: self.lower_resolved_type(
                &incan_frontend::library_manifest::resolved_type_from_manifest_type_ref(&function.return_type),
            ),
        }
    }
        ...
        let callable_signature = signature.map(|signature| {
            let signature = FunctionSignature {
                params: signature
                    ...
                    .collect(),
                return_type: self.lower_resolved_type(
                    &incan_frontend::library_manifest::resolved_type_from_manifest_type_ref(&signature.return_type),
                ),
            };
            self.compiled_provider_external_signature(provider_crate, signature)
        });
    ...
    pub(in crate::lower) fn callable_signature_for_imported_stdlib_path(
        &mut self,
        path: &[String],
    ) -> Result<Option<FunctionSignature>, LoweringError> {
    ...
    pub(in crate::lower) fn wrap_with_validated_newtype_coercion(
        &mut self,
        mut expr: TypedExpr,
        span: ast::Span,
    ...
    fn aggregate_field_coercion(
        &self,
        span: ast::Span,
    ) -> Option<incan_frontend::typechecker::ValidatedNewtypeCoercionInfo> {
        self.type_info
            .as_ref()
            .and_then(|info| info.validated_newtype_coercion(span).cloned())
    ...
    pub(in crate::lower) fn wrap_with_rust_return_coercion(
        &mut self,
        expr: TypedExpr,
        span: ast::Span,
    ) -> Result<TypedExpr, LoweringError> {
    ...
    pub(in crate::lower) fn lower_call_expr(
        &mut self,
        ...
        // An overload set has no single declaration, so a provider exports only its concrete overloads -- each under
    ...
    fn is_named_field_constructor_call(type_info: &incan_frontend::symbols::TypeInfo, args: &[ast::CallArg]) -> bool {
        let fields = match type_info {
            incan_frontend::symbols::TypeInfo::Model(info) => &info.fields,
            incan_frontend::symbols::TypeInfo::Class(info) => &info.fields,
            _ => return false,
        };
        !args.is_empty()
    ...
    pub(in crate::lower) fn lower_call_args(&mut self, args: &[ast::CallArg]) -> Result<Vec<IrCallArg>, LoweringError> {
...
mod tests {
    ...
    fn canonical_path_names_the_overload_a_call_selected() {
        ...
        let named = canonical_path_naming_selected_overload(path, Some("__incan_v1_00deadbeef"));

        assert_eq!(
            named.last().map(String::as_str),
            Some("__incan_v1_00deadbeef"),
            "a provider exports the concrete overload, never the bare set spelling"
        );
    }

    #[test]
    fn canonical_path_keeps_its_spelling_when_no_overload_was_selected() {
        // A non-overloaded function is exported under its own name, and the emitter's provider-metadata lookup is
        // keyed on this source-shaped path.
        let path = vec!["pub".to_string(), "widgets".to_string(), "make_widget".to_string()];

        assert_eq!(canonical_path_naming_selected_overload(path.clone(), None), path);
    }
    ...
    fn imported_pub_callable_signature_uses_identity_graph_before_short_name_lookup() -> Result<(), String> {
        ...
        let signature = lowering
            .callable_signature_for_imported_pub_path(
                ...
                None,
                incan_frontend::ast::Span::default(),
            )
    ...
    fn lower_rust_named_field_constructor_wraps_fields_with_rust_arg_coercion() -> Result<(), String> {
        ...
        type_info.expressions.ident_kinds.insert(
            ...
            incan_frontend::typechecker::IdentKind::TypeName,
        );
