...
impl AstLowering {
    ...
    pub(in crate::lower) fn trait_impl_targets_for_adopted_trait_bound(
        &self,
    ...
    pub(in crate::lower) fn rusttype_forwarding_satisfied_by_alias(&self, type_name: &str, trait_name: &str) -> bool {
    ...
    pub(in crate::lower) fn lower_decorated_method_statics(
        &mut self,
        type_name: &str,
        methods: &[Spanned<ast::MethodDecl>],
        ...
        for method in methods {
            ...
            let original_params = match binding.original_unbound_ty {
                incan_frontend::symbols::ResolvedType::Function(params, _) => params,
                _ => Vec::new(),
            };
            let incan_frontend::symbols::ResolvedType::Function(params, ret) = binding.unbound_ty else {
                continue;
            };
    ...
    fn lower_decorated_method_wrapper(
        ...
        let original_callable_params = match binding.original_unbound_ty {
            incan_frontend::symbols::ResolvedType::Function(original_params, _) => original_params,
            _ => Vec::new(),
        };
    ...
    fn decorated_method_callable_signature(
        &mut self,
        callable_params: &[incan_frontend::symbols::CallableParam],
        callable_ret: &incan_frontend::symbols::ResolvedType,
        method: &ast::MethodDecl,
        original_callable_params: Option<&[incan_frontend::symbols::CallableParam]>,
    ...
    fn decorated_method_original_adapter(
        ...
        let incan_frontend::symbols::ResolvedType::Function(params, ret) = binding.original_unbound_ty else {
            return self.lower_method_with_type_params(method, None);
        };
        let Some((receiver_param, surface_params)) = params.split_first() else {
            return self.lower_method_with_type_params(method, None);
    ...
    pub(in crate::lower) fn lower_trait_impl(
        &mut self,
        ...
        let lowered_result = (|| {
            ...
            for trait_method in &trait_methods {
                ...
                if trait_method.node.body.is_some() {
                    ...
                    self.active_imported_trait_defaults.push(has_type_paths);
    ...
    pub(in crate::lower) fn lower_method_with_type_params(
        &mut self,
        m: &ast::MethodDecl,
        type_param_names: Option<&std::collections::HashSet<&str>>,
    ) -> Result<IrFunction, LoweringError> {
    ...
    pub(in crate::lower) fn lower_property_with_type_params(
        &mut self,
...
#[derive(Clone, Copy)]
pub(in crate::lower) enum PropertyLoweringMode {
    Inherent,
    TraitDecl,
    TraitImpl,
}
