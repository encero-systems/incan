...
//! const or calls a function of the declaring module therefore reaches the caller as a path to that item. A private
//! const or function is still a valid default for a public callable, so the generated item such a path names is
//! published beyond its module: its Incan visibility is unchanged, and only the generated path a default takes becomes
...
impl AstLowering {
    /// Record the module consts and functions that any parameter default or method-partial preset of this module
    /// names.
    ///
    /// Every callable a module declares counts: functions, and the methods and method partials of its models, classes,
    /// traits and newtypes, plus the methods of its enums. A name only counts when the module declares a const or a
    ...
    /// Return the generated visibility of a module const or function, published when a default names it.
