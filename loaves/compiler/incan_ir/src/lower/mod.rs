...
#[derive(Debug, Clone)]
pub struct ImportedAliasTarget {
...
pub struct AstLowering {
    ...
    pub scopes: Vec<HashMap<String, IrType>>,
    /// Scope chain for local bindings that preserve RFC 052 live static semantics.
    pub static_binding_scopes: Vec<std::collections::HashSet<String>>,
    /// Scope chain for local callable signatures that carry default expressions not representable in [`IrType`].
    pub local_callable_signature_scopes: Vec<HashMap<String, Option<FunctionSignature>>>,
    ...
    pub active_trait_type_substitutions: Vec<HashMap<String, IrType>>,
    ...
    pub struct_field_aliases: HashMap<String, HashMap<String, String>>,
    ...
    /// straight-line block.
    pub remaining_ident_reads: Vec<HashMap<String, usize>>,
    ...
    pub non_linear_context_depth: usize,
    ...
    pub rust_import_aliases: HashMap<String, Vec<String>>,
    /// Function-typed parameters for the currently lowered callable body.
    pub callable_param_scopes: Vec<HashSet<String>>,
    ...
    pub overload_alias_reexport_targets: HashSet<String>,
    ...
    pub rusttype_underlying: HashMap<String, IrType>,
    /// Raw `interop:` edge declarations keyed by rusttype alias name.
    pub rusttype_interop_edges: HashMap<String, Vec<ast::InteropEdgeDecl>>,
    ...
    pub generated_method_partial_wrappers: HashSet<(String, usize, usize, String)>,
    /// Source-local generated method-partial wrappers, indexed without their original owner for inherited methods.
    pub local_generated_method_partial_wrappers: HashSet<(usize, usize, String)>,
    /// Best-effort source module name for compiler-provided call-site metadata.
    pub current_source_module_name: Option<String>,
...
impl AstLowering {
    ...
    pub(in crate::lower) fn emitted_function_reference_name(&self, span: ast::Span) -> Option<String> {
        self.type_info
            .as_ref()?
    ...
    /// untouched.
    pub(in crate::lower) fn emitted_method_reference_name(
        &self,
        span: ast::Span,
    ...
    pub fn push_callable_return_type(&mut self, return_type: &IrType) {
    ...
    pub fn pop_callable_return_type(&mut self) {
    ...
    pub fn coerce_checked_c_return_value(&self, source: &ast::Spanned<ast::Expr>, value: TypedExpr) -> TypedExpr {
    ...
    pub fn checked_c_ir_type(ty: &CBindingType) -> Option<IrCheckedCType> {
        match ty {
    ...
    pub fn lower_param_container_type(kind: ast::ParamKind, base_ty: IrType) -> IrType {
        match kind {
    ...
    fn lower_callable_surface_return_type(
        &self,
        callable_ret: &incan_frontend::symbols::ResolvedType,
        original_ret: Option<&incan_frontend::symbols::ResolvedType>,
    ...
    pub fn current_default_logger_name(&self) -> String {
    ...
    pub fn active_trait_default_function_path(&self, name: &str) -> Option<Vec<String>> {
    ...
    pub fn active_trait_default_value_type_path(&self, name: &str) -> Option<Vec<String>> {
    ...
    pub fn active_trait_type_substitution(&self, name: &str) -> Option<IrType> {
    ...
    pub fn is_active_callable_type_param(&self, name: &str) -> bool {
    ...
    fn metadata_partial_wrapper(&self, partial: &ast::PartialDecl, span: ast::Span) -> Option<ast::FunctionDecl> {
        ...
        let callee = match projection.target_kind {
            incan_frontend::typechecker::PartialProjectionTargetKind::ModelConstructor
            | incan_frontend::typechecker::PartialProjectionTargetKind::ClassConstructor
            | incan_frontend::typechecker::PartialProjectionTargetKind::NewtypeConstructor => {
                Self::ident_expr(projection.target_path.last()?.clone(), span)
            }
            incan_frontend::typechecker::PartialProjectionTargetKind::Function
            | incan_frontend::typechecker::PartialProjectionTargetKind::Unknown => {
    ...
    pub fn consume_ident_read(&mut self, name: &str) -> bool {
    ...
    pub fn define_local_binding(&mut self, name: String, ty: IrType, is_static_binding: bool) {
    ...
    pub fn define_local_callable_signature(&mut self, name: String, signature: Option<FunctionSignature>) {
    ...
    pub fn update_local_callable_signature(&mut self, name: &str, signature: Option<FunctionSignature>) {
    ...
    pub fn lookup_local_callable_signature(&self, name: &str) -> Option<FunctionSignature> {
    ...
    pub fn define_nominal_callable(&mut self, name: String, signature: FunctionSignature) {
    ...
    pub fn lookup_nominal_callable(&self, name: &str) -> Option<FunctionSignature> {
        self.scopes
            .iter()
    ...
    pub fn partial_expr_signature_for_span(&self, span: ast::Span) -> Option<FunctionSignature> {
    ...
    pub fn partial_expr_callable_signature(
        &mut self,
        partial: &ast::PartialExpr,
        span: ast::Span,
    ) -> Result<Option<FunctionSignature>, LoweringError> {
        let Some(incan_frontend::symbols::ResolvedType::Function(params, ret)) =
    ...
    pub fn update_local_binding(&mut self, name: &str, ty: IrType) {
    ...
    pub fn push_callable_param_scope(&mut self, params: &[FunctionParam]) {
    ...
    pub fn is_static_binding(&self, name: &str) -> bool {
    ...
    /// The name of a bare identifier the typechecker resolved to a module static, `None` for anything else.
    pub fn is_direct_static_ident(&self, expr: &ast::Spanned<ast::Expr>) -> Option<String> {
        ...
        self.type_info
            ...
            .filter(|kind| matches!(kind, incan_frontend::typechecker::IdentKind::Static))
            .map(|_| name.clone())
    }
    ...
    pub fn resolve_field_alias(&self, struct_name: &str, field_name: &str) -> String {
    ...
    pub fn register_field_aliases(&mut self, struct_name: &str, fields: &[ast::Spanned<ast::FieldDecl>]) {
        ...
        for decl in &program.declarations {
            if let ast::Declaration::Function(ref f) = decl.node {
                ...
                if let Some(binding) = self.decorated_function_binding_for_decl(&f.name, decl.span)
                    && let incan_frontend::symbols::ResolvedType::Function(callable_params, callable_ret) = binding.ty
                {
                    let (original_params, original_ret) = match &binding.original_ty {
                        incan_frontend::symbols::ResolvedType::Function(params, ret) => {
    ...
    fn decorated_function_binding_for_decl(
        ...
        name: &str,
        span: ast::Span,
    ) -> Option<incan_frontend::typechecker::DecoratedFunctionBindingInfo> {
        let info = self.type_info.as_ref()?;
        info.declarations
    ...
    fn lower_decorated_function_declarations(
        ...
        let (original_params, original_ret) = match binding.original_ty {
            incan_frontend::symbols::ResolvedType::Function(params, ret) => (params, Some(ret)),
            _ => (Vec::new(), None),
        };
    ...
    pub fn decorated_param_defaults_for_surface(
        &mut self,
...
"#,
        ...
        assert!(matches!(vault.fields[0].visibility, crate::decl::Visibility::Private));
        assert!(vault.fields[0].is_type_private);
        assert!(matches!(vault.fields[1].visibility, crate::decl::Visibility::Public));
...
"#,
        ...
        assert!(matches!(
            ...
            Some(IrExprKind::Literal(crate::expr::Literal::StaticStr(name))) if name == "ValueError"
        ));
        Ok(())
    }
...
"#,
        ...
        let returned_expr = |name: &str| -> &crate::expr::TypedExpr {
            let function = ir
                .declarations
                .iter()
                .find_map(|decl| match &decl.kind {
