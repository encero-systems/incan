...
fn parse_incan_program(source: &str, context: &str) -> incan_frontend::ast::Program {
...
fn generate_rust_with_vocab_wasm_desugaring(source: &str) -> String {
    use incan_frontend::library_manifest::{LibraryManifest, VocabDesugarerArtifact, VocabExports};
    use incan_frontend::library_manifest_index::{
        LibraryArtifactMetadata, LibraryManifestIndex, LibraryManifestIndexEntry,
    };
...
fn load_test_file(name: &str) -> String {
    ...
    let Ok(content) = fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join(&path)) else {
...
"#;
    ...
    let plan = incan_frontend::provider::ProviderPlan::default()
        .with_bootstrap_sdk_namespace_roots(["collections".to_string()]);
    let mut codegen = IrCodegen::new();
...
/// and threads the explicit constructor type argument so a binding without an annotation still names `T`.
#[test]
...
#[test]
fn test_rust_generic_bounds() -> TestResult {
    let source = fs::read_to_string(incan_test_support::fixture("valid/rust_generic_bounds.incn"))?;
