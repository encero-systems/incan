...
pub fn collect_serde_derives(main: &Program, deps: &[(&str, &Program)]) -> (bool, bool) {
    ...
    if !has_serialize && !has_deserialize {
        let serde_used = incan_ir::scanners::detect_serde_usage(main)
            || deps
                .iter()
                .any(|(_, program)| incan_ir::scanners::detect_serde_usage(program));
        if serde_used {
            has_serialize = true;
        }
    }
...
pub fn add_serde_to_newtypes(ir_program: &mut incan_ir::IrProgram, add_serialize: bool, add_deserialize: bool) {
