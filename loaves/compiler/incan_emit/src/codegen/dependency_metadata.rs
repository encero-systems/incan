...
pub fn should_preserve_dependency_public_items(
    _module_path: &[String],
    ...
    P: FnMut(&IrExpr) -> bool,
{
    ...
    match &expr.kind {
        ...
        IrExprKind::Format { parts } => parts.iter().any(|part| match part {
            ...
            incan_ir::expr::FormatPart::Expr { expr, .. } => ir_expr_any_expr(expr, predicate),
        }),
...
pub fn collect_externally_reachable_items_by_module(
    ...
    fn record_imports(
        ...
        if !module_import_bindings.is_empty() {
            let _ = incan_frontend::ast_walk::any_expr_in_program(program, |expr| {
...
mod tests {
    ...
    fn parse(source: &str) -> Program {
        let tokens =
            incan_frontend::lexer::lex(source).unwrap_or_else(|errors| panic!("fixture should lex: {errors:?}"));
        incan_frontend::parser::parse(&tokens).unwrap_or_else(|errors| panic!("fixture should parse: {errors:?}"))
    }
...
pub struct DependencySymbolMetadata {
...
pub fn collect_dependency_symbol_metadata(deps: &[(&str, &Program, Option<Vec<String>>)]) -> DependencySymbolMetadata {
