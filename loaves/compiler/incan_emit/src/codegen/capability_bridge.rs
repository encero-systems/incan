...
pub fn compilation_imports_contract(
    main: &Program,
...
pub fn trait_export_lookup_keys(trait_export: &TraitExport) -> impl Iterator<Item = String> + '_ {
...
pub fn export_adopts_capability(
...
mod tests {
    ...
    fn parse(source: &str) -> Result<Program, String> {
        let tokens = incan_frontend::lexer::lex(source)
            .map_err(|errors| format!("capability import fixture should lex: {errors:?}"))?;
        incan_frontend::parser::parse(&tokens)
