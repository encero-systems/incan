...
use incan_ir::types::Mutability;

/// Get the root variable name of an expression.
fn root_var_name(expr: &incan_ir::expr::IrExpr) -> Option<&str> {
    match &expr.kind {
...
fn expr_contains_mutation(expr: &incan_ir::expr::IrExpr, var: &str) -> bool {
    match &expr.kind {
        ...
        IrExprKind::Format { parts } => parts.iter().any(|part| match part {
            incan_ir::expr::FormatPart::Literal(_) => false,
            incan_ir::expr::FormatPart::Expr { expr, .. } => expr_contains_mutation(expr, var),
        }),
        _ => false,
    }
}
...
pub(in crate::emit) fn stmt_mutates_var(stmt: &IrStmt, var: &str) -> bool {
    match &stmt.kind {
...
fn expr_storage_binding_root_name(expr: &incan_ir::expr::IrExpr) -> Option<&str> {
    match &expr.kind {
        IrExprKind::Var {
            name,
            ref_kind: incan_ir::expr::VarRefKind::StaticBinding,
            ..
        } => Some(name.as_str()),
...
fn expr_mutates_storage_binding(expr: &incan_ir::expr::IrExpr, names: &mut HashSet<String>) {
    ...
    match &expr.kind {
        IrExprKind::MethodCall {
            ...
            if !matches!(arg_policy, incan_ir::expr::MethodCallArgPolicy::PreserveShape)
...
impl<'a> IrEmitter<'a> {
    ...
    pub fn emit_stmts(&self, stmts: &[IrStmt]) -> Result<Vec<TokenStream>, EmitError> {
    ...
    pub fn emit_function_body(&self, stmts: &[IrStmt], is_generator: bool) -> Result<Vec<TokenStream>, EmitError> {
    ...
    pub fn emit_stmts_before_expr(&self, stmts: &[IrStmt], value: &TypedExpr) -> Result<Vec<TokenStream>, EmitError> {
    ...
    fn emit_storage_rooted_assignment(
        ...
        let rhs_expr = incan_ir::expr::TypedExpr::new(
            IrExprKind::Var {
                name: rhs_name.to_string(),
                access: incan_ir::expr::VarAccess::Move,
                ref_kind: incan_ir::expr::VarRefKind::Value,
            },
            value.ty.clone(),
        );
    ...
    pub fn emit_stmt(&self, stmt: &IrStmt) -> Result<TokenStream, EmitError> {
    ...
    fn positional_call_args(args: &[incan_ir::expr::IrCallArg]) -> Option<Vec<TypedExpr>> {
        args.iter()
