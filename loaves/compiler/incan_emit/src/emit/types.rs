...
impl<'a> IrEmitter<'a> {
    ...
    pub fn emit_union_type_path(&self, ty: &IrType) -> TokenStream {
    ...
    pub fn emit_union_type_path_with_qualifier(&self, ty: &IrType, qualifier: Option<&[String]>) -> TokenStream {
        ...
        let union_name = semantic_ty
            .union_type_name()
            .unwrap_or_else(|| incan_ir::types::IR_UNION_TYPE_NAME.to_string());
    ...
    pub fn emit_type(&self, ty: &IrType) -> TokenStream {
        match ty {
            IrType::Unit => quote! { () },
            ...
            IrType::NamedGeneric(name, _) if name == incan_ir::types::IR_UNION_TYPE_NAME => {
    ...
    pub(in crate::emit) fn emit_callable_fn_type(&self, params: &[IrType], ret: &IrType) -> TokenStream {
    ...
    pub fn emit_type_params(&self, type_params: &[incan_ir::decl::IrTypeParam]) -> TokenStream {
    ...
    pub fn emit_deserialize_type_params(&self, type_params: &[incan_ir::decl::IrTypeParam]) -> TokenStream {
        let params: Vec<TokenStream> = type_params
            .iter()
            .map(|tp| {
                let name = format_ident!("{}", &tp.name);
    ...
    ///
    /// Returns empty tokens if there are no type parameters.
    pub fn emit_type_params_bare(&self, type_params: &[incan_ir::decl::IrTypeParam]) -> TokenStream {
        if type_params.is_empty() {
            return quote! {};
        }
    ...
    pub fn emit_supertrait_bound_path(&self, trait_path: &str, type_args: &[IrType]) -> TokenStream {
        let bound = incan_ir::decl::IrTraitBound::with_type_args(trait_path, type_args.to_vec());
        self.emit_trait_bound(&bound)
    }

    /// Emit visibility modifier.
    pub fn emit_visibility(&self, vis: &Visibility) -> TokenStream {
        match vis {
    ...
    pub fn emit_pattern(&self, pattern: &Pattern) -> TokenStream {
    ...
    pub(in crate::emit) fn emit_pattern_with_mutable_bindings(
        &self,
        pattern: &Pattern,
        ...
        for binding in &arm.bindings {
            let value_expr = if for_guard {
                let guard_uses_binding = arm
                    .guard
                    .as_ref()
                    .is_some_and(|guard| incan_ir::scanners::expr_uses_binding_name(guard, &binding.name));
    ...
    pub fn emit_match_arm_body(&self, arm: &MatchArm, result_ty: Option<&IrType>) -> Result<TokenStream, EmitError> {
