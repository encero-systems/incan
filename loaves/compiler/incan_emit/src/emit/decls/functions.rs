...
impl<'a> IrEmitter<'a> {
    ...
    fn exact_float_ingress_param_names(func: &incan_ir::decl::IrFunction, boundary_is_public: bool) -> HashSet<String> {
        ...
        func.params
            ...
            .map(|param| {
                ...
                let is_borrowed = mutable_params_are_borrowed
                    && !matches!(param.mutability, incan_ir::types::Mutability::OwnedMutable)
                    && (mutated_params.contains(&param.name)
                        || matches!(param.mutability, incan_ir::types::Mutability::Mutable));
    ...
    fn emit_source_callable_blanket_impl(&self, trait_decl: &incan_ir::decl::IrTrait) -> TokenStream {
    ...
    pub(in crate::emit) fn emit_method_return_type(
        &self,
        return_type: &IrType,
        method_type_params: &[incan_ir::decl::IrTypeParam],
    ) -> TokenStream {
    ...
    fn rewrite_borrowed_param_types_in_expr(expr: &mut incan_ir::expr::IrExpr, borrowed: &HashMap<String, IrType>) {
        ...
        match &mut expr.kind {
            ...
            IrExprKind::Generator { element, clauses } => {
                ...
                for clause in clauses {
                    match clause {
                        incan_ir::expr::IrGeneratorClause::For { iterable, .. } => {
                            Self::rewrite_borrowed_param_types_in_expr(iterable, borrowed);
                        }
                        incan_ir::expr::IrGeneratorClause::If(condition) => {
                            Self::rewrite_borrowed_param_types_in_expr(condition, borrowed);
                        }
                    }
                }
            ...
            IrExprKind::Format { parts } => {
                for part in parts {
                    if let incan_ir::expr::FormatPart::Expr { expr, .. } = part {
                        Self::rewrite_borrowed_param_types_in_expr(expr, borrowed);
                    }
                }
            }
    ...
    fn borrowed_function_clone(
        ...
        helper_name: String,
        indices: &[usize],
    ) -> Option<incan_ir::decl::IrFunction> {
        ...
        helper.visibility = incan_ir::decl::Visibility::Private;
    ...
    pub(in crate::emit) fn emit_borrowed_function_adapter(
        &self,
        func: &incan_ir::decl::IrFunction,
    ...
    pub(in crate::emit) fn emit_result_observer_borrowed_method(
        &self,
        func: &incan_ir::decl::IrFunction,
    ) -> Result<Option<TokenStream>, EmitError> {
    ...
    pub(in crate::emit) fn emit_function(&self, func: &incan_ir::decl::IrFunction) -> Result<TokenStream, EmitError> {
        ...
        let params: Vec<TokenStream> = func
            ...
            .map(|p| {
                ...
                if p.is_self {
                    if matches!(p.mutability, incan_ir::types::Mutability::Mutable) {
                        quote! { &mut self }
                    } else {
                        quote! { &self }
                    }
                } else if matches!(p.mutability, incan_ir::types::Mutability::OwnedMutable) {
                    if param_is_used {
                        quote! { mut #pname: #pty }
                    } else {
                        quote! { _: #pty }
                    ...
                    || matches!(p.mutability, incan_ir::types::Mutability::Mutable)
                {
                    if !param_is_used {
                        match &p.ty {
    ...
    pub(in crate::emit) fn emit_method(&self, func: &incan_ir::decl::IrFunction) -> Result<TokenStream, EmitError> {
    ...
    fn emit_extern_method(&self, func: &incan_ir::decl::IrFunction) -> Result<TokenStream, EmitError> {
        let Some(ref module_path) = self.rust_module_path else {
            return Err(EmitError::Unsupported(format!(
    ...
    pub(in crate::emit) fn emit_trait_method(
        &self,
        func: &incan_ir::decl::IrFunction,
    ...
    fn emit_rust_attributes(&self, attributes: &[incan_ir::decl::IrRustAttribute]) -> Vec<TokenStream> {
        attributes
            .iter()
            .map(|a| {
    ...
    pub(in crate::emit) fn emit_rust_lint_allows(&self, allows: &[IrRustLintAllow]) -> Vec<TokenStream> {
        if allows.is_empty() {
            return Vec::new();
        }
    ...
    fn collect_function_used_names(func: &incan_ir::decl::IrFunction) -> HashSet<String> {
...
mod tests {
    use super::*;
    use incan_ir::decl::{FunctionParam, IrFunction, IrTrait, IrTypeParam, Visibility};
    use incan_ir::expr::{IrExprKind, VarAccess, VarRefKind};
    use incan_ir::stmt::{IrStmt, IrStmtKind};
    use incan_ir::{FunctionRegistry, Mutability, TypedExpr};
