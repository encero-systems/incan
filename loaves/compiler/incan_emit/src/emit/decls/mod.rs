...
use incan_ir::decl::{IrDecl, IrDeclKind, IrImportOrigin, IrImportQualifier, Visibility};
...
impl<'a> IrEmitter<'a> {
    ...
    pub fn emit_decl(&self, decl: &IrDecl) -> Result<TokenStream, EmitError> {
        match &decl.kind {
            ...
            IrDeclKind::SymbolAlias {
                ...
                if let Some(identity) = target_canonical {
                    if !incan_ir::decl::is_projected_source_symbol(identity) {
                ...
                let projection_reexport = (matches!(visibility, incan_ir::decl::Visibility::Public)
    ...
    fn emit_static(
        &self,
        visibility: &incan_ir::decl::Visibility,
        name: &str,
        provenance: &incan_ir::decl::IrStaticProvenance,
        ty: &IrType,
        value: &incan_ir::TypedExpr,
    ) -> Result<TokenStream, EmitError> {
        let vis = self.emit_visibility(visibility);
        let name_ident = self.rust_static_declaration_ident(name, provenance)?;
        let rust_facing_alias = if matches!(provenance, incan_ir::decl::IrStaticProvenance::Source(_))
            && !matches!(visibility, incan_ir::decl::Visibility::Private)
        {
    ...
    fn emit_const_struct_value(
        &self,
        name: &str,
        fields: &[(String, incan_ir::TypedExpr)],
    ) -> Result<TokenStream, EmitError> {
        ...
        let mut provided: std::collections::HashMap<&str, &incan_ir::TypedExpr> = std::collections::HashMap::new();
    ...
    pub fn is_incan_source_stdlib_import(
    ...
    fn emit_import(
        ...
        path: &[String],
        alias: &Option<String>,
        items: &[incan_ir::decl::IrImportItem],
    ) -> Result<TokenStream, EmitError> {
        ...
        let is_public_reexport = !matches!(visibility, incan_ir::decl::Visibility::Private);
        ...
        if let Some(alias_name) = alias {
            ...
            let should_reexport_item = |item: &incan_ir::decl::IrImportItem| {
                ...
                .map(|item| {
                    ...
                    let reexport_carries_alias = renames_shared_projection
                        ...
                        && item
                            .canonical
                            .as_ref()
                            .is_some_and(incan_ir::decl::is_projected_source_symbol)
                        && source_binding != emitted_name;
...
impl crate::IrEmitter<'_> {
    ...
    pub fn stdlib_import_item_emitted_name(&self, path: &[String], item: &incan_ir::decl::IrImportItem) -> String {
...
mod tests {
    ...
    use incan_ir::decl::IrImportItem;
    use incan_ir::types::IrType;
