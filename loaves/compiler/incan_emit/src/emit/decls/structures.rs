...
impl<'a> IrEmitter<'a> {
    ...
    pub(in crate::emit) fn emit_struct(&self, s: &IrStruct) -> Result<TokenStream, EmitError> {
                ...
                for field in &s.fields {
                    ...
                    if let Some(default) = field.default.as_ref() {
                        let default = if matches!(field.ty, IrType::String)
                            && let incan_ir::expr::IrExprKind::BinOp {
                                op: incan_ir::expr::BinOp::Add,
                                left,
                                right,
                            } = &default.kind
                            && let Some(static_default) = self.try_emit_static_str_add(left, right)?
                        {
                            ...
                            self.emit_expr_for_use(
                                default,
                                crate::ownership::ValueUseSite::StructField {
    ...
    fn validate_value_enum_variant_is_unit(e: &IrEnum, variant: &incan_ir::decl::EnumVariant) -> Result<(), EmitError> {
    ...
    fn enum_variant_match_pattern(variant: &incan_ir::decl::EnumVariant) -> TokenStream {
        let vname = Self::rust_ident(&variant.name);
