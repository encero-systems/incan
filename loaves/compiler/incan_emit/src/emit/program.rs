...
struct GeneratedUseAnalyzer<'program> {
    ...
    impls_by_target: HashMap<String, Vec<&'program incan_ir::decl::IrImpl>>,
...
impl<'program> GeneratedUseAnalyzer<'program> {
            ...
            IrDeclKind::Enum(e) => {
                ...
                for variant in &e.variants {
                    match &variant.fields {
                        incan_ir::decl::VariantFields::Unit => {}
                        incan_ir::decl::VariantFields::Tuple(types) => {
                            for ty in types {
                                self.scan_type(ty);
                            }
                        }
                        incan_ir::decl::VariantFields::Struct(fields) => {
                            for field in fields {
                                self.scan_type(&field.ty);
                            }
                        }
    ...
    fn scan_impl(&mut self, impl_block: &'program incan_ir::decl::IrImpl) {
    ...
    fn scan_impl_blocks(&mut self, impl_blocks: &[&'program incan_ir::decl::IrImpl]) {
        loop {
    ...
    fn impl_method_body_is_emitted(&self, impl_block: &'program incan_ir::decl::IrImpl, method: &IrFunction) -> bool {
...
impl<'a> IrEmitter<'a> {
    ...
    fn checked_c_wrapper_parameter_type(
        ...
        match ty {
            ...
            IrCheckedCType::Resource { access, resource } => {
                ...
                match access {
                    incan_frontend::typechecker::CResourceAccess::Owned => quote! { #resource },
                    incan_frontend::typechecker::CResourceAccess::Borrowed => quote! { &#resource },
                    incan_frontend::typechecker::CResourceAccess::BorrowedMut => quote! { &mut #resource },
                }
            }
    ...
    fn checked_c_ffi_argument(
        ...
        match ty {
            ...
            IrCheckedCType::Resource { access, .. } => match access {
                incan_frontend::typechecker::CResourceAccess::Owned => quote! { #name.into_raw() },
                incan_frontend::typechecker::CResourceAccess::Borrowed
                | incan_frontend::typechecker::CResourceAccess::BorrowedMut => quote! { #name.as_raw() },
            },
    ...
    pub(in crate::emit) fn newtype_constraint_predicate(
    ...
    pub(in crate::emit) fn newtype_underlying_is_float(underlying: &IrType) -> bool {
    ...
    fn field_overlay_value_type_from_struct(strukt: &incan_ir::decl::IrStruct) -> Option<IrType> {
    ...
    pub fn collect_union_types_from_type(ty: &IrType, out: &mut HashMap<String, IrType>) {
    ...
    pub fn collect_union_types_from_program(program: &IrProgram) -> HashMap<String, IrType> {
    ...
    fn emit_generated_union_member_type(&self, ty: &IrType) -> TokenStream {
        match ty {
            ...
            IrType::NamedGeneric(name, args) if name == incan_ir::types::IR_UNION_TYPE_NAME => {
    ...
    pub fn emit_program(&mut self, program: &IrProgram) -> Result<String, EmitError> {
        ...
        // that alias's `pub use`; an ordinary import of the same declaration stands aside for it.
        {
            ...
            for declaration in &program.declarations {
                if let incan_ir::decl::IrDeclKind::SymbolAlias {
                    visibility: incan_ir::decl::Visibility::Public,
                    target_canonical: Some(identity),
                    ..
                } = &declaration.kind
                    && incan_ir::decl::is_projected_source_symbol(identity)
                {
    ...
    pub fn callable_name_use_facts_for_program(
        program: &IrProgram,
        externally_reachable_items: &HashSet<String>,
        preserve_public_items: bool,
...
mod tests {
    ...
    fn emit_qualified_marker_impl(binding: &str, trait_path: &str) -> Result<String, super::EmitError> {
        use incan_ir::decl::{IrImpl, IrImportOrigin, IrImportQualifier, IrStruct, IrStructKind, Visibility};
        use incan_ir::{IrDecl, IrDeclKind};
