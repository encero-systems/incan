...
//!
//! ## See also
//! - [`crate::conversions`]: conversion policy for emitted Rust
...
pub const SERDE_SERIALIZE_DERIVE: &str = "serde::Serialize";
/// Rust derive path emitted for Serde deserialization.
pub const SERDE_DESERIALIZE_DERIVE: &str = "serde::Deserialize";
...
use incan_frontend::module::logical_source_path_candidates;
...
#[derive(Debug, Clone)]
pub struct ExternalOrdinalValueEnum {
...
#[derive(Debug, Clone)]
pub struct ExternalOrdinalCustomKey {
    /// Dependency key used as the generated Rust crate alias.
    pub dependency_key: String,
    /// Exported type name.
    pub name: String,
...
#[derive(Debug, Clone)]
pub struct CallableNameResolution {
...
pub struct CallableNameUseFacts {
    pub signature_keys: HashSet<String>,
    pub function_arg_signature_keys: HashSet<String>,
    pub generic_trait_used: bool,
}
...
pub struct GeneratedUseAnalysis {
    /// Top-level declaration names that must be emitted.
    pub reachable_items: HashSet<String>,
    ...
    pub used_extension_trait_imports: HashSet<String>,
    /// Struct/class fields that are read by emitted code.
    pub read_fields: HashSet<(String, String)>,
...
pub enum StructConstructorSurface {
...
pub struct StructConstructorMetadata {
    ...
    field_defaults: HashMap<String, incan_ir::IrExpr>,
...
pub struct IrEmitter<'a> {
    ...
    struct_field_defaults: std::collections::HashMap<(String, String), incan_ir::IrExpr>,
    ...
    native_nominal_origins:
        std::collections::BTreeMap<String, incan_frontend::library_manifest::NominalTypeOriginExport>,
...
impl<'a> IrEmitter<'a> {
    ...
    pub fn emit_named_constructor_arguments(
        &self,
        target_name: &str,
        ...
        for field_name in metadata.constructor_fields() {
            ...
            let value = if let Some(value) = provided.get(field_name.as_str()) {
                ...
                if metadata.uses_constructor_function() {
                    ...
                    self.emit_expr_for_use(default_expr, crate::ownership::ValueUseSite::StructField { target_ty })?
                }
            } else {
                return Err(EmitError::Unsupported(format!(
    ...
    pub fn set_callable_name_current_module_path(&mut self, path: Vec<String>) {
    ...
    pub fn set_canonical_function_registry(&mut self, registry: FunctionRegistry) {
        self.canonical_function_registry = Some(registry);
    }

    /// Return the canonical function registry used for callable-name lookups.
    pub fn canonical_function_registry(&self) -> &FunctionRegistry {
    ...
    pub fn canonical_stdlib_function_identity(&self, path: &[String]) -> Option<&CanonicalSymbolId> {
        let normalized_path;
    ...
    pub fn member_projection(&self, owner: &str, source_name: &str) -> Option<String> {
    ...
    pub fn set_callable_name_resolutions(&mut self, resolutions: HashMap<String, CallableNameResolution>) {
    ...
    pub fn set_callable_name_used_signature_keys(&mut self, keys: HashSet<String>) {
        self.callable_name_used_signature_keys = keys;
    }

    /// Configure the local callable registry used by generated callable-name helpers.
    pub fn set_callable_name_local_registry(&mut self, registry: FunctionRegistry) {
    ...
    pub fn add_callable_name_resolutions_for_program(
    ...
    pub fn callable_name_helper_ident(key: &str) -> proc_macro2::Ident {
    ...
    pub fn callable_name_register_ident(key: &str) -> proc_macro2::Ident {
    ...
    pub fn callable_name_registry_ident(key: &str) -> proc_macro2::Ident {
    ...
    pub fn callable_name_signature_key(params: &[IrType], ret: &IrType) -> Option<String> {
    ...
    pub fn local_callable_name_signature_keys(&self) -> HashSet<String> {
    ...
    pub fn callable_name_local_registry(&self) -> &FunctionRegistry {
        self.callable_name_local_registry
            .as_ref()
            .unwrap_or(self.function_registry)
    }
    ...
    pub(in crate::emit) fn call_signature_type_matches(&self, left: &IrType, right: &IrType) -> bool {
        if left == right {
            return true;
        }
    ...
    pub(in crate::emit) fn resolve_type_aliases_for_emit(&self, ty: &IrType) -> IrType {
    ...
    pub fn emit_module_static_init_call(&self) -> TokenStream {
    ...
    pub fn set_imported_static_init_bindings(&self, bindings: HashSet<String>) {
        *self.imported_static_init_bindings.borrow_mut() = bindings;
    }

    /// Replace imported static modules that need module-level init calls.
    pub fn set_imported_static_module_init_bindings(&self, bindings: Vec<String>) {
    ...
    pub fn imported_static_init_ident(name: &str) -> proc_macro2::Ident {
    ...
    pub fn static_needs_imported_init_call(&self, name: &str) -> bool {
    ...
    pub fn static_reference_needs_imported_init_call(
        &self,
        name: &str,
        reference_kind: incan_ir::expr::IrStaticReferenceKind,
    ) -> bool {
        matches!(reference_kind, incan_ir::expr::IrStaticReferenceKind::Source)
            && self.static_needs_imported_init_call(name)
    }

    /// Return whether a static binding needs any imported static init support.
    pub fn static_needs_imported_init_import(&self, name: &str) -> bool {
        self.static_needs_imported_init_call(name)
            || self
    ...
    pub fn emit_static_init_call_for_reference(
        &self,
        name: &str,
        reference_kind: incan_ir::expr::IrStaticReferenceKind,
    ) -> TokenStream {
    ...
    pub fn result_observer_borrowed_method_name() -> &'static str {
    ...
    pub fn borrowed_function_adapter_name(name: &str, indices: &[usize]) -> String {
    ...
    pub fn set_result_observer_callable_types(&self, callable_types: HashSet<String>) {
    ...
    pub fn set_borrowed_function_adapters(&self, adapters: HashSet<(String, Vec<usize>)>) {
    ...
    pub fn needs_result_observer_callable_helper(&self, type_name: &str) -> bool {
    ...
    pub fn claim_result_observer_callable_helper(&self, type_name: &str) -> bool {
    ...
    pub fn needs_borrowed_function_adapter(&self, name: &str, indices: &[usize]) -> bool {
    ...
    pub fn is_internal_module_root(&self, name: &str) -> bool {
        self.internal_module_roots.contains(name)
    }

    /// Check if a full module path is known internally.
    pub fn is_internal_module_path(&self, segments: &[String]) -> bool {
    ...
    fn rust_static_reference_ident(
        ...
        match reference_kind {
            incan_ir::expr::IrStaticReferenceKind::Source => self.rust_source_static_ident(name),
            incan_ir::expr::IrStaticReferenceKind::CompilerGenerated => Ok(Self::rust_generated_static_ident(name)),
        }
    }
    ...
    pub fn set_external_ordinal_value_enums(&mut self, enums: Vec<ExternalOrdinalValueEnum>) {
    ...
    pub fn set_external_ordinal_custom_keys(&mut self, keys: Vec<ExternalOrdinalCustomKey>) {
    ...
    pub fn set_generated_use_analysis(&self, analysis: GeneratedUseAnalysis) {
        *self.generated_use_analysis.borrow_mut() = analysis;
    }

    /// True when a top-level declaration with `name` should be emitted.
    pub fn should_emit_decl_name(&self, name: &str, visibility: &Visibility) -> bool {
    ...
    pub fn should_emit_import_binding(&self, name: &str) -> bool {
    ...
    pub fn should_emit_extension_trait_import(&self, name: &str) -> bool {
    ...
    pub fn should_emit_method(&self, target_type: &str, method_name: &str, visibility: &Visibility) -> bool {
    ...
    pub fn should_emit_enum_message_method(&self, enum_name: &str, visibility: &Visibility) -> bool {
    ...
    pub fn struct_constructor_surface(&self, s: &IrStruct) -> StructConstructorSurface {
    ...
    pub fn should_expect_private_field_dead_code(
        &self,
        struct_name: &str,
        field_name: &str,
        visibility: &Visibility,
    ...
    pub(in crate::emit) fn emit_dependency_type_path(&self, name: &str) -> Option<TokenStream> {
    ...
    /// imports.
    pub(in crate::emit) fn emit_public_dependency_type_path(&self, name: &str) -> Option<TokenStream> {
    ...
    pub fn seed_nominal_metadata_from_program(&mut self, program: &IrProgram) {
    ...
    pub fn seed_dependency_nominal_metadata_from_program(&mut self, program: &IrProgram) {
    ...
    pub fn set_native_nominal_origins(
        &mut self,
        origins: std::collections::BTreeMap<String, incan_frontend::library_manifest::NominalTypeOriginExport>,
    ) {
    ...
    pub fn seed_public_dependency_nominal_metadata(
        &mut self,
        index: &LibraryManifestIndex,
        routes: &HashMap<String, HashMap<String, String>>,
        plan: Option<&incan_frontend::provider::ProviderPlan>,
    ) -> Result<(), EmitError> {
        ...
        for library in index.known_libraries() {
            ...
            let projected = incan_frontend::library_manifest::with_checked_native_unions(
            ...
            let projected =
                incan_frontend::library_manifest::with_native_nominal_origins(projected, &self.native_nominal_origins);
            manifests.insert(
                library.clone(),
                incan_frontend::library_manifest::with_checked_type_routes(projected, routes.get(&library)),
            );
        }
            ...
            if let Some(api) = manifest.contract_metadata.api.as_ref() {
                for module in &api.modules {
                    ...
                    for declaration in module.declarations.iter().filter(|declaration| {
                        incan_frontend::api_metadata::checked_api_declaration_is_public_namespace_member(declaration)
                    }) {
    ...
    fn api_declaration_for_target_path<'api>(
        api: &'api incan_frontend::api_metadata::CheckedApiMetadataPackage,
        target_path: &[String],
    ...
    pub fn seed_sdk_provider_manifest_metadata(
        &mut self,
        manifest: &LibraryManifest,
        plan: Option<&incan_frontend::provider::ProviderPlan>,
        ...
        let self_owned =
            incan_frontend::library_manifest::with_self_owned_native_unions(manifest.clone(), &provider_crate);
        let library = self_owned.name.clone();
        let manifest =
            &incan_frontend::library_manifest::with_checked_native_unions(self_owned, &library, plan, routes)
                .map_err(EmitError::InternalInvariant)?;
    ...
    pub fn set_compiled_sdk_module_paths(&mut self, paths: HashSet<Vec<String>>) {
        self.compiled_sdk_module_paths = paths;
    }

    /// Return whether a dependency module is supplied by a linked compiled SDK provider.
    pub(in crate::emit) fn is_compiled_sdk_module_path(&self, module: &[String]) -> bool {
    ...
    pub(in crate::emit) fn is_compiled_sdk_emission_path(&self, path: &[String]) -> bool {
    ...
    pub(in crate::emit) fn is_compiled_sdk_type_in_module(&self, type_name: &str, source_module: &str) -> bool {
    ...
    fn seed_compiled_provider_export_metadata(
        &mut self,
        provider_crate: &str,
        models: &[incan_frontend::library_manifest::ModelExport],
        classes: &[incan_frontend::library_manifest::ClassExport],
        enums: &[incan_frontend::library_manifest::EnumExport],
    ...
    fn seed_compiled_provider_factory_metadata(
        &mut self,
        functions: &[incan_frontend::library_manifest::FunctionExport],
        models: &[incan_frontend::library_manifest::ModelExport],
        classes: &[incan_frontend::library_manifest::ClassExport],
    ) {
    ...
    fn register_manifest_method_metadata(
        ...
        type_params: &[incan_frontend::library_manifest::TypeParamExport],
    ) {
                    ...
                    params: method
                        ...
                        .map(|param| FunctionParam {
                            ...
                            kind: match param.kind {
                                ParamKindExport::Normal => incan_frontend::ast::ParamKind::Normal,
                                ParamKindExport::RestPositional => incan_frontend::ast::ParamKind::RestPositional,
                                ParamKindExport::RestKeyword => incan_frontend::ast::ParamKind::RestKeyword,
                            },
        ...
        FunctionSignature {
            params: signature
                ...
                .map(|param| {
                    ...
                    let ty = match kind {
                        incan_frontend::ast::ParamKind::Normal => base_ty,
                        incan_frontend::ast::ParamKind::RestPositional => IrType::List(Box::new(base_ty)),
                        incan_frontend::ast::ParamKind::RestKeyword => {
                    ...
                    FunctionParam {
                        ...
                        default: Self::manifest_param_default_to_ir_expr(library, param)
                            .map(incan_ir::decl::FunctionParamDefault::source),
                    }
                })
                .collect(),
    ...
    fn manifest_type_ref_to_ir_type(ty: &TypeRef) -> IrType {
        incan_ir::types::ir_type_from_projected_manifest(ty, &|ordinary| {
    ...
    pub fn struct_constructor_metadata_for_fields(
        &self,
        name: &str,
        fields: &[(String, TypedExpr)],
    ...
    pub fn public_dependency_constructor_metadata_for_path(
        &self,
        path: &[String],
        fields: &[(String, TypedExpr)],
    ) -> Option<&StructConstructorMetadata> {
    ...
    pub fn method_signature_for_receiver(&self, receiver_ty: &IrType, method_name: &str) -> Option<&FunctionSignature> {
        match receiver_ty {
    ...
    pub fn method_call_signature_for_receiver(
        &self,
        receiver_ty: &IrType,
    ...
    pub fn specialized_method_signature_for_receiver(
        &self,
        receiver_ty: &IrType,
        method_name: &str,
    ) -> Option<FunctionSignature> {
    ...
    pub fn specialize_signature_by_result_target(
    ...
    pub fn specialize_signature_by_receiver_args(
        signature: &FunctionSignature,
        receiver_ty: &IrType,
    ) -> Option<FunctionSignature> {
    ...
    pub fn type_is_user_enum(&self, ty: &IrType) -> bool {
        match ty {
            IrType::Enum(_) => true,
            IrType::Struct(name) | IrType::NamedGeneric(name, _) => {
...
mod tests {
    ...
    use incan_ir::expr::{IrExprKind, IrStaticReferenceKind, TypedExpr};
    use incan_ir::stmt::{IrStmt, IrStmtKind};
    use incan_ir::{FunctionRegistry, IrProgram, IrType};
    ...
    fn manifest_exporting_a_native_union(
        library: &str,
        owner: incan_frontend::library_manifest::NativeUnionOwnerExport,
    ) -> LibraryManifest {
        let mut manifest = LibraryManifest::new(library, "0.6.0");
        manifest
            .exports
            .enums
            .push(incan_frontend::library_manifest::EnumExport {
                ...
                variants: vec![incan_frontend::library_manifest::EnumVariantExport {
                    name: "Carried".to_string(),
                    canonical: None,
                    fields: vec![incan_frontend::library_manifest::TypeRef::NativeUnion(
                        incan_frontend::library_manifest::NativeUnionExport {
                            owner,
                            rust_name: "__IncanUnionProbe".to_string(),
                            members: vec![incan_frontend::library_manifest::TypeRef::Named {
                                name: "int".to_string(),
                                origin: None,
                            }],
    ...
    fn an_sdk_provider_binds_the_native_unions_it_owns() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let manifest = manifest_exporting_a_native_union(
            "incan_stdlib_codecs",
            incan_frontend::library_manifest::NativeUnionOwnerExport::ContainingArtifact,
        );
    ...
    fn sdk_provider_seeding_refuses_a_union_owned_by_an_unadmitted_artifact() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        let manifest = manifest_exporting_a_native_union(
            "incan_stdlib_codecs",
            incan_frontend::library_manifest::NativeUnionOwnerExport::SelectedArtifact(
                incan_frontend::provider::ProviderIdentity {
    ...
    fn qualified_rust_receiver_does_not_inherit_same_named_incan_method_signature() -> Result<(), String> {
        ...
        emitter.method_signatures.insert(
            ...
            FunctionSignature {
                params: vec![incan_ir::decl::FunctionParam {
                    name: "value".to_string(),
                    ty: IrType::String,
                    mutability: incan_ir::Mutability::Immutable,
                    is_self: false,
        ...
        let signature_with_source_default = FunctionSignature {
            params: vec![incan_ir::decl::FunctionParam {
                ...
                is_self: false,
                kind: incan_frontend::ast::ParamKind::Normal,
                default: Some(incan_ir::decl::FunctionParamDefault::source(TypedExpr::new(
                    IrExprKind::Int(8080),
                    IrType::Int,
                ))),
            }],
    ...
    fn rust_generated_static_ident_uses_uppercase_global_style() {
        let registry = incan_ir::FunctionRegistry::new();
    ...
    fn manifest_constructor_metadata_exports_only_public_fields_for_private_defaults() {
        use incan_frontend::library_manifest::{FieldExport, FieldVisibilityExport, ParamDefaultExport, TypeRef};
