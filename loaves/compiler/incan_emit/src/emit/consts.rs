...
//! - [`crate::emit::program`]: where const string folding is initialized
...
impl<'a> IrEmitter<'a> {
    ...
    pub fn validate_const_emittable(&self, name: &str, ty: &IrType, value: &TypedExpr) -> Result<(), EmitError> {
    ...
    fn validate_const_expr_kind(kind: &IrExprKind) -> Result<(), EmitError> {
        ...
        match kind {
            ...
            K::Call {
                ...
                && args.len() == 1
                && matches!(args[0].kind, incan_ir::expr::IrCallArgKind::Positional) =>
            {
    ...
    pub fn resolve_static_str_const(
        name: &str,
        const_exprs: &std::collections::HashMap<String, TypedExpr>,
        visiting: &mut std::collections::HashSet<String>,
        cache: &mut std::collections::HashMap<String, String>,
