...
impl EmittedDeclarationTypes {
    ...
    pub fn apply(&self, manifest: &mut LibraryManifest) {
...
fn capture_local_nominal_bindings(
    ...
    let public_nominals = identities
        ...
        .filter(|entry| {
            matches!(
                entry.kind,
                incan_frontend::library_manifest::ExportIdentityKind::Model
                    | incan_frontend::library_manifest::ExportIdentityKind::Class
                    | incan_frontend::library_manifest::ExportIdentityKind::Enum
                    | incan_frontend::library_manifest::ExportIdentityKind::Newtype
    ...
    for declaration in &module.declarations {
        ...
        let Some(source_name) = incan_frontend::api_metadata::api_declaration_public_name(declaration) else {
            continue;
        };
    ...
    for declaration in &program.declarations {
        let IrDeclKind::Import {
            items,
            origin: incan_ir::decl::IrImportOrigin::Standard,
            ..
        } = &declaration.kind
        else {
            continue;
...
impl IrEmitter<'_> {
    ...
    pub fn emitted_native_union_types(&self) -> HashMap<String, IrType> {
    ...
    pub fn capture_native_union_metadata(
        &self,
        module: &CheckedApiMetadata,
        program: &IrProgram,
        ...
        for declaration in &module.declarations {
            ...
            if !replacements.is_empty() || !bridge_roots.is_empty() {
                captured.push(EmittedDeclarationTypes {
                    ...
                    anchor: anchor.clone(),
                    source_name: incan_frontend::api_metadata::api_declaration_public_name(declaration)
                        .ok_or_else(|| {
    ...
    fn native_union_definition(
        ...
        let members = members
            .iter()
            .map(|member| {
                incan_ir::types::manifest_type_ref_from_ir(member)
                    .map(|ty| incan_frontend::library_manifest::with_checked_type_origins(ty, origins))
    ...
    fn project_emitted_union_type(
        ...
        {
            let expanded =
                incan_ir::types::manifest_type_ref_from_ir(&lowered).map_err(EmitError::InternalInvariant)?;
...
fn pair_function<'a>(
    params: &'a [incan_frontend::library_manifest::ParamExport],
    ret: &'a TypeRef,
...
fn pair_fields<'a>(
    fields: &'a [incan_frontend::library_manifest::FieldExport],
    lowered: &'a [incan_ir::decl::StructField],
    pairs: &mut Vec<(&'a TypeRef, &'a IrType)>,
) {
    for field in fields {
...
pub fn preserve_native_aliases(
    ...
    for module in &mut api.modules {
        for declaration in &mut module.declarations {
            ...
            let Some(incan_frontend::library_manifest_index::LibraryManifestIndexEntry::Loaded {
                manifest: provider,
                ..
            ...
            {
                alias.projected_function = Some(incan_frontend::api_metadata::ApiProjectedFunction {
                    source_path: alias.target_path.clone(),
                    callable: incan_frontend::api_metadata::ApiCallableMetadata {
    ...
    incan_frontend::api_metadata::materialize_api_alias_projections(&mut api.modules);
            ...
            for declaration in &module.declarations {
                ...
                if contains_native_union(&alias.projected_function) {
                    export.projected_function = alias
                        .projected_function
                        .as_ref()
                        .map(incan_frontend::api_metadata::function_export_from_api_projected);
                }
            }
        }
    }
...
fn native_function_at_canonical(
...
) -> Result<Option<incan_frontend::library_manifest::FunctionExport>, String> {
    ...
    Ok(contains_native_union(declaration).then(|| incan_frontend::api_metadata::function_export_from_api(declaration)))
}
...
fn foreign_native_function_projection(
    ...
    use incan_frontend::library_manifest::ExportIdentityProjection;
    let entry = graph
        .exports
        .iter()
        .find(|entry| {
...
type DeclaredAliasSurface = (
    Option<TypeRef>,
    Option<incan_frontend::library_manifest::FunctionExport>,
);
...
fn declared_alias_surface(manifest: &LibraryManifest, path: &[String]) -> Result<Option<DeclaredAliasSurface>, String> {
    ...
    let Some(namespace) = incan_frontend::api_metadata::checked_api_public_namespace(api, module_path) else {
        return Ok(None);
    };
    ...
    let Some(declaration) = module.declarations.iter().find(|declaration| {
        incan_frontend::api_metadata::api_declaration_public_name(declaration) == Some(source_name.as_str())
    }) else {
        return Ok(None);
    };
    Ok(match declaration {
        ...
        ApiDeclaration::Function(function) => Some((
            None,
            Some(incan_frontend::api_metadata::function_export_from_api(function)),
        )),
        _ => None,
    })
}
...
mod tests {
    ...
    fn emitted_module(source: &str, module: &str) -> Result<EmittedModule, Box<dyn std::error::Error>> {
        ...
        let identities = LibraryIdentityGraph::from_checked_exports(
            "producer",
            &incan_frontend::library_exports::collect_checked_public_exports(&ast, &checker),
        );
    ...
    fn assert_native_function_alias_overloads(module: &str) -> TestResult {
        ...
        if module != "lib" {
            ...
            incan_frontend::api_metadata::materialize_checked_api_public_namespaces(
    ...
    fn assert_foreign_overload_projection(manifest: LibraryManifest, rust: &str) -> TestResult {
        ...
        let metadata = incan_frontend::library_manifest_index::LibraryArtifactMetadata::from_manifest_path(
            "admitted",
            "producer",
            path,
            root.to_path_buf(),
        );
        let index = incan_frontend::library_manifest_index::LibraryManifestIndex::from_entries(HashMap::from([(
            "admitted".to_string(),
        ...
        let plan = std::sync::Arc::new(incan_frontend::provider::ProviderPlan::from_resolved_inputs(
            index,
            None,
            None,
            None,
    ...
    fn assert_private_signature_bridge_projection(
        plan: &std::sync::Arc<incan_frontend::provider::ProviderPlan>,
    ) -> TestResult {
    ...
    fn assert_local_native_callable_projection(
        plan: &std::sync::Arc<incan_frontend::provider::ProviderPlan>,
    ) -> TestResult {
    ...
    fn assert_imported_native_overload_calls(
        plan: &std::sync::Arc<incan_frontend::provider::ProviderPlan>,
    ) -> TestResult {
        use incan_ir::{expr::IrExprKind, stmt::IrStmtKind};
...
"#
        ...
        let incan_frontend::library_manifest_index::LibraryManifestIndexEntry::Loaded { manifest, metadata } = entry
        else {
        ...
        checker.check_program(&ast).map_err(|errors| format!("{errors:?}"))?;
        for declaration in &ast.declarations {
            let incan_frontend::ast::Declaration::Function(function) = &declaration.node else {
                continue;
            };
            let Some(incan_frontend::ast::Statement::Return(Some(call))) =
                function.body.first().map(|statement| &statement.node)
            else {
                return Err("checked imported return missing".into());
            };
            let incan_frontend::ast::Expr::Call(callee, _, _) = &call.node else {
        ...
        let first = ast
            ...
            .find_map(|decl| match &decl.node {
                incan_frontend::ast::Declaration::Function(function) if function.name == "first" => Some(function),
                _ => None,
            })
            .ok_or("first declaration absent")?;
        let incan_frontend::ast::Statement::Return(Some(call)) = &first.body[0].node else {
        ...
        for namespace_group in [false, true].into_iter().filter(|nested| !nested || namespace) {
            ...
            let incomplete_plan = std::sync::Arc::new(incan_frontend::provider::ProviderPlan::for_library_index(index));
            let mut lowering = AstLowering::new_with_type_info(checker.type_info().clone());
    ...
    fn assert_decorated_native_callable_projection(
        plan: &std::sync::Arc<incan_frontend::provider::ProviderPlan>,
    ) -> TestResult {
...
"#,
            ...
            for lowered in &program.declarations {
                if matches!(lowered.kind, IrDeclKind::Impl(_))
                    || matches!(
                        &lowered.kind,
                        IrDeclKind::Static {
                            provenance: incan_ir::decl::IrStaticProvenance::CompilerGenerated,
                            ..
                        }
                    )
                {
    ...
    fn native_union_local_nominals_keep_module_identity_through_forwarding() -> TestResult {
        ...
        let semantic =
            incan_frontend::library_manifest::resolved_type_from_manifest_type_ref(&TypeRef::NativeUnion(forwarded));
        assert!(
            matches!(semantic, incan_frontend::symbols::ResolvedType::Generic(_, ref members)
            if members.iter().any(|member| matches!(member, incan_frontend::symbols::ResolvedType::Named(name)
    ...
    ) -> Result<incan_frontend::provider::ProviderPlan, Box<dyn std::error::Error>> {
