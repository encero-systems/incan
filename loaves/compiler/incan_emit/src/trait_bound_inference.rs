...
use crate::ownership::{
    RegularMethodArgumentContext, ValueUseSite, list_constructor_item_type, list_index_assignment_element_type,
    plan_list_constructor_source, regular_method_argument_use_site, value_use_requires_clone_bound,
...
pub struct ImplementationBoundRequirement {
...
pub fn infer_trait_bounds(program: &mut IrProgram) {
    ...
    let trait_decls: HashMap<String, incan_ir::decl::IrTrait> = program
        .declarations
        .iter()
...
pub fn collect_local_implementation_bound_requirements(program: &IrProgram) -> Vec<ImplementationBoundRequirement> {
    program
        .declarations
        .iter()
            ...
            IrDeclKind::Enum(e) if e.derives.iter().any(|derive| derive == tb::CLONE) => {
                ...
                for variant in &e.variants {
                    match &variant.fields {
                        ...
                        incan_ir::decl::VariantFields::Tuple(items) => {
                            for item in items {
                    ...
                    RegularMethodArgumentContext {
                        ...
                        has_incan_method_signature: matches!(arg_policy, MethodCallArgPolicy::SourceOwned)
                            || matches!(
                                dispatch,
                                Some(
                                    incan_ir::expr::IrMethodDispatch::Trait(_)
                                        | incan_ir::expr::IrMethodDispatch::SourceProjection(_)
                                )
                            ),
...
fn collect_inferred_bounds_for_callable(
    ...
    trait_decls: &HashMap<String, incan_ir::decl::IrTrait>,
    function_bounds: &mut HashMap<String, Vec<IrTypeParam>>,
    function_params: &mut HashMap<String, Vec<FunctionParam>>,
) {
    if type_params.is_empty() {
...
/// Record the bounds one trait bound implies, walking its type arguments and the traits it extends without
/// revisiting a trait already seen.
...
mod tests {
    use super::*;
    use incan_ir::decl::{FunctionParam, IrDecl, IrDeclKind, IrImpl, Visibility};
    use incan_ir::expr::{
        FormatStyle, IrCallArgKind, IrMethodDispatch, IrTraitDispatch, Literal, MethodCallArgPolicy, VarAccess,
    };
    use incan_ir::{FunctionRegistry, FunctionSignature, Mutability, TypedExpr};
