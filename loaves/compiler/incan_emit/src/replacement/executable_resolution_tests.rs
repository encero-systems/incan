...
use crate::replacement::{ReplacementExecutionGraph, execute_free_function};
use incan_frontend::body_ir::build_body_ir_module_v0;
use incan_frontend::executable_resolution::{ExecutableResolutionError, ResolvedExecutableModules};
use incan_frontend::library_exports::collect_checked_public_exports;
use incan_frontend::library_manifest::published_layout::{executable_surface_path, public_executable_identities};
use incan_frontend::library_manifest::{ExecutableRepresentationExport, LibraryManifest};
...
fn resolve_executable_requirements(
    ...
    let plan = incan_frontend::provider::ProviderPlan::from_resolved_inputs(index.clone(), None, None, None, [])
        .map_err(|error| ExecutableResolutionError::DependencyArtifact {
            library: "fixture".into(),
            version: "1.2.3".into(),
            reason: error.to_string(),
        })?;
    incan_frontend::executable_resolution::resolve_executable_requirements(&plan, required)
}
...
fn artifact_with_provider_plan(
    ...
    library: &str,
    source: &str,
    provider_plan: std::sync::Arc<incan_frontend::provider::ProviderPlan>,
    ...
    let unrepresentable = module
        .bodies
        .iter()
        .filter(|body| crate::replacement::validate_direct_body_profile(body).is_err())
...
fn two_packages_with_identical_module_paths_keep_distinct_physical_owners() -> Result<(), Box<dyn Error>> {
    ...
    let consumer = incan_frontend::body_ir::build_body_ir_module_v0_with_executable_context(
        &program,
        &path,
        checker.type_info(),
        &resolved.modules,
    ...
    let execution = crate::replacement::prepare_free_function_execution_in_graph(graph, "main", &[], None)?;
    assert_eq!(
        crate::replacement::execute_prevalidated_free_function(execution)?
            .value
            .observable_text(),
        "86"
    );
    ...
    let execution = crate::replacement::prepare_free_function_execution_in_graph(graph, "main", &[], None)?;
    let Err(error) = crate::replacement::execute_prevalidated_free_function(execution) else {
...
fn exercise_catalog_signature(type_facade: bool) -> Result<(), Box<dyn Error>> {
    ...
    let catalog_digest = incan_frontend::library_manifest::digest_provider_artifact(&catalog_root)?;
    ...
    let catalog_plan = || {
        incan_frontend::provider::ProviderPlan::from_resolved_inputs(
    ...
    let pricing_plan = incan_frontend::provider::ProviderPlan::from_resolved_inputs(
    ...
    let incan_frontend::library_manifest::TypeRef::Named {
        origin: Some(origin), ..
    ...
    let plan = incan_frontend::provider::ProviderPlan::from_resolved_inputs(
    ...
    let resolved = incan_frontend::executable_resolution::resolve_executable_requirements(&plan, &required)?;
    let consumer = incan_frontend::body_ir::build_body_ir_module_v0_with_executable_context(
        &program,
        &["main".into()],
        checker.type_info(),
...
fn attach_public_dependency(
    ...
    owner.contract_metadata.provider.provider_dependencies.push(
        incan_frontend::library_manifest::ProviderDependencyMetadata {
            ...
            artifact_digest: incan_frontend::library_manifest::digest_provider_artifact(target_root)?,
...
fn aliased_public_model_and_enum_execute_without_a_package_function_call() -> Result<(), Box<dyn Error>> {
    ...
    let resolved = resolve_executable_requirements(&index, &required)?;
    assert_eq!(resolved.decoded_declarations, 3);
    let module = incan_frontend::body_ir::build_body_ir_module_v0_with_executable_context(
        &program,
        &module_path,
        checker.type_info(),
        &resolved.modules,
    ...
    let execution = crate::replacement::prepare_free_function_execution_in_graph(graph, "main", &[], None)?;
    let result = crate::replacement::execute_prevalidated_free_function(execution)?;
...
fn a_transitive_facade_resolves_the_declaring_artifact() -> Result<(), Box<dyn Error>> {
    ...
    facade.contract_metadata.provider.provider_dependencies.push(
        incan_frontend::library_manifest::ProviderDependencyMetadata {
            ...
            artifact_digest: incan_frontend::library_manifest::digest_provider_artifact(&origin_root)?,
    ...
    edge.kind = incan_frontend::library_manifest::ProviderDependencyKind::PrivateImplementation;
    ...
    let edge = facade
        ...
        .first_mut()
        .ok_or("facade edge absent")?;
    edge.kind = incan_frontend::library_manifest::ProviderDependencyKind::PublicPackage;
...
fn product_pair_provider_plan(
    root: &Path,
) -> Result<std::sync::Arc<incan_frontend::provider::ProviderPlan>, Box<dyn Error>> {
    let mut entries = HashMap::new();
...
fn two_foreign_products_keep_distinct_signature_origins() -> Result<(), Box<dyn Error>> {
    use incan_frontend::library_manifest::{TypeRef, VisitTypeRefs};
    let temporary = tempfile::tempdir()?;
