...
pub fn with_checked_type_routes<T: VisitTypeRefs>(
    mut value: T,
...
pub fn with_checked_type_origins<T: VisitTypeRefs>(
    mut value: T,
...
pub fn contains_native_union(value: &(impl VisitTypeRefs + Clone)) -> bool {
    let mut found = false;
    value
        .clone()
    ...
    members.visit_type_refs(&mut |member| match member {
        TypeRef::Named { name, origin: None } if !name.starts_with("::") => {
            if matches!(
                ...
                crate::symbols::ResolvedType::Named(_)
            ) {
...
pub fn with_self_owned_native_unions<T: VisitTypeRefs>(mut value: T, provider_crate: &str) -> T {
...
pub fn with_checked_native_unions<T: VisitTypeRefs>(
    mut value: T,
    library: &str,
    ...
    value.visit_type_refs(&mut |ty| {
        ...
        let owner_path = crate::rust_type_display::provider_bridge_route(library, route);
...
pub fn with_native_nominal_origins<T: VisitTypeRefs>(
    mut value: T,
    origins: &BTreeMap<String, NominalTypeOriginExport>,
) -> T {
