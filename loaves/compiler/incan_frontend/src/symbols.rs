...
pub fn canonical_builtin_function_identity(builtin: BuiltinFnId) -> Option<CanonicalSymbolId> {
...
pub fn overload_emitted_name(source_name: &str, hash: u64) -> String {
...
pub fn overloaded_function_emitted_name(source_name: &str, info: &FunctionInfo) -> String {
...
pub fn overload_emitted_name_prefix(source_name: &str) -> String {
...
pub fn is_overload_emitted_name(name: &str) -> bool {
...
impl SymbolTable {
    ...
    pub fn is_active_lookup_binding(&self, id: SymbolId) -> bool {
    ...
    pub fn import_binding_path(&self, name: &str) -> Option<&[String]> {
    ...
    pub fn active_import_binding_paths(&self) -> impl Iterator<Item = (&str, &[String])> {
    ...
    pub fn active_module_source_bindings(&self) -> impl Iterator<Item = (Span, &str, Option<&CanonicalSymbolId>)> + '_ {
    ...
    pub fn module_type_identity(&self, name: &str) -> Option<&CanonicalSymbolId> {
        let id = self
            .dependency_interface_bindings
            .as_ref()
            .and_then(|bindings| bindings.get(name))
    ...
    pub fn take_binding_collisions(&mut self) -> Vec<SymbolBindingCollision> {
        std::mem::take(&mut self.binding_collisions)
    }

    /// Open a fresh temporary lookup view for one dependency interface.
    pub fn begin_dependency_interface_bindings(&mut self) {
    ...
    pub fn finish_dependency_interface_bindings(&mut self) {
    ...
    pub fn begin_current_scope_binding_transaction(&mut self) {
    ...
    pub fn finish_current_scope_binding_transaction(&mut self) -> HashMap<String, Option<SymbolId>> {
    ...
    pub fn restore_current_scope_binding(&mut self, name: &str, binding: Option<SymbolId>) {
        if self.current_scope == 0
...
pub struct ClassInfo {
    ...
    pub field_default_metadata: Box<HashMap<String, crate::library_exports::CheckedParamDefault>>,
...
pub fn field_is_type_private(field: &crate::ast::FieldDecl, private_fields_are_type_private: bool) -> bool {
    private_fields_are_type_private && matches!(field.visibility, crate::ast::Visibility::Private)
}
...
pub fn field_surface_type_name(source: &Type, resolved: &ResolvedType) -> String {
...
impl ResolvedType {
    ...
    pub fn iterator_item_type(&self) -> Option<&ResolvedType> {
        match self {
            ResolvedType::Generic(name, args)
    ...
    pub fn builtin_zip_item_type(&self) -> Option<&ResolvedType> {
        match self {
