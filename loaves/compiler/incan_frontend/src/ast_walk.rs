...
pub fn any_expr_in_program<F>(program: &Program, mut pred: F) -> bool
where
    F: FnMut(&Expr) -> bool,
{
    program
...
pub fn any_expr_in_body<F>(body: &[Spanned<Statement>], mut pred: F) -> bool
where
    F: FnMut(&Expr) -> bool,
{
...
///
/// Use this for a single expression slot, such as a match arm's `=> expr` body or its guard.
...
fn method_has_expr<F>(method: &crate::ast::MethodDecl, pred: &mut F) -> bool
where
    F: FnMut(&Expr) -> bool,
{
...
fn params_have_expr<F>(params: &[Spanned<crate::ast::Param>], pred: &mut F) -> bool
where
    F: FnMut(&Expr) -> bool,
{
...
fn decorators_have_expr<F>(decorators: &[Spanned<crate::ast::Decorator>], pred: &mut F) -> bool
where
    F: FnMut(&Expr) -> bool,
{
    ...
    F: FnMut(&Expr) -> bool,
{
    ...
    match stmt {
        ...
        Statement::Surface(surface_stmt) => match &surface_stmt.payload {
            crate::ast::SurfaceStmtPayload::KeywordArgs(args) => args.iter().any(|arg| expr_has(&arg.node, pred)),
        },
...
fn assert_has_expr<F>(assert_stmt: &crate::ast::AssertStmt, pred: &mut F) -> bool
where
    F: FnMut(&Expr) -> bool,
{
    ...
    F: FnMut(&Expr) -> bool,
{
    ...
    match expr {
        ...
        Expr::Surface(surface_expr) => match &surface_expr.payload {
            crate::ast::SurfaceExprPayload::PrefixUnary(expr) => expr_has(&expr.node, pred),
            crate::ast::SurfaceExprPayload::RaceFor(race) => race.arms.iter().any(|arm| {
                expr_has(&arm.awaitable.node, pred)
                    || match &arm.body {
                        crate::ast::RaceForBody::Expr(expr) => expr_has(&expr.node, pred),
                        crate::ast::RaceForBody::Block(stmts) => any_expr_in_body_impl(stmts, pred),
                    }
            }),
            crate::ast::SurfaceExprPayload::LeadingDotPath { .. } => false,
            crate::ast::SurfaceExprPayload::ScopedGlyph { left, right, .. } => {
                expr_has(&left.node, pred) || expr_has(&right.node, pred)
            }
            crate::ast::SurfaceExprPayload::ScopedSymbolCall { args, .. } => args.iter().any(|arg| match arg {
                crate::ast::CallArg::Positional(expr)
                | crate::ast::CallArg::Named(_, expr)
                | crate::ast::CallArg::PositionalUnpack(expr)
                | crate::ast::CallArg::KeywordUnpack(expr) => expr_has(&expr.node, pred),
            }),
        },
        ...
        Expr::FString(parts) => parts.iter().any(|part| match part {
            ...
            crate::ast::FStringPart::Expr { expr, .. } => expr_has(&expr.node, pred),
        }),
        ...
        Expr::Embedded(fragment) => fragment.holes().iter().any(|hole| expr_has(&hole.node, pred)),
