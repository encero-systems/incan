...
pub struct CheckedMethod {
    ...
    pub receiver: Option<crate::ast::Receiver>,
...
pub struct CheckedExportIdentity {
    ...
    pub type_origins: std::collections::BTreeMap<String, crate::library_manifest::NominalTypeOriginExport>,
}
...
pub struct CheckedNewtypeExport {
    ...
    pub constraints: Vec<crate::symbols::NewtypePrimitiveConstraint>,
...
fn checked_overload_function_exports(
    exported_name: String,
    overloads: &[crate::symbols::FunctionOverloadInfo],
    identity: CheckedExportIdentity,
) -> Vec<CheckedNamedExport> {
    overloads
        .iter()
...
fn checked_preset_value(expr: &Expr, context: DefaultPathContext<'_>) -> CheckedPresetValue {
    match expr {
        ...
        Expr::Call(callee, _type_args, args) => {
            ...
            for arg in args {
                let crate::ast::CallArg::Named(field, value) = arg else {
                    return CheckedPresetValue::Unsupported;
                };
    ...
    match &expr.node {
        ...
        Expr::Call(callee, _type_args, args) => {
            ...
            let args = args
                .iter()
                .map(|arg| match arg {
                    crate::ast::CallArg::Positional(value) => CheckedParamDefaultArg {
                        name: None,
                        value: checked_param_default(value, context),
                    },
                    crate::ast::CallArg::Named(name, value) => CheckedParamDefaultArg {
                        name: Some(name.node.clone()),
                        value: checked_param_default(value, context),
                    },
                    crate::ast::CallArg::PositionalUnpack(_) | crate::ast::CallArg::KeywordUnpack(_) => {
                        CheckedParamDefaultArg {
                            name: None,
...
fn map_fields(
    ...
    let map_type = |name: &str, info: &FieldInfo| {
        ...
        crate::rust_type_display::qualify_for_manifest(
            &ty,
            provider_libraries
                .and_then(|providers| providers.get(name))
                .map(String::as_str),
...
pub fn qualify_checked_default_for_manifest(
    ...
    let qualify_path = |path: &[String]| {
        let mut qualified = vec![
            crate::rust_type_display::PROVIDER_RUST_BRIDGE_MODULE.to_string(),
            provider.to_string(),
        ];
            ...
            signature: signature
                ...
                .map(|signature| -> Result<CheckedParamDefaultCallSignature, String> {
                    ...
                    for param in &mut params {
                        param.ty =
                            crate::rust_type_display::qualify_for_manifest(&param.ty, Some(provider), type_params)?;
                    }
                    Ok(CheckedParamDefaultCallSignature {
                        params,
                        return_type: crate::rust_type_display::qualify_for_manifest(
...
fn map_methods_with_defaults(
    ...
    ast_methods: &[Spanned<crate::ast::MethodDecl>],
    checker: &TypeChecker,
) -> Vec<CheckedMethod> {
...
fn ast_method_param_names_match_checked(ast_method: &Spanned<crate::ast::MethodDecl>, checked: &CheckedMethod) -> bool {
...
fn ast_method_shape_matches_checked(
    ast_method: &Spanned<crate::ast::MethodDecl>,
    checked: &CheckedMethod,
    checker: &TypeChecker,
) -> bool {
...
mod tests {
    ...
    fn checked_foreign_generic_bound_retains_import_identity() {
        use crate::symbols::{RustImportBindingKind, RustItemInfo, Symbol, SymbolKind};
