...
use crate::symbols::{ResolvedType, ScopeKind, Symbol, SymbolKind, TypeInfo, VariableInfo, union_ty};
...
impl TypeChecker {
    ...
    pub(in crate::typechecker::check_expr) fn check_await(
        &mut self,
        inner: &Spanned<Expr>,
        span: Span,
    ) -> ResolvedType {
    ...
    fn method_set_has_async_method(
        methods: &std::collections::HashMap<String, crate::symbols::MethodInfo>,
        overloads: &std::collections::HashMap<String, Vec<crate::symbols::MethodInfo>>,
        method: &str,
    ) -> bool {
    ...
    pub(in crate::typechecker) fn await_output_type_from_type(&self, ty: &ResolvedType) -> Option<ResolvedType> {
    ...
    pub(in crate::typechecker::check_expr) fn check_race_for(
        &mut self,
        race: &RaceForExpr,
        span: Span,
    ) -> ResolvedType {
    ...
    pub(in crate::typechecker::check_expr) fn check_try(&mut self, inner: &Spanned<Expr>, span: Span) -> ResolvedType {
        self.check_try_with_expected(inner, span, None)
    }

    /// Validate the `?` operator while preserving contextual `Ok` type information.
    pub(in crate::typechecker::check_expr) fn check_try_with_expected(
        &mut self,
        inner: &Spanned<Expr>,
        span: Span,
    ...
    pub(in crate::typechecker) fn validate_try_result_type(
        &mut self,
    ...
    pub(in crate::typechecker::check_expr) fn check_if_expr(&mut self, if_expr: &IfExpr, _span: Span) -> ResolvedType {
    ...
    pub(in crate::typechecker::check_expr) fn check_loop_expr(
        &mut self,
        loop_expr: &LoopExpr,
    ...
    pub(in crate::typechecker::check_expr) fn check_range_expr(
        &mut self,
        start: &Spanned<Expr>,
        end: &Spanned<Expr>,
    ) -> ResolvedType {
