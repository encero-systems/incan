...
type InheritedMembers = (
    ...
    HashMap<String, crate::library_exports::CheckedParamDefault>,
...
impl TypeChecker {
    ...
    pub fn collect_declaration(&mut self, decl: &Spanned<Declaration>) {
        match &decl.node {
            ...
            match &kind {
                ...
                SymbolKind::Static(info) => {
                    self.type_info.declarations.static_bindings.insert(
                        alias.name.clone(),
                        crate::typechecker::StaticBindingInfo {
    ...
    pub(in crate::typechecker) fn record_function_overload_binding(
        &mut self,
        local_name: &str,
    ...
    pub(in crate::typechecker) fn record_imported_function_binding(&mut self, local_name: &str, kind: &SymbolKind) {
        match kind {
    ...
    pub fn local_partial_params(mut params: Vec<CallableParam>, args: &[PartialArg]) -> Vec<CallableParam> {
    ...
    fn collect_static(&mut self, static_decl: &StaticDecl, span: Span) {
        ...
        self.type_info.declarations.static_bindings.insert(
            ...
            crate::typechecker::StaticBindingInfo { is_imported: false },
        );
    }
    ...
    pub fn trait_bound_source_name(&self, name: &str) -> Option<String> {
        if let Some(path) = self.import_binding_path(name) {
    ...
    pub fn imported_generic_rust_bound_path(&self, name: &str) -> Option<String> {
    ...
    pub fn resolve_qualified_trait(&mut self, name: &str) -> Option<(String, TraitInfo)> {
    ...
    pub fn imported_trait_is_derivable(&mut self, module_path: &[String], trait_name: &str) -> bool {
        if self
            .provider_plan
            .active_sdk_provider_for_module(module_path)
            .is_some_and(|provider| provider.manifest.is_some())
    ...
    pub fn resolve_pending_trait_supertraits(&mut self) {
    ...
    pub fn finalize_supertrait_graph(&mut self) {
    ...
    pub fn merge_supertrait_requires_into_traits(&mut self) {
