...
use crate::symbols::*;
use crate::testing_markers::{TestingFixtureMarkerArgs, TestingMarkerSemantics, resolve_testing_fixture_marker_args};
...
use super::{
    ...
    MethodDecoratorReceiverRole, MethodDecoratorReceiverSlot, TestingFixtureInfo, TypeChecker, YieldContext,
};
...
/// How a method decorator declaration's shapes hold the decorated method's receiver (#1790).
...
/// for `mut self`. That surface form is what the decorator chain is checked against. The recorded binding keeps the
/// form the method's generated wrapper passes, `&Owner` or `&mut Owner`, so lowering builds the decorated static,
/// adapter and wrapper from the same types it built them from when source had to spell that form.
#[derive(Debug, Clone)]
struct DecoratedMethodReceiver {
...
impl DecoratedMethodReceiver {
    ...
    fn new(owner: &str, receiver: Option<Receiver>) -> Self {
        Self {
    ...
    fn surface_param(&self) -> CallableParam {
    ...
    fn with_passing_receiver(&self, shape: ResolvedType) -> ResolvedType {
...
#[derive(Debug, Clone)]
pub(in crate::typechecker) struct TraitMethodEntry {
...
fn is_possible_testing_fixture_decorator(
    dec: &Decorator,
    aliases: &impl crate::decorator_resolution::DecoratorPrefixLookup,
) -> bool {
    let resolved = crate::decorator_resolution::resolve_decorator_path(dec, aliases);
    resolved.len() == 3
...
impl TypeChecker {
    ...
    /// order.
    pub fn collect_testing_fixture_names(&mut self, program: &Program) {
    ...
    fn concretize_self_type_in_annotation(ty: &ResolvedType, self_ty: &ResolvedType) -> ResolvedType {
        match ty {
            ...
            ResolvedType::Function(params, ret) => ResolvedType::Function(
                params
                    .iter()
                    .map(|param| crate::symbols::CallableParam {
    ...
    pub(in crate::typechecker) fn method_sigs_compatible(&self, expected: &MethodInfo, found: &MethodInfo) -> bool {
    ...
    pub(in crate::typechecker) fn trait_method_info_resolved(
        &mut self,
        adopted_trait: &str,
        method: &str,
        ambiguity_span: Span,
    ...
    pub(in crate::typechecker) fn trait_method_info_resolved_for_adoption(
        &mut self,
        adoption: &TypeBoundInfo,
        method: &str,
        ambiguity_span: Span,
    ...
    pub(in crate::typechecker) fn trait_method_entry_resolved_for_adoption(
        &mut self,
        adoption: &TypeBoundInfo,
        method: &str,
        ambiguity_span: Span,
    ...
    pub(in crate::typechecker) fn trait_property_info_resolved_for_adoption(
        &mut self,
        adoption: &TypeBoundInfo,
        property: &str,
        ambiguity_span: Span,
    ...
    fn check_derives_metadata(&mut self, konst: &ConstDecl) {
        ...
        for entry in entries {
            let crate::ast::ListEntry::Element(expr) = entry else {
                self.errors.push(CompileError::type_error(
        ...
        for field in active_model_fields {
            ...
            self.symbols.define(Symbol {
                ...
                kind: SymbolKind::Field(FieldInfo {
                    ...
                    surface_type_name: Some(crate::symbols::field_surface_type_name(&field.node.ty.node, &ty)),
                    ty,
                    visibility: field.node.visibility,
                    is_type_private: crate::symbols::field_is_type_private(
                        &field.node,
        ...
        for field in active_class_fields {
            ...
            self.symbols.define(Symbol {
                ...
                kind: SymbolKind::Field(FieldInfo {
                    ...
                    ty,
                    visibility: field.node.visibility,
                    is_type_private: crate::symbols::field_is_type_private(&field.node, true),
    ...
    pub(in crate::typechecker) fn rust_trait_path_suffix(path: &str) -> &str {
    ...
    pub(in crate::typechecker) fn rust_type_metadata_implements_trait(
    ...
    fn user_defined_decorator_callable_type(&mut self, decorator: &Spanned<Decorator>, display: &str) -> ResolvedType {
    ...
    fn check_function(&mut self, func: &FunctionDecl, decl_span: Span) {
        ...
        let previous_c_abi_raw_call_owner =
            self.current_c_abi_raw_call_owner
                .replace(crate::typechecker::CBindingRawCallOwner {
    ...
    pub fn check_property(&mut self, property: &Spanned<PropertyDecl>, owner: &str, owner_params: &[TypeParam]) {
        let owner_type_params = self
            .lookup_type_info(owner)
            .map(|info| match info {
                TypeInfo::Model(model) => model.type_params.clone(),
