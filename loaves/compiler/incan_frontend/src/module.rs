...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum SourceModuleImportResolution {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ResolvedProgramSourceImport {
    pub span: Span,
...
/// namespace.
pub fn declaration_package_identity(package_identity: Option<&str>, module_path: Option<&[String]>) -> Option<String> {
    package_identity
        .filter(|_| {
            !matches!(
...
pub fn resolve_source_module_import(
    base_dir: &Path,
    source_root: Option<&Path>,
    import: &ImportDecl,
...
pub fn resolve_source_module_import_from_source_file(
    base_dir: &Path,
    source_root: Option<&Path>,
...
pub fn self_import_diagnostic_message(
    module_ref: &SourceModuleRef,
    import_path: &[String],
    can_use_root_import: bool,
) -> String {
...
/// distinct module.
pub fn canonicalize_source_module_segments(segments: &[String]) -> Vec<String> {
...
pub fn logical_source_import_candidates(current_module_path: &[String], path: &ImportPath) -> Vec<Vec<String>> {
...
pub fn logical_source_path_candidates(
...
pub fn logical_module_segments_from_file(base: &Path, module_file: &Path) -> Option<Vec<String>> {
...
pub fn logical_module_name_from_source_path(source_path: &str) -> Option<String> {
...
fn resolve_source_root_for_imports(start_dir: &Path) -> Option<PathBuf> {
    let manifest = oven_model::manifest::ProjectManifest::discover(start_dir)
        .ok()
        .flatten()?;
