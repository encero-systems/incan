...
- `src/oven_facet.rs`: `compiler_identity()` and `provider_hooks()` — Oven's `CompilerIdentity` and `OvenProviderHooks` filled with Incan's facts — and the SDK-staging functions that read the inventory, which `legacy_cargo` used to take from the provider directly
