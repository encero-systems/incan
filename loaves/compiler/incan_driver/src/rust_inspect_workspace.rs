...
use incan_frontend::ast::{ImportKind, Program};
...
use incan_provider::inventory::{normalize_sdk_artifact_projections, normalize_sdk_dependency_rebindings};
...
fn hash_dependency_spec_for_rust_inspect(hasher: &mut Sha256, spec: &DependencySpec) {
    use oven_model::manifest::GitReference;
...
fn rust_inspect_workspace_dir(project_root: &Path, project_name: &str, fingerprint: &str) -> PathBuf {
    ...
    let suffix = fingerprint
        ...
        .take(16)
        .collect::<String>();
    oven_model::lock::compiler_lock_state_dir(project_root)
        .join("rust_inspect")
...
pub fn configure_rust_inspect_cargo_target(manifest_dir: &Path, target_dir: &Path) -> CliResult<()> {
...
#[cfg(all(test, feature = "rust_inspect"))]
#[allow(clippy::too_many_arguments)]
pub fn ensure_rust_inspect_workspace(
    project_root: &Path,
    project_name: &str,
...
pub fn collect_rust_inspect_query_paths(modules: &[ParsedModule]) -> Vec<String> {
...
pub fn collect_rust_inspect_query_paths_from_programs<'a>(
...
pub fn collect_rust_inspect_derive_probe_paths(modules: &[ParsedModule]) -> Vec<String> {
    use incan_frontend::ast::{Declaration, Decorator, DecoratorArg, Expr, Literal};

    /// Return decorators only for declarations that can emit a concrete Rust type.
    fn declaration_decorators(declaration: &Declaration) -> Option<&[incan_frontend::ast::Spanned<Decorator>]> {
        match declaration {
...
pub fn mark_oven_direct_rust_inspection(manifest_dir: &Path) -> CliResult<()> {
    let bootstrap_marker = manifest_dir.join(::rust_inspect::OVEN_CARGO_BOOTSTRAP_INSPECTION_MARKER);
    ...
    let marker = manifest_dir.join(::rust_inspect::OVEN_DIRECT_INSPECTION_MARKER);
...
pub fn mark_oven_cargo_bootstrap_rust_inspection(manifest_dir: &Path) -> CliResult<()> {
    let direct_marker = manifest_dir.join(::rust_inspect::OVEN_DIRECT_INSPECTION_MARKER);
    ...
    let marker = manifest_dir.join(::rust_inspect::OVEN_CARGO_BOOTSTRAP_INSPECTION_MARKER);
...
fn oven_direct_rust_inspection_marked(manifest_dir: &Path) -> bool {
    manifest_dir
        .join(::rust_inspect::OVEN_DIRECT_INSPECTION_MARKER)
        .is_file()
}
...
pub fn prewarm_rust_inspect_workspace(
    manifest_dir: &Path,
    target_dir: &Path,
    query_paths: &[String],
        ...
        let requirements = ProjectRequirements {
            ...
            sdk_dependency_rebindings: vec![SdkDependencyRebinding {
                containing_artifact: LibraryArtifactMetadata {
                    ...
                    kind: incan_frontend::library_manifest_index::LibraryArtifactKind::Materialized,
                },
    ...
    fn rust_inspect_helper_materializes_sdk_projection_issue911() -> Result<(), Box<dyn std::error::Error>> {
        ...
        manifest.contract_metadata.provider.provider_dependencies.push(
            incan_frontend::library_manifest::ProviderDependencyMetadata {
                kind: incan_frontend::library_manifest::ProviderDependencyKind::PrivateImplementation,
