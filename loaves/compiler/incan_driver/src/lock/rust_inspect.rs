...
fn install_oven_inspection_source_authority(
    manifest_dir: &Path,
    packages: &[oven_rustc::rustc::OvenRustcRegistrySourcePackage],
    artifact_root: &Path,
    extension_paths: Option<&BTreeSet<String>>,
    base_source_authority: Option<(&Path, &[oven_rustc::rustc::OvenRustcRegistrySourcePackage])>,
) -> CliResult<()> {
...
fn oven_inspection_sources(
    ...
    extension_paths: Option<&BTreeSet<String>>,
    base_source_authority: Option<(&Path, &[oven_rustc::rustc::OvenRustcRegistrySourcePackage])>,
) -> CliResult<Vec<::rust_inspect::OvenInspectionRegistrySource>> {
    packages
        .iter()
        .map(|package| {
...
pub fn registry_source_is_owned_by_catalog(
    source: &oven_rustc::rustc::OvenRustcRegistrySourcePackage,
    catalog: &[oven_rustc::rustc::OvenRustcRegistrySourcePackage],
) -> bool {
...
pub fn prepare_rust_inspect_typecheck_workspace(
    ...
    let oven_build_inputs = crate::build_unit::oven_build_unit_inputs(
        provider_plan,
...
mod tests {
    ...
    fn registry_source_ownership_unifies_feature_variants_with_the_same_source_archive() {
        let source = oven_rustc::rustc::OvenRustcRegistrySource {
        ...
        let requested = oven_rustc::rustc::OvenRustcRegistrySourcePackage {
            package: "syn".to_string(),
            version: "2.0.117".to_string(),
            features: vec!["derive".to_string()],
            source: source.clone(),
        };
        let owner = oven_rustc::rustc::OvenRustcRegistrySourcePackage {
            features: vec!["clone-impls".to_string(), "full".to_string()],
            ..requested.clone()
        };

        assert!(registry_source_is_owned_by_catalog(&requested, &[owner]));
        let different_archive = oven_rustc::rustc::OvenRustcRegistrySourcePackage {
