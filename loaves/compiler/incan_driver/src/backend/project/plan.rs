...
//!
//! [`CompilationPlan`] is a pure, testable representation of what the compiler will produce. [`Executor`] performs the
//! actual filesystem operations and cargo invocations.
...
///
/// This is the only place where side effects occur in the project generation pipeline. The caller owns the plan's
/// output directory and its contained Cargo target. Incan's CLI does not use this legacy executor; CLI
/// build/run/test/library commands select the managed generated-build cache instead.
