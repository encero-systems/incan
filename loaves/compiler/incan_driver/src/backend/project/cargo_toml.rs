...
///
/// Returns [`toml::Value::String`] for simple version-only registry deps (shorthand form), or [`toml::Value::Table`]
/// for deps that need extra fields (features, git, path, etc.).
...
pub(crate) fn toolchain_crate_path(crate_name: &str) -> PathBuf {
    oven_model::toolchain_layout::resolve_toolchain_crate_path(crate_name)
}
...
impl ProjectGenerator {
    ...
    pub(crate) fn generate_cargo_toml(&self) -> io::Result<String> {
        let edition = self
            .rust_edition
            .as_deref()
...
mod tests {
    ...
    fn cargo_rendering_and_semantic_identity_share_toolchain_support_paths() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let semantic = incan_provider::requirements::semantic_sdk_path_dependencies(
