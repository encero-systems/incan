...
#[derive(Debug, Clone)]
pub struct CargoLockProjection {
...
impl CargoLockProjection {
    ...
    pub fn new(canonical_payload: String, canonical_root_name: String) -> io::Result<Self> {
    ...
    pub fn reconciliation_pass_limit(&self, projected_payload: &str) -> io::Result<usize> {
    ...
    pub fn validate_convergence(&self, first: &str, second: &str) -> io::Result<()> {
        if first == second {
            Ok(())
        } else {
            Err(io::Error::other(
