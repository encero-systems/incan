...
//! ## What is compared
//!
//! The profile is [`SHADOW_COMPARISON_PROFILE_ID`]: one named free function in a source-only Incan module, called with
//! concrete scalar arguments. A module containing `main` is outside this harness profile. Both routes observe the
...
//! function returned normally (and the routes must agree on the typed scalar), or it stopped on a runtime failure this
//! profile can classify identically on both routes ([`RuntimeFailureClass`]). A failure neither route can classify is
//! [`ShadowUnavailable`], not agreement: "both stopped somehow" is not proof that they stopped the same way, and an
//! arithmetic overflow is not a division by zero.
//!
...
//!
//! Three things are deliberately *not* comparators, because none of them observes program meaning: generated Rust text,
//! whether that Rust compiled, and the produced artifact's shape. A legacy program that fails to build makes the
//! comparison [`ShadowComparisonState::Unavailable`] with that build failure as the reason — it never becomes a
//! divergence claim, because a comparison that could not run has proven nothing.
//!
...
#[derive(Debug, Clone)]
pub struct ShadowLegacyMaterialization {
...
impl ShadowLegacyMaterialization {
    ...
    pub fn require_compatible_oven_build_unit_inputs(
        &self,
    ...
    pub fn require_profile_source(&self, profile: &ShadowComparisonProfile) -> Result<(), ShadowUnavailable> {
...
impl ShadowComparisonProfile {
    ...
    /// Content identity of this source input: the profile kind, the source, the observed function, and the exact
    /// arguments.
    ///
    ...
    fn program_after_input_contract(&self) -> Result<incan_frontend::ast::Program, ShadowUnavailable> {
...
///
/// Only the scalars the profile can spell in source are accepted. A collection, tuple, range, or unit argument has no
/// literal form this profile is willing to synthesize, so it becomes an explicit unavailable reason rather than a
/// guessed spelling that would silently change what was executed.
...
///
/// `detail` is never compared; it exists so a divergence or an unavailable result can name what actually happened
/// without widening the comparison surface itself. `profile_identity` binds the observation to the exact profile
/// instance that produced it, so [`classify_observations`] can refuse to compare observations of two different profiles
/// instead of reporting a meaningless verdict.
#[derive(Debug, Clone)]
...
impl RouteObservation {
    ...
    fn describe(&self) -> String {
        format!(
            ...
            self.stdout.len(),
            oven_store::digest_bytes(&self.stdout),
            self.stderr.len(),
            oven_store::digest_bytes(&self.stderr),
        )
    }
}
...
#[must_use]
pub fn compare_source_observable_with_materialization(
...
pub fn validate_source_observable_profile(profile: &ShadowComparisonProfile) -> Result<(), ShadowUnavailable> {
...
#[must_use]
pub fn unavailable_source_observable_comparison(
    profile: &ShadowComparisonProfile,
...
pub struct ReplacementRouteResult {
    pub observation: Option<RouteObservation>,
    pub execution: Option<ReplacementExecution>,
    pub output: ProgramOutput,
    pub unavailable_reason: Option<String>,
}

/// A completed Oven-owned legacy process plus its authority and retained raw evidence.
pub struct LegacyRouteResult {
    pub observation: Option<RouteObservation>,
    pub authority: LegacyExecutionAuthority,
    pub process: LegacyProcessEvidence,
...
pub struct PreparedShadowProfile {
    ...
    fn new(profile: &ShadowComparisonProfile) -> Result<Self, ShadowUnavailable> {
        ...
        let function_declaration = program
            ...
            .find(|declaration| {
                matches!(
                    &declaration.node,
                    incan_frontend::ast::Declaration::Function(function) if function.name == profile.function
                )
            })
            .ok_or_else(|| {
                ShadowUnavailable::new(format!(
...
fn refuse_source_process_import(program: &incan_frontend::ast::Program) -> Result<(), ShadowUnavailable> {
    let imports_process = program.declarations.iter().any(|declaration| {
        ...
        match &import.kind {
            incan_frontend::ast::ImportKind::RustCrate { crate_name, path, .. } => {
                crate_name == "std" && (path.is_empty() || path.first().is_some_and(|segment| segment == "process"))
            }
            incan_frontend::ast::ImportKind::RustFrom {
                crate_name,
                path,
                items,
                ..
        ...
        Err(ReplacementExecutionError::RuntimeFailure { detail, .. }) => {
            let observation = classify_replacement_failure(&detail).map(|failure| RouteObservation {
                ...
                output_identity: digest_output(&[
                    ...
                    &detail,
                    &oven_store::digest_bytes(output.stdout()),
                    &oven_store::digest_bytes(output.stderr()),
                ]),
            });
            (None, observation)
        }
...
pub fn emit_legacy_rust_with_materialization(
    program: &str,
    materialization: &ShadowLegacyMaterialization,
) -> Result<String, ShadowUnavailable> {
...
fn emit_legacy_rust_with_provider_plan(
    ...
    incan_frontend::typechecker::check(&ast)
...
pub fn emit_legacy_rust(program: &str) -> Result<String, ShadowUnavailable> {
...
pub fn observe_legacy_process(
    ...
    let stdout_digest = oven_store::digest_bytes(&process.stdout);
    let stderr_digest = oven_store::digest_bytes(&process.stderr);
    let report_digest = process
        .result_report
        .as_deref()
        .map(oven_store::digest_bytes)
...
/// diagnostic words. Overflow is tested before division because a division-overflow panic also names division. An
/// unrecognized failure is unavailable, not a shared failure observation.
