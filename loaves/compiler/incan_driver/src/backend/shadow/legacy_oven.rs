...
//!
//! Parity evidence must come from the adopted build and execution boundary, not from an ad-hoc compiler invocation. The
//! legacy route therefore runs the same way an ordinary Incan build does: the emitted Rust is authorized by an
//! [`OvenReceipt`], an immutable direct-`rustc` plan is selected from the bounded [`OvenStore`] against that receipt's
//! reusable build unit, Oven compiles the caller-owned source with no Cargo process, and the produced binary is
//! executed. Every one of those authority facts is carried back in a [`LegacyExecutionAuthority`] and folded into the
//! legacy receipt's output identity.
//!
...
impl LegacyOvenCapability {
    ...
    ///
    /// `baked_receipt_path` is a persisted Oven receipt — in practice `.incan/oven/receipt.json` from a project that
    /// has been through `incan oven bake`. It is parsed and identity-verified before it can authorize anything; a
    /// receipt that does not verify is refused rather than trusted, exactly as `incan oven run` treats its own input.
    ...
    #[must_use]
    pub fn intent(&self) -> &oven_store::OvenBuildIntent {
    ...
    pub fn require_materialization_compatibility(
        &self,
    ...
    pub fn select_for_materialization(
        &self,
        materialization: &ShadowLegacyMaterialization,
    ) -> Result<Self, ShadowUnavailable> {
...
pub(crate) enum ForcedResultTransportFailure {
    Write,
    Rename,
}
...
pub(crate) fn observe_legacy_route_with_forced_transport_failure(
    profile: &ShadowComparisonProfile,
    prepared: &PreparedShadowProfile,
    materialization: &ShadowLegacyMaterialization,
    capability: &LegacyOvenCapability,
...
pub fn materialize_legacy_program(
    program: &str,
    materialization: &ShadowLegacyMaterialization,
    project_root: &Path,
...
impl LegacyOvenCapability {
    ...
    pub fn from_environment() -> Result<Self, ShadowUnavailable> {
        ...
        let mut capability =
            Self::adopt_baked_project(oven_store::store::store_root_for_home(&incan_home), rustc, receipt_path)?;
    ...
    #[cfg(test)]
    pub fn from_environment_for_materialization(
        materialization: &ShadowLegacyMaterialization,
    ) -> Result<Self, ShadowUnavailable> {
        let incan_home = environment_path(INCAN_HOME_ENV)?;
        let rustc = environment_path(RUSTC_ENV)?;
        let store_root = oven_store::store::store_root_for_home(&incan_home);
        let receipt_paths = environment_receipt_paths()?;
...
mod receipt_selection_tests {
    ...
    fn source_materialization_selects_the_later_exact_receipt() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let materialization = ShadowLegacyMaterialization::from_provider_plan(
            Arc::new(incan_provider::ProviderPlan::default()),
