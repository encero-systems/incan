//! The build report: what a build, run or inspection produced, in the shape the CLI renders and tools read.
//!
//! Assembled by the driver, rendered by the CLI: the report builders live here beside the types, the JSON and text
//! emitters stay with the command.
...
pub struct BuildReport {
    ...
    /// Backend-selection identity and execution receipt for this build (#986). A completed-output reuse retains the
    /// verified receipt sealed by the explicit bake that produced the selected output.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl BuildReport {
    ...
    pub fn with_workspace_context(mut self, workspace: BuildWorkspaceContext) -> Self {
...
impl BuildReportDraft {
    ...
    pub fn finish(self, timings_ms: BTreeMap<String, u64>) -> BuildReport {
        BuildReport {
...
pub fn generated_project_report(
    project_path: &Path,
    crate_root: &Path,
...
pub fn oven_generated_project_report(
    project_path: &Path,
    crate_root: &Path,
...
pub fn artifact_report(kind: impl Into<String>, path: &Path) -> BuildArtifactReport {
...
pub fn incan_dependencies_report(dependencies: Vec<(&String, &LibraryDependencySpec)>) -> Vec<IncanDependencyReport> {
