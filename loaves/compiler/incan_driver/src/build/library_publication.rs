...
use incan_frontend::library_manifest::published_layout::{LIBRARY_ARTIFACT_DIRECTORY, LIBRARY_MANIFEST_EXTENSION};
...
impl LibraryPublication {
    ...
    pub fn begin(project: &Path, output: &Path, receipts: Vec<PathBuf>) -> CliResult<Self> {
    ...
    pub fn begin_receipt_update(project: &Path, receipts: Vec<PathBuf>) -> CliResult<Self> {
    ...
    pub fn retaining_package_cache(mut self) -> Self {
    ...
    pub fn finish<T>(self, result: CliResult<T>) -> CliResult<T> {
...
fn owned_library_output(path: &Path) -> CliResult<bool> {
    ...
    for entry in fs::read_dir(path).map_err(io_error)? {
        ...
        if entry
            ...
            && incan_frontend::library_manifest::LibraryManifest::read_from_path(&entry.path()).is_ok()
        {
            return Ok(true);
        }
    }
...
fn is_generated_output(name: &std::ffi::OsStr) -> bool {
    ...
    let directory = matches!(
        ...
        == Some(incan_frontend::library_manifest::published_layout::EXECUTABLE_SURFACE_DIRECTORY);
    let artifact = path.extension().is_some_and(|extension| {
        extension == LIBRARY_MANIFEST_EXTENSION
            || extension == incan_frontend::library_manifest::published_layout::EXECUTABLE_SURFACE_EXTENSION
    });
    directory || artifact
}
...
mod tests {
    ...
    fn absent_outputs_and_unrelated_custom_files_keep_their_prior_contract() -> Result<(), Box<dyn Error>> {
        ...
        incan_frontend::library_manifest::LibraryManifest::new("library", "1.2.3")
