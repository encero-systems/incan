...
use crate::build::backend_selection::{
...
pub struct ReplacementModuleInputs {
    pub program: incan_frontend::ast::Program,
    pub module_path: Vec<String>,
    pub type_info: typechecker::TypeCheckInfo,
...
pub fn build_replacement_file_report(
    file_path: &str,
    ...
    let published = incan_frontend::executable_resolution::resolve_executable_requirements(
...
mod tests {
    ...
    fn the_replacement_build_pipeline_analyzes_its_session_exactly_once() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let analysis_scope = crate::session::scoped_compilation_session_analysis_invocations();
        let report = build_replacement_file_report(
            &entrypoint.to_string_lossy(),
            replacement_build_options(),
            &BuildReportOptions::default(),
