...
use crate::build::CompiledProviderMetadataInputs;
use crate::error::{CliError, CliResult};
use crate::project::resolve_source_root;
use incan_frontend::ast::{Declaration, Expr, ImportKind, Literal, Spanned, Statement, Visibility};
...
use incan_frontend::library_manifest_index::{LibraryArtifactKind, LibraryManifestIndex, LibraryManifestIndexEntry};
...
use incan_provider::{PackageFeatureGraph, PackageFeaturePlan, ProviderPlan, SDK_PROVIDER_BUILD_ENV};
...
pub fn synchronize_projected_provider_dependencies(
...
pub fn collect_unprojected_provider_modules(
    library_entrypoint: &Path,
    session: &crate::session::CompilationSession,
) -> CliResult<Vec<ParsedModule>> {
    let mut pending = crate::modules::library_source_seeds(library_entrypoint, session)?;
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        use incan_frontend::typechecker::{ProviderOperationDeclarationInfo, TypeCheckInfo};
...
"#;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let session = crate::session::CompilationSession::discover_with_feature_selection(
            &entry_path,
