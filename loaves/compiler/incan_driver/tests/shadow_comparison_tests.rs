...
//!
//! The legacy route needs a staged Oven capability (see `incan_driver::backend::shadow::legacy_oven`). Tests that
//! assert an executed comparison require it and say so when it is missing; tests about honest unavailability do not.
//!
...
use incan_driver::backend::selection::{BackendKind, FallbackOutcome, FallbackPolicy, ShadowComparisonState};
use incan_driver::backend::shadow::legacy_oven::LegacyOvenCapability;
use incan_driver::backend::shadow::{
