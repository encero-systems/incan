...
//!
//! A row may additionally declare the bounded comparison profile in [`incan_driver::backend::shadow`]. That row is then
...
//! including test crate supplies it as a `shadow_capability` module
//! (`loaves/compiler/incan_driver/tests/support/shadow_capability.rs`); when nothing is staged, the row degrades to
//! direct-execution-only evidence with that reason recorded — never to a green result, and never losing the replacement
//! execution that did happen.
...
use incan_driver::backend::shadow::{LegacyExecutionAuthority, ShadowComparison, ShadowComparisonProfile};
...
pub(crate) enum BehaviorCategory {
    ...
    /// Behavior owned by a stdlib facet under `loaves/stdlib/<component>/rust`, `loaves/kernel/incan_lang`, or `.incn`
    /// stdlib source.
...
///
/// Execution variants carry the real [`incan_driver::backend::selection::BackendExecutionReceipt::identity`] produced
/// by the run. Identity-only conformance carries a recomputable evidence identity and an optional receipt only when
...
fn retained_replacement_evidence(
    ...
    replacement: &incan_driver::backend::shadow::RouteEvidence,
...
/// gaps instead of defaulting to green — see the red-state tests in
...
///
/// This does not itself assert anything; callers in `loaves/compiler/incan_driver/tests/parity_corpus_tests.rs` are
/// responsible for turning `non_green_behavior > 0` (an unexpected regression, as opposed to a case whose disposition
/// already expects a non-green mismatch) into a test failure.
pub(crate) fn summarize(cases: &[ParityCase]) -> CorpusSummary {
    ...
    let reports: Vec<CaseReport> =
        incan_frontend::compiler_stack::run_on_compiler_stack(move || cases.iter().map(evaluate_case).collect());
    let green = reports
        .iter()
