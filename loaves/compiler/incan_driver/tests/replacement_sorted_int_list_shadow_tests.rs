//! Same-source proof for bounded canonical integer-list sorting through direct Body IR and receipt-backed native
//! execution.
...
/// Both routes retain the source list, sort a fresh integer list, and preserve exact ordinary streams.
#[test]
fn sorted_int_list_matches_the_receipt_backed_native_route() -> Result<(), Box<dyn std::error::Error>> {
    incan_frontend::compiler_stack::run_on_compiler_stack(|| check_sorted_int_list().map_err(|error| error.to_string()))
        .map_err(Into::into)
}

/// Compile and execute on the same compiler-sized stack used by the CLI.
fn check_sorted_int_list() -> Result<(), Box<dyn std::error::Error>> {
    let workspace = tempfile::tempdir()?;
    let profile = ShadowComparisonProfile::new(SORTED_INT_SOURCE, "sorted_int_list", Vec::new());
    ...
    let execution = comparison
        ...
        .as_ref()
        .ok_or("missing direct sorted-integer execution")?;
    ...
    let legacy = comparison
        .legacy
        .as_ref()
        .ok_or("missing native sorted-integer evidence")?;
    let replacement = comparison
        .replacement
        .as_ref()
        .ok_or("missing direct sorted-integer evidence")?;
