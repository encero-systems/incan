...
fn fixture_path(relative: &str) -> PathBuf {
    repo_root()
        .join("loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit")
        .join(relative)
}
...
fn json_report_records_explicit_artifacts_and_marker_counts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        ...
        "loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/main.rs"
    );
    assert_eq!(program["check_status"], "present");
    assert_eq!(program["strictness_status"], "available_for_review");
    assert_eq!(
        program["rust_files"],
        serde_json::json!(["loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/main.rs"])
    );
    ...
    assert_eq!(
        ...
        "loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/nested"
    );
    assert_eq!(
        stdlib["rust_files"],
        serde_json::json!(["loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/nested/lib.rs"])
    );
...
fn markdown_report_contains_objective_rows_and_details() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(markdown.contains("| `program-main` | `loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/main.rs` | present | available_for_review | 3 marker(s); notes: pending | 3 marker(s); notes: pending | 1 marker(s); notes: pending |"));
...
fn missing_and_no_rust_artifacts_are_reported_without_failing_by_default() -> Result<(), Box<dyn std::error::Error>> {
    let missing_spec = "missing-surface=loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/missing.rs";
    let no_rust_spec = "notes-only=loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/no_rust_dir";
    ...
    assert_eq!(
        ...
        "loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/missing.rs"
    );
...
fn fail_on_missing_exits_nonzero_after_emitting_report() -> Result<(), Box<dyn std::error::Error>> {
    let output = run_audit(&[
        ...
        "--fail-on-missing",
        "--artifact",
        "missing-surface=loaves/compiler/incan_driver/tests/fixtures/generated_rust_audit/missing.rs",
    ])?;
