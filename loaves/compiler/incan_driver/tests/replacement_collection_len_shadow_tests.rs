//! Same-source proof for bounded set/dict `len` through direct Body IR and receipt-backed native execution.

use incan_driver::backend::replacement::ReplacementValue;
use incan_driver::backend::selection::{BackendKind, FallbackOutcome, FallbackPolicy};
use incan_driver::backend::shadow::ShadowComparisonProfile;
...
/// Both routes return the same duplicate-normalized entry counts and preserve the exact ordinary streams.
#[test]
fn collection_len_matches_the_receipt_backed_native_route() -> Result<(), Box<dyn std::error::Error>> {
    incan_frontend::compiler_stack::run_on_compiler_stack(|| check_collection_len().map_err(|error| error.to_string()))
        .map_err(Into::into)
}

/// Compile and execute on the same compiler-sized stack used by the CLI.
fn check_collection_len() -> Result<(), Box<dyn std::error::Error>> {
    let workspace = tempfile::tempdir()?;
    let profile = ShadowComparisonProfile::new(COLLECTION_LEN_SOURCE, "collection_len", Vec::new());
    ...
    let execution = comparison
        ...
        .as_ref()
        .ok_or("missing direct collection-length execution")?;
    assert_eq!(execution.value, ReplacementValue::Int(2200));
    assert_eq!(execution.output.stdout(), b"collection len\n");
    assert!(execution.output.stderr().is_empty());

    let legacy = comparison
        .legacy
        .as_ref()
        .ok_or("missing native collection-length evidence")?;
    let replacement = comparison
        .replacement
        .as_ref()
        .ok_or("missing direct collection-length evidence")?;
