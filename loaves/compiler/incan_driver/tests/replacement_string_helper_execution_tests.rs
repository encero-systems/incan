...
use incan_driver::backend::replacement::{ProgramIo, ReplacementValue, execute_free_function_with_io};
...
fn string_membership_cli_records_the_shared_helper_result() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let receipt: incan_driver::backend::selection::BackendExecutionReceipt =
        serde_json::from_slice(&std::fs::read(temporary.path().join(".incan/backend/receipt.json"))?)?;
    receipt.verify_identity()?;
    assert_eq!(
        receipt.executed_backend,
        incan_driver::backend::selection::BackendKind::Replacement
    );
    assert_eq!(
        receipt.fallback_outcome,
        incan_driver::backend::selection::FallbackOutcome::NotNeeded
    );
