...
impl OvenStoreExecutionPayload {
    ...
    pub fn original_native_receipt(&self) -> Option<&OvenReceipt> {
    ...
    #[must_use]
    pub fn admitted_materialized_files(&self) -> &[OvenArtifactMaterializedFileManifest] {
...
pub struct OvenLegacyCargoPublisherReservation {
    /// Applied inactive-entry reclamation preceding this serialized publication.
    pub prune_report: OvenStorePruneReport,
...
impl OvenStore {
    ...
    pub fn publish_from_legacy_cargo(
        &self,
        request: &OvenArtifactPublishRequest,
    ) -> Result<OvenArtifactManifest, OvenStoreError> {
    ...
    pub fn publish_batch_from_legacy_cargo(
        &self,
        requests: &[OvenArtifactPublishRequest],
    ) -> Result<Vec<OvenArtifactManifest>, OvenStoreError> {
    ...
    pub fn inspect_for_exact_reuse(&self) -> Result<OvenStoreInspection, OvenStoreError> {
        self.ensure_layout()?;
        let manager = open_lock(&self.root.join(MANAGER_LOCK_FILE))?;
        manager.lock().map_err(|source| OvenStoreError::Io {
            path: self.root.join(MANAGER_LOCK_FILE),
    ...
    pub fn reclaim_superseded_release_entries(
        &self,
        active_domain: &str,
        apply: bool,
    ) -> Result<OvenStorePruneReport, OvenStoreError> {
    ...
    #[cfg(test)]
    pub fn ensure_layout_for_tests(&self) -> Result<(), OvenStoreError> {
        self.ensure_layout()
    }
...
pub fn sync_directory(path: PathBuf) -> Result<(), OvenStoreError> {
    File::open(&path)
...
pub fn sync_directory_tree(directory: &Path) -> Result<(), OvenStoreError> {
    for child in fs::read_dir(directory).map_err(|source| OvenStoreError::Io {
        path: directory.to_path_buf(),
        source,
    })? {
...
pub(crate) mod tests {
    ...
    /// Pruning under an active release reclaims the entries older release domains left behind, even while the store is
    /// within its byte policy; the active release's own entries stay.
    #[test]
