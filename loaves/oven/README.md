...
The build system: project model, resolution, store, direct-rustc execution, interop, Cargo compatibility, registry access. Depends on **no Incan ring**: everything Oven needs to know about Incan arrives through the provider interface that `compiler/incan_oven_facet` implements. That is the rule by dependency; by content the ring still holds Incan facts that moved with the code (the `incan_` runtime-crate prefix in `oven_rustc`, the `incan_std_core` root-extern rule, the toolchain layout tables, embedded `.incn` fixtures), and RFC 118 is where they become provider-supplied. The measured edges and their disposition are recorded on #1480.
...
| `oven_rustc/` | Direct-rustc planning and execution, the Loaf model, host/target unit graph, build-script and proc-macro host providers, the wire contract the native route reads. Runs no Cargo. |
...
| `oven_cargo_compat/` | Explicit Cargo-compatibility and adoption mode: the `legacy_cargo` baker, compiler-suite publication and the Loaf bake that run Cargo. Over `oven_rustc`; never a hidden backend. |
