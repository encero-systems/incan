...
use crate::oven_interop::LockedInteropTarget;
...
impl CargoFeatureSelection {
    /// The selection with its feature list sorted and deduplicated, the form every lock records.
...
impl IncanLock {
    /// Read and parse the lock at `path`, rejecting an unsupported format version.
    ...
    pub fn write_while_locked(&self, path: &Path, publication_lock: &PublicationLock) -> Result<(), LockfileError> {
    ...
    /// Construct a lock for callers that have no provider or package-feature semantic state; `compiler_version` is
    /// the version of the compiler writing it, recorded in the lock's header.
    pub fn new(
    ...
    /// `compiler_version` is the version of the compiler writing it, recorded in the lock's header.
    pub fn new_with_semantic(
        ...
        Self {
            ...
            incan_version: compiler_version.to_string(),
            deps_fingerprint,
...
pub fn portable_project_path(project_root: &Path, path: &Path) -> String {
...
pub fn compiler_lock_state_dir(project_root: &Path) -> PathBuf {
...
pub fn acquire_publication_lock(path: &Path) -> io::Result<PublicationLock> {
...
/// Canonicalize a Cargo.lock payload for embedding: LF line endings and exactly one trailing newline, so the same
/// lock hashes the same on every host.
...
impl CargoFeatureSelectionFingerprint {
    /// The fingerprint view of a feature selection: features sorted and deduplicated so order never changes the hash.
...
/// Collapse the whitespace in a version requirement so `>= 1.0` and `>=1.0` fingerprint alike.
...
mod tests {
    ...
    fn spawn_publication_lock_helper(
        ...
        let child = std::process::Command::new(std::env::current_exe()?)
            .args(["--exact", "lock::tests::publication_lock_process_helper", "--nocapture"])
...
"#;
    ...
    fn stale_fingerprint_is_detectable() {
        ...
        let lock = IncanLock::new("0.0.0-test", fp_v1.clone(), selection.clone(), "payload".to_string());
