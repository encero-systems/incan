...
pub const OVEN_INTEROP_BUNDLED_DIRECTORY: &str = "interop-bundled";
/// Store-owned provenance file that records the selected interop execution contract without local paths.
pub const OVEN_INTEROP_EXECUTION_PROVENANCE_PATH: &str = "provenance/interop-execution.json";
/// Caller-owned adapter manifest emitted beside staged native runtime files.
pub const OVEN_INTEROP_ADAPTER_MANIFEST_PATH: &str = "incan-interop-adapter.json";
...
pub struct OvenInteropAdapterStageRequest<'a> {
...
#[derive(Debug, Clone)]
pub struct OvenInteropAdapterStage {
...
pub struct OvenInteropNativeBakeRequest<'a> {
    ...
    pub sdk_identity_file: Option<&'a Path>,
}

/// Result of one receipt-bound interop native bake.
#[derive(Debug, Clone)]
pub struct OvenInteropNativeBake {
...
pub fn interop_execution_build_unit_inputs(receipt: &OvenInteropExecutionReceipt) -> BTreeMap<String, String> {
    BTreeMap::from([
        (
...
pub fn final_interop_plan_receipt(
...
#[must_use]
pub fn default_interop_execution_receipt_path(project_root: impl AsRef<Path>, target: &str) -> PathBuf {
...
pub fn write_interop_execution_receipt(
    ...
    let result = oven_store::write_receipt_staged(&payload, &staged, path, parent);
...
pub fn load_interop_execution_receipt(path: impl AsRef<Path>) -> Result<OvenInteropExecutionReceipt, String> {
    let path = path.as_ref();
...
pub fn validate_interop_execution_receipt(
...
/// with its runtime.
pub fn bind_interop_native_archives(
...
pub fn stage_interop_adapter(request: OvenInteropAdapterStageRequest<'_>) -> Result<OvenInteropAdapterStage, String> {
...
pub fn selected_interop_toolchain_identity(
    target: &LockedInteropTarget,
...
fn required_bundled_artifact_field(
    value: Option<&str>,
    artifact: &oven_model::oven_interop::LockedInteropArtifact,
    label: &str,
) -> Result<String, String> {
    value
...
pub fn receipt_interop_execution(
    target: &LockedInteropTarget,
...
mod tests {
    ...
    use oven_store::{OvenBuildIntent, OvenGeneratedProjectRequest, digest_bytes, receipt_generated_project};
    use std::fs;
    #[cfg(unix)]
    ...
    fn static_bake_request<'a>(
        ...
        base_receipt: &'a oven_store::OvenReceipt,
        ...
        let target = LockedInteropTarget {
            ...
            shims: vec![
                ...
                LockedInteropShim {
                    ...
                    language: oven_model::oven_interop::InteropShimLanguage::Cxx,
                    sources: vec![LockedInteropInput {
