...
pub struct OvenRuntimeClosureDependency {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct OvenRuntimeClosureUnit {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct OvenRuntimeClosurePayload {
...
impl OvenRuntimeClosurePayload {
    ...
    pub fn identity(&self) -> Result<String, OvenRustcError> {
    ...
    pub fn domain(&self) -> Result<String, OvenRustcError> {
...
impl OvenSelectedRuntimeClosure {
    ...
    pub fn payload(&self) -> &OvenRuntimeClosurePayload {
        &self.payload
    }
...
pub fn publish_runtime_closure(
    store: &OvenStore,
    receipt: &OvenReceipt,
    foundation: &ValidatedOvenRuntimeFoundation,
    build: &OvenRuntimeFoundationBuild,
...
pub fn require_runtime_closure(
    ...
    receipt: &OvenReceipt,
    expected: &OvenRuntimeClosurePayload,
) -> Result<OvenSelectedRuntimeClosure, oven_store::OvenError> {
    match select_runtime_closure(store, expected) {
        ...
        Ok(None) => Err(oven_store::OvenError::SelectedNativePlanUnavailable {
            build_unit_identity: receipt.build_unit_identity.clone(),
        }),
        Err(error) => Err(oven_store::OvenError::SelectedNativePlanUnreadable {
            build_unit_identity: receipt.build_unit_identity.clone(),
...
mod tests {
    ...
    use super::*;
    use crate::rustc::runtime_executor::tests::{FIXTURE_CLOSURE, fixture, fixture_at_version};
    use crate::rustc::{OvenRuntimeCompilerClosure, execute_runtime_foundation_rebuild};
    ...
    fn publication_receipt(
        ...
        let intent = oven_store::normalized_build_intent(target, toolchain, "debug", &[])?;
        ...
        Ok(OvenReceipt {
            ...
            identity: oven_store::receipt_identity(&identity, &sources, &intent, &compatibility)?,
            build_unit_identity: oven_store::build_unit_identity(&intent, &compatibility, &sources.build_unit_inputs)?,
            project: identity,
            sources,
            intent,
            compatibility,
    ...
    fn an_unpublished_closure_is_a_typed_selection_miss() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let Err(oven_store::OvenError::SelectedNativePlanUnavailable { build_unit_identity }) = refusal else {
