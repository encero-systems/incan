...
fn run_bounded_loaf_cargo(
    ...
    let status = loop {
        ...
        let observed = capacity_roots.iter().try_fold(0_u64, |total, root| {
            crate::legacy_cargo::conservative_directory_reservation(root).map(|bytes| total.saturating_add(bytes))
        })?;
...
mod tests {
    ...
    use crate::legacy_cargo::OvenLegacyCargoInspectionSource;
    use crate::rustc::{
    ...
    fn native_loaf_captures_search_paths_before_helper_promotion() -> Result<(), Box<dyn std::error::Error>> {
        ...
        plan.supporting_artifacts
            .push(crate::rustc::OvenRustcSupportingArtifact {
    ...
    fn native_loaf_promotes_compiler_runtime_externs_for_compatible_callers() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        plan.entrypoint_dependency_search_paths.insert(
            ...
            crate::rustc::OvenRustcSourceSearchClosure::default(),
        );
        plan.supporting_artifacts = vec![
            crate::rustc::OvenRustcSupportingArtifact {
    ...
    fn native_loaf_keeps_compiler_vocab_helpers_off_generated_root() -> Result<(), Box<dyn std::error::Error>> {
        ...
        super::promote_compiler_runtime_externs(&mut plan)?;
        plan.externs.extend([
            crate::rustc::OvenRustcArtifactExtern {
    ...
    fn native_vocab_loaf_copies_only_cargo_reported_artifact_closure() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let selected = crate::rustc::trusted_artifact_plan_for_source_evidence(&materialized, &plan, "generated-root")?;
