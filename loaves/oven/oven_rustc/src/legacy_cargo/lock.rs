...
pub type CargoLockPackageIdentity = (String, String, Option<String>);

/// One resolved package node from a Cargo lock graph.
pub struct CargoLockPackageNode {
...
pub fn cargo_lock_package_graph(
    cargo_lock: &[u8],
    field: &'static str,
...
pub fn validate_release_cohort_registry_lock(
    release_lock: &[u8],
    seeded_lock: &[u8],
    generated_lock: &[u8],
...
pub fn validate_generated_registry_lock(
    compiler_lock: &[u8],
    generated_lock: &[u8],
) -> Result<(), OvenLegacyCargoError> {
...
pub fn locked_generated_project(
    manifest_path: &Path,
    manifest: &[u8],
    source_lock: &[u8],
...
pub fn release_cohort_generated_project_lock(
    manifest_path: &Path,
    manifest: &[u8],
    source_lock: &[u8],
) -> Result<Vec<u8>, OvenLegacyCargoError> {
...
pub fn locked_generated_project_with_registry_policy(
    manifest_path: &Path,
    manifest: &[u8],
    source_lock: &[u8],
...
pub fn lock_package_identities(lock: &toml::Value) -> Vec<(String, String, Option<String>)> {
    lock.get("package")
        .and_then(toml::Value::as_array)
        .into_iter()
        .flatten()
...
pub fn lock_contains_package(lock: &toml::Value, name: &str, version: &str, source: Option<&str>) -> bool {
