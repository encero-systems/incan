...
The direct installer links `incan` and `incan-lsp` into `~/.local/bin` by default and provisions the Rust backend through `rustup` when needed, including the `wasm32-wasip1` target used by packages with vocab companions. The pipx package delegates to that same installer and verified toolchain archive contract. npm installs script-free command shims plus a host-specific platform package, and Homebrew installs the prebuilt Incan commands through the generated formula; both npm and Homebrew expect Rust and `wasm32-wasip1` to be managed separately. Cargo installation compiles from source and is mainly for Rust users who prefer that workflow. See [Install and run Incan](workspaces/docs-site/docs/tooling/how-to/install_and_run.md) for supported hosts, dry-run installation, manifest pinning, Cargo installation, and source-build fallback instructions.
